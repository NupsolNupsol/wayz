...
export const useDeliveryValidator = (
  ...
  switch (transitionCode) {
    ...
    case 'TO_RELEASE_APPROVED': {
      ...
      const confirming =
        typeof payload.confirmCourierId === 'string' ? payload.confirmCourierId : '';
      if (!confirming) {
        errors.push('Confirm which courier is collecting before approving.');
      } else if (confirming !== delivery.assignedTo) {
        errors.push(
          'That is not the courier this delivery is assigned to — do not release the bags.'
        );
      }
    ...
    case 'TO_PICKED_UP': {
      ...
      if (duplicates.size)
        errors.push(`Duplicate scan(s): ${[...duplicates].join(', ')} — scan each bag once.`);

      const foreign = [...seen].filter((s) => !known.has(s));
      if (foreign.length) {
        errors.push(
          `Wrong bag — ${foreign.length} barcode(s) do not belong to this customer: ${foreign.join(', ')}.`
        );
      }
      ...
      if (missing.length) {
        errors.push(
          `${missing.length} bag(s) not scanned (${missing.map((b) => `Bag ${b.index}`).join(', ')}).`
        );
      }
      break;
    }
    ...
    case 'TO_FAILED': {
      ...
      if (!String(payload.reason ?? '').trim())
        errors.push('Record why the delivery could not be completed.');
      break;
    }

    case 'TO_CANCELLED': {
      if (!String(payload.reason ?? '').trim())
        errors.push('A reason is required to cancel a delivery.');
