 ...
 */
export const usePhotographyOperation = (transitionCode: string, ctx: WorkflowContext): OperationResult => {
   ...
   */
  const delivery = String(ctx.payload.delivery ?? ctx.booking.metadata?.delivery ?? 'DIGITAL').toUpperCase()
