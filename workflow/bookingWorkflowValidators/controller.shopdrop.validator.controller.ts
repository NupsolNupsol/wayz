...
function validateBagScans(ctx: WorkflowContext): string[] {
  ...
  if (foreign.length) {
    errors.push(
      `Wrong bag — ${foreign.length} scanned barcode(s) do not belong to this booking: ${foreign.join(', ')}.`
    );
  }
...
function validateCompartmentScan(ctx: WorkflowContext): string[] {
  ...
  const scanned =
    typeof ctx.payload.scannedUnitId === 'string' ? ctx.payload.scannedUnitId.trim() : '';
...
function validateAllBagsRetrieved(ctx: WorkflowContext): string[] {
  const notOut = ctx.booking.bags.filter(
    (b) => b.status !== BAG_RETRIEVED && b.status !== BAG_DELIVERED
  );
  return notOut.length ? [`Cannot complete — ${notOut.length} bag(s) not yet scanned out.`] : [];
}

export const useShopDropValidator = (
