...
export async function tenantOverview(scope: ManagerScope) {
  ...
  const overdue = live.filter(
    (b) => b.session?.expectedEndAt && computeOvertime(b.session as never, now).isOvertime
  );
  ...
  const byRole = Object.fromEntries(
    ROLES.map((r) => [r, staff.filter((u) => u.role === r).length])
  ) as Record<Role, number>;

  const byEngine = ENGINE_KINDS.map((engineKind) => {
    const ids = new Set(assetTypes.filter((a) => a.engineKind === engineKind).map((a) => a._id));
  ...
  const perSite = sites.map((site) => {
    ...
    return {
      ...
      revenue30d: round2(
        sales.filter((p) => stationIds.has(p.stationId)).reduce((x, p) => x + p.amount, 0)
      ),
    };
  });
  ...
  return {
    ...
    estate: {
      ...
      outOfService: units.filter((u) =>
        ['OUT_OF_SERVICE', 'MAINTENANCE', 'BLOCKED'].includes(u.status)
      ).length,
    ...
    operations: {
      ...
      deliveriesOpen: deliveries.filter(
    ...
    money: {
      ...
      cash30Days: round2(
        sales.filter((p) => p.method === 'CASH').reduce((x, p) => x + p.amount, 0)
      ),
      card30Days: round2(
        sales.filter((p) => p.method !== 'CASH').reduce((x, p) => x + p.amount, 0)
      ),
...
export async function updateCompany(scope: ManagerScope, patch: CompanyPatch) {
  ...
  if (patch.enabledEngines) {
    ...
    if (patch.enabledEngines.length === 0)
      throw ApiError.badRequest('At least one engine must stay enabled.');
  ...
  if (patch.company)
    tenant.company = { ...tenant.company, ...patch.company } as typeof tenant.company;
  if (patch.branding)
    tenant.branding = { ...tenant.branding, ...patch.branding } as typeof tenant.branding;
...
export async function readRules(scope: ManagerScope) {
  ...
  return {
    ...
    engineKinds:
      (await Tenant.findById(scope.tenantId, { enabledEngines: 1 }).lean())?.enabledEngines ?? [],
    roles: [...ROLES],
    defaults: {
...
export async function updateRules(
  ...
  if (patch.rental) {
    ...
    if (merged.statedGraceMin > merged.graceMin) {
      throw ApiError.badRequest(
        'The grace you tell the customer cannot be longer than the one the system allows.'
      );
    }
  ...
  if (patch.penalties) {
    ...
    for (const rule of patch.penalties) {
      ...
      if (rule.amount !== null && (!Number.isFinite(rule.amount) || rule.amount < 0)) {
        throw ApiError.badRequest(
          `Penalty "${code}" needs an amount of zero or more, or none at all for a replacement value.`
        );
      }
  ...
  if (patch.discountReasons) {
    ...
    for (const reason of patch.discountReasons) {
      ...
      seen.add(code);
      if (!reason.label?.trim())
        throw ApiError.badRequest(`Discount reason "${code}" needs a label.`);
      const pct = Number(reason.maxPercent);
      if (!Number.isFinite(pct) || pct < 0 || pct > 100) {
        throw ApiError.badRequest(
          `Discount reason "${code}" needs a ceiling between 0 and 100 percent.`
        );
      }
    }
  ...
  if (patch.transfers) {
    ...
    if (merged.requesters.length === 0)
      throw ApiError.badRequest('Somebody has to be able to raise a transfer.');
    if (merged.approvers.length === 0)
      throw ApiError.badRequest('Somebody has to be able to approve a transfer.');
  ...
  if (patch.procurement) {
    ...
    if (merged.standardApprovers.length === 0 || merged.highValueApprovers.length === 0) {
      throw ApiError.badRequest(
        'Both the standard and the high-value approver lists need somebody in them.'
      );
    }
