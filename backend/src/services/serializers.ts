...
export async function withAmountsDue<
  T extends { orderId: string; session?: { overtime?: ReturnType<typeof computeOvertime> } },
>(tenantId: string, rows: T[]) {
  ...
  return rows.map((row) => {
    ...
    const running = row.session?.overtime
      ? pendingOvertime(row.session.overtime, linesOf.get(row.orderId) ?? [])
      : 0;
    return {
      ...row,
...
export function bookingDTO(b: BookingDoc) {
  const obj =
    typeof (b as unknown as { toObject?: () => unknown }).toObject === 'function'
