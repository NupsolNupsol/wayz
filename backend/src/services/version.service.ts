...
type ChangeInput = Omit<VersionChange, 'checks' | 'issues'> & {
  checks?: VersionChange['checks'];
  issues?: VersionChange['issues'];
};
...
export async function listVersions() {
  ...
  return rows.map((v) => ({
    ...
    openIssues: (v.changes ?? []).reduce(
      (sum, c) => sum + (c.issues ?? []).filter((i) => i.status === 'OPEN').length,
      0
    ),
...
export async function writeVersion(input: VersionInput, id?: string) {
  ...
  if (id) {
    const existing = await Version.findOneAndUpdate(
      { _id: id },
      { $set: patch },
      { new: true }
    ).lean();
