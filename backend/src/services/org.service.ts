...
export async function orgTree(scope: ManagerScope) {
  const [sites, stations, kiosks, gates, unitAgg, bookingAgg] = await Promise.all([
    Site.find({ tenantId: scope.tenantId, active: { $ne: false } })
      .sort({ name: 1 })
      .lean(),
    Station.find({ tenantId: scope.tenantId, active: { $ne: false } })
      .sort({ name: 1 })
      .lean(),
    Kiosk.find({ tenantId: scope.tenantId, active: { $ne: false } })
      .sort({ name: 1 })
      .lean(),
    Gate.find({ tenantId: scope.tenantId, active: { $ne: false } })
      .sort({ name: 1 })
      .lean(),
    AssetUnit.aggregate([
  ...
  type UnitRow = {
    _id: { stationId: string; kioskId: string | null; gateId: string | null };
  } & Counts;
  for (const row of unitAgg as UnitRow[]) {
    ...
    if (row._id.kioskId)
      byKiosk.set(row._id.kioskId, {
        total: row.total,
        available: row.available,
        inUse: row.inUse,
      });
    if (row._id.gateId)
      byGate.set(row._id.gateId, { total: row.total, available: row.available, inUse: row.inUse });
  }

  const activeByStation = new Map(
    bookingAgg.map((b: { _id: string; active: number }) => [b._id, b.active])
  );
...
export async function stationMap(tenantId: string, siteId?: string) {
  const [sites, stations, kiosks] = await Promise.all([
    Site.find({ tenantId, active: { $ne: false } })
      .sort({ name: 1 })
      .lean(),
    Station.find({ tenantId, active: { $ne: false }, ...(siteId ? { siteId } : {}) })
      .sort({ name: 1 })
      .lean(),
    Kiosk.find({ tenantId, active: { $ne: false }, ...(siteId ? { siteId } : {}) })
      .sort({ name: 1 })
      .lean(),
  ]);
...
export async function saveStationMap(
  ...
  if (missing.length)
    throw ApiError.badRequest(`Nothing in this tenant to place: ${missing.join(', ')}.`);
...
export async function updateSite(
  scope: ManagerScope,
  id: string,
  patch: Partial<SiteInput> & { active?: boolean }
) {
  ...
  if (patch.active === false) {
    const live = await Booking.countDocuments({
      tenantId: scope.tenantId,
      stationId: {
        $in: (await Station.find({ tenantId: scope.tenantId, siteId: id }).lean()).map(
          (s) => s._id
        ),
      },
    ...
    if (live > 0)
      throw ApiError.unprocessable(
        `This site still has ${live} live session(s) across its stations.`
      );
  }

  const clean = sanitise(patch);
  if (clean.venueType !== undefined)
    clean.venueType = normaliseVenueType(clean.venueType as string);
...
export async function updateStation(
  scope: ManagerScope,
  id: string,
  patch: Partial<StationInput> & { active?: boolean }
) {
  ...
  if (patch.active === false) {
    ...
    if (live > 0) {
      throw ApiError.unprocessable(
        `${station.name} still has ${live} live session(s) — close them before deactivating it.`
      );
    }
  }
...
export async function createKiosk(scope: ManagerScope, input: KioskInput) {
  ...
  /*
   * A desk serves one of the activities its organisation has adopted.
   *
   * Every activity is a coded module registered in the platform's catalogue; an organisation
   * adopts the ones it runs, and a counter names one of those. Checking against what the
   * *station* runs is the same rule it always was.
   */
  ...
  if (input.engineKind) {
    ...
    throw ApiError.badRequest('Choose what this desk sells.', [
      'One of the activities this organisation has adopted.',
    ]);
  }
...
export async function updateKiosk(
  scope: ManagerScope,
  id: string,
  patch: Partial<KioskInput> & { active?: boolean }
) {
  const kiosk = await Kiosk.findOne({ _id: id, tenantId: scope.tenantId });
  if (!kiosk) throw ApiError.notFound('Kiosk not found.');

  if (patch.active === false) {
    const busy = await AssetUnit.countDocuments({
      tenantId: scope.tenantId,
      kioskId: id,
      status: { $in: IN_USE },
    });
    if (busy > 0)
      throw ApiError.unprocessable(`${kiosk.name} has ${busy} unit(s) in use — empty it first.`);
  }
  ...
  if (patch.engineKind && patch.engineKind !== kiosk.engineKind) {
    ...
    const staffed = await User.countDocuments({
      tenantId: scope.tenantId,
      kioskId: id,
      active: true,
    });
    if (staffed > 0) {
      throw ApiError.unprocessable(
        `${kiosk.name} still has ${staffed} staff member(s) assigned — reassign them first.`
      );
    }
...
export async function removeKiosk(scope: ManagerScope, id: string) {
  ...
  if (live > 0)
    throw ApiError.unprocessable(
      `${kiosk.name} still has ${live} live session(s) — finish them first.`
    );
  if (units > 0)
    throw ApiError.unprocessable(
      `${kiosk.name} still holds ${units} unit(s) — move them to another desk first.`
    );
  if (staff > 0)
    throw ApiError.unprocessable(
      `${kiosk.name} still has ${staff} staff member(s) assigned — reassign them first.`
    );
...
export async function removeStation(scope: ManagerScope, id: string) {
  ...
  if (live > 0)
    throw ApiError.unprocessable(
      `${station.name} still has ${live} live session(s) — finish them first.`
    );
  if (desks > 0)
    throw ApiError.unprocessable(`${station.name} still has ${desks} desk(s) — remove them first.`);
  if (units > 0)
    throw ApiError.unprocessable(
      `${station.name} still holds ${units} asset(s) — move them first.`
    );
  if (staff > 0)
    throw ApiError.unprocessable(
      `${station.name} still has ${staff} staff member(s) — reassign them first.`
    );
...
export async function removeSite(scope: ManagerScope, id: string) {
  ...
  if (stations > 0)
    throw ApiError.unprocessable(
      `${site.name} still has ${stations} live station(s) — remove them first.`
    );
...
function assertStationRuns(runs: EngineKind[], stationName: string, engineKind: EngineKind) {
  if (!ENGINE_KINDS.includes(engineKind))
    throw ApiError.badRequest(`Unknown activity "${engineKind}".`);
  if (!runs.includes(engineKind)) {
    throw ApiError.badRequest(
      `${stationName} does not run ${engineKind.replaceAll('_', ' ').toLowerCase()}.`
    );
  }
}
...
export async function createGate(scope: ManagerScope, input: GateInput) {
  ...
  const clash = await Gate.findOne({
    tenantId: scope.tenantId,
    stationId: input.stationId,
    name,
    active: { $ne: false },
...
export async function updateGate(
  scope: ManagerScope,
  id: string,
  patch: Partial<GateInput> & { active?: boolean }
) {
  ...
  if (patch.active === false) {
    ...
    if (busy > 0)
      throw ApiError.unprocessable(`${gate.name} has ${busy} locker(s) in use — empty it first.`);
  }
...
export async function removeGate(scope: ManagerScope, id: string) {
  ...
  if (units > 0)
    throw ApiError.unprocessable(
      `${gate.name} still holds ${units} locker(s) — move them to another gate first.`
    );
  if (staff > 0)
    throw ApiError.unprocessable(
      `${gate.name} still has ${staff} staff member(s) posted to it — reassign them first.`
    );
