...
export function getAvailableTransitions(
  ...
  if (!wf) {
    return {
      allowed: false,
      message: `No workflow registered for ${booking.engineKind}.`,
      transitions: [],
    };
  }
  if (!roles.some((r) => wf.actors.includes(r))) {
    return {
      allowed: false,
      message: `You have no permission on ${booking.engineKind}.`,
      transitions: [],
    };
  }
  ...
  return {
    ...
    message: available.length
      ? 'Transitions fetched.'
      : `No transitions available from "${booking.status}".`,
...
function applySnapshot(booking: BookingHydrated, next: BookingSnapshot): void {
  ...
  booking.session = {
    ...booking.session,
    ...next.session,
  } as unknown as BookingHydrated['session'];
...
async function applyAssetIntents(intents: AssetIntent[], tenantId: string): Promise<void> {
  for (const intent of intents) {
    ...
    if (intent.restingUntil !== undefined)
      patch.restingUntil = intent.restingUntil ? new Date(intent.restingUntil) : null;
...
async function noUnitHere(
  ...
  const names = await AssetType.find(
    {
      _id: { $in: otherSizes.map((row) => row._id).filter((id) => id !== assetTypeId) },
      tenantId,
      kind: type?.kind,
    },
    { name: 1 }
  ...
  if (free.length)
    lines.push(`Free at ${here}: ${free.join(', ')} — go back and pick one of those.`);
  if (elsewhere > 0)
    lines.push(`${elsewhere} ${size} are free at other desks, but a desk can only lock its own.`);
  return lines;
}
...
export async function checkTransition(params: ApplyTransitionParams) {
  ...
  const wf = getWorkflow(booking.engineKind);
  if (!wf)
    throw ApiError.unprocessable(`No workflow registered for engine "${booking.engineKind}".`);

  const transition = wf.transitions.find(
    (t) => t.code === code && t.source.includes(booking.status)
  );
  if (!transition) {
    throw ApiError.unprocessable(
      `Transition "${code}" is not allowed from status "${booking.status}".`
    );
  }
  /*
  ...
  if (!validator)
    throw ApiError.unprocessable(`No validator registered for "${booking.engineKind}".`);
...
export async function applyTransition(
  ...
  if (!operator)
    throw ApiError.unprocessable(`No operator registered for "${booking.engineKind}".`);
  const result = await operator(code, ctx);
  if (result.errors.length) {
    if (result.errors.some((e) => NO_UNIT_ERRORS.includes(e))) {
      const detail = await noUnitHere(
        tenantId,
        stationId,
        kioskId,
        booking.metadata?.assetTypeId as string | undefined
      );
