...
function getUnitCode(engineType: string | null | undefined): string {
  if (engineType && ['SHOP_AND_DROP', 'MOBILITY'].includes(engineType)) {
    return 'HUR';
  }
...
export async function buildInvoiceXml(
  ...
  /*
   * The seller, including the address ZATCA requires.
   *
   * `generateInvoiceXml` throws outright without `address` — BR-KSA-09 wants the seller's
   * street, building, district, city and postal code as separate elements, not one line — so
   * this is checked here and refused by name rather than surfacing as "Cannot read properties
   * of undefined".
   */
  const address = tenant.zatcaAddress ?? ({} as NonNullable<typeof tenant.zatcaAddress>);
  const missing = [
    !tenant.legalName?.trim() && 'the registered legal name',
    !tenant.vatNumber?.trim() && 'the VAT number',
    !address.street?.trim() && 'the street',
    !address.building?.trim() && 'the building number',
    !address.district?.trim() && 'the district',
    !address.city?.trim() && 'the city',
    !address.postalCode?.trim() && 'the postal code',
  ].filter(Boolean) as string[];

  if (missing.length > 0) {
    throw ApiError.unprocessable(
      `This company cannot issue a ZATCA invoice yet — it is missing ${missing.join(', ')}.`,
  ...
  const supplier = {
    ...
    address: {
      ...
      ...(address.additionalNumber?.trim() ? { additionalNumber: address.additionalNumber } : {}),
      district: address.district,
      ...(address.countrySubentity?.trim() ? { countrySubentity: address.countrySubentity } : {}),
      city: address.city,
