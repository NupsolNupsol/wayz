...
import {
  OVERTIME_LINE_PRODUCT_ID,
  computeOvertime,
  describeOvertime,
  pendingOvertime,
} from '../domain/overtime.js';
...
async function loadBooking(scope: Scope, bookingId: string): Promise<BookingHydrated> {
   ...
   */
  if (!mine && !canWorkEngine(scope, booking.engineKind))
...
async function assertDeskCanFulfil(
  ...
  const elsewhere =
    freeElsewhere > 0
      ? [`${freeElsewhere} are free elsewhere, but this desk cannot reach them.`]
      : [];
...
export async function createBooking(scope: Scope, input: CreateBookingInput) {
  ...
  if (product.engineKind !== input.engineKind)
    throw ApiError.badRequest('Product does not belong to that engine.');
  ...
  if (byTours && !(product.tourPrice && product.tourPrice > 0)) {
    throw ApiError.unprocessable(
      `${product.name} is not sold by the tour — price it by the hour, or give it a tour price.`
    );
  }
   ...
   */
  const sessionLength =
    wf.sessionKind === 'EXPERIENCE' && product.tourMinutes ? product.tourMinutes : undefined;
  const durationMin = ridesOnly
    ? 0
    : byTours
      ? tours * tourMinutes
      : (input.durationMin ?? sessionLength ?? 120);
  ...
  if (input.engineKind === 'SHOP_AND_DROP') {
    ...
    if (bagInputs.length < 1)
      throw ApiError.badRequest('At least one bag is required for Shop & Drop.');
    ...
    const quote = priceQuote(
      ...
      product.basePrice,
      bags.length,
      packed.numberOfCompartmentsRequired,
      periods,
      product.basePrice
    );
    ...
    lines.push({
      ...
      name: soldAs,
      nameAr: byTours
        ? lineNameAr.tours(arabicProduct, tours)
        : lineNameAr.quantity(arabicProduct, quantity),
      quantity: 1,
  ...
  const order = await Order.create({
    ...
    hold: holdAssetTypeId
      ? {
          assetTypeId: holdAssetTypeId,
          quantityRequired: holdQty,
  ...
  if (input.engineKind === 'LAGOON') {
    ...
    const people = Math.max(
      1,
      Math.floor(Number((input.metadata ?? {}).visitors ?? quantity) || 1)
    );
    if (people > boat.free) {
      throw ApiError.unprocessable(
        boat.free === 0
...
export async function discountBooking(
  ...
  if (order.status === 'PAID')
    throw ApiError.unprocessable('That sale is already paid — refund it instead.');

  const rules = await tenantRules(scope.tenantId);
  const reason =
    systemReason ??
    rules.discountReasons.find((r) => r.code === input.reasonCode.trim().toUpperCase());
  ...
  const asked =
    input.percent !== undefined
      ? (chargeable * Math.max(0, Math.min(100, input.percent))) / 100
  ...
  if (existing) {
    const codeAlreadySpent = await Voucher.exists({
      tenantId: scope.tenantId,
  ...
  if (existing) {
    ...
    order.lines.push({
      productId: DISCOUNT_LINE_PRODUCT_ID,
      name,
      nameAr: arabicName,
      quantity: 1,
      unitPrice: -off,
...
async function resolvePayers(
  ...
  const others = [
    ...new Set(
      splits.map((s) => s.payerId).filter((id): id is string => !!id && id !== booking.customerId)
    ),
  ];
  ...
  if (missing.length)
    throw ApiError.badRequest(`No customer ${missing.join(', ')} in this tenant.`);
...
export async function payBooking(scope: Scope, bookingId: string, splits: PaymentSplit[]) {
  ...
  if (booking.status !== 'DRAFT')
    throw ApiError.badRequest(`Cannot pay a booking in status ${booking.status}.`);
  ...
  for (const payer of payers) {
    ...
    throw ApiError.unprocessable(
      payer.isPrimary
        ? 'Confirm the customer before taking their money.'
        : `Confirm ${payer.name} before taking their money.`,
      [
    ...
    if (split.method === 'CARD') {
      if (!split.cardScheme) {
        throw ApiError.badRequest(
          'Say which card was used — the bank charges a different commission for each.'
        );
      }
  ...
  await Payment.insertMany(
    splits.map((s, i) => {
      ...
      const tax =
        kind === 'DEPOSIT' ? noVat(round2(s.amount)) : splitInclusive(round2(s.amount), vatRate);
      return {
        _id: paymentIds[i],
        tenantId: scope.tenantId,
        stationId: scope.stationId,
...
async function confirmAlreadyPaid(
  scope: Scope,
  booking: BookingHydrated,
  order: { _id: string; updatedAt?: Date }
) {
...
async function applyOvertimeCharge(scope: Scope, booking: BookingHydrated): Promise<number> {
  ...
  const charged = round2(
    state.penaltyAmount - (existing ? existing.unitPrice * (existing.quantity ?? 1) : 0)
  );
 ...
 */
async function settleWrongDesk(
  scope: Scope,
  booking: BookingHydrated,
  audits: AuditIntent[]
): Promise<boolean> {
    ...
    Payment.aggregate([
      { $match: { tenantId, orderId, status: 'CAPTURED' } },
      {
        $group: {
          ...
          total: {
            $sum: {
              $cond: [{ $eq: ['$kind', 'REFUND'] }, { $multiply: ['$amount', -1] }, '$amount'],
            },
          },
        },
      },
...
export async function settleOutstanding(
  ...
  return {
    ...
    collected: paid,
    due: await amountDue(scope.tenantId, order._id),
  };
}

export async function returnAtStation(
  scope: Scope,
  bookingId: string,
  code: string,
  payload: TransitionPayload = {}
...
export async function scanBagOut(scope: Scope, bookingId: string, barcode: string) {
  const booking = await loadBooking(scope, bookingId);
  if (booking.status !== 'RETRIEVAL_IN_PROGRESS')
    throw ApiError.badRequest('Booking is not in retrieval.');
  const bag = booking.bags.find((b) => b.barcode === barcode.trim());
  if (!bag)
    throw ApiError.unprocessable('Wrong bag — this barcode does not belong to this booking.');
  if (bag.status === 'RETRIEVED' || bag.status === 'DELIVERED')
    throw ApiError.badRequest('Bag already scanned out.');
  ...
  booking.custody.push({
    ...
    at: new Date(),
    bagIndex: bag.index,
    note: `Bag ${bag.index} scanned out`,
  });
  await booking.save();
  return booking;
}
