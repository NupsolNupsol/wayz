...
function applySnapshot(doc: DeliveryRequestDoc, next: DeliverySnapshot): void {
  ...
  doc.timeline = next.timeline.map((t) => ({
    status: t.status,
    at: new Date(t.at),
    by: t.by,
    note: t.note,
...
export async function courierScopeFrom(
  tenantId: string,
  stationId: string,
  userId: string,
  role: Role
): Promise<CourierScope> {
...
export async function customerBagsElsewhere(scope: Scope, bookingId: string) {
  ...
  const open = await DeliveryRequest.find({
    ...
    $or: [
      { bookingId: { $in: held.map((b) => b._id) } },
      { 'stops.bookingId': { $in: held.map((b) => b._id) } },
    ],
  })
  ...
  const unitIds = held
    .map((b) => b.reservation?.assetUnitId ?? b.assetUnitId)
...
async function gatherExtraStops(scope: Scope, booking: BookingHydrated, bookingIds: string[]) {
  ...
  const bookings = await Booking.find(
    { tenantId: scope.tenantId, _id: { $in: wanted } },
    { bags: 1 }
  ).lean();
...
export async function createDeliveryRequest(scope: Scope, input: CreateDeliveryInput) {
  ...
  if (!ACTIVE_BOOKING_STATES.includes(booking.status)) {
    throw ApiError.unprocessable(
      `The bags must be in storage to arrange a delivery (this booking is ${booking.status}).`
    );
  }
  ...
  if (input.toKioskId) {
    const found = await Kiosk.findOne(
      { _id: input.toKioskId, tenantId: scope.tenantId },
      { name: 1 }
    ).lean();
  ...
  if (!address)
    throw ApiError.badRequest('Say where the bags are going — an exit gate, or an address.');
  ...
  if (existing)
    throw ApiError.conflict(`A delivery (${existing._id}) is already open for this booking.`);
  ...
  const unit = unitId
    ? await AssetUnit.findOne({ _id: unitId, tenantId: scope.tenantId }).lean()
    : null;
  ...
  const homeKiosk =
    (booking.metadata?.kioskId as string | undefined) ?? unit?.kioskId ?? booking.kioskId ?? null;
...
export async function requestStorageRun(scope: Scope, bookingId: string) {
  ...
  if (booking.status !== 'RESERVED') {
    throw ApiError.unprocessable(
      `${booking.ref} is ${booking.status.toLowerCase()}, so there is nothing to carry yet.`,
      [
  ...
  if (!unit?.gateId) {
    throw ApiError.unprocessable(
      'No locker is being held for this booking, so there is nowhere to carry the bags.'
    );
  }
...
export async function courierBoard(scope: CourierScope) {
  ...
  const withDue = await Promise.all(mine.map(async (d) => d));
...
export async function exitGates(scope: Scope) {
  const station = await Station.findOne(
    { _id: scope.stationId, tenantId: scope.tenantId },
    { siteId: 1 }
  ).lean();
...
export async function stationDeliveries(
  scope: Scope,
  opts: { status?: string; bookingId?: string } = {}
) {
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      activeStop: stop,
      atMyKiosk: stop
        ? stop.kioskId === (scope.kioskId ?? null)
        : row.kioskId === (scope.kioskId ?? null),
    };
  });
}
...
export async function deliveryDetail(tenantId: string, id: string, actor?: DeliveryActor) {
  ...
  return {
    ...
    booking: booking
      ...
      : null,
    courier: courier
      ? { _id: courier._id, fullName: courier.fullName, email: courier.email, phone: courier.phone }
      : null,
...
export async function applyDeliveryTransition(params: ApplyDeliveryParams) {
  ...
  if (!transition) {
    throw ApiError.unprocessable(
      `Transition "${code}" is not allowed from status "${doc.status}".`
    );
  }
  ...
  if (code === 'TO_DELIVERED' && carryingIn) {
    if (!booking)
      throw ApiError.unprocessable('The booking these bags belong to no longer exists.');
  ...
  if (validation.errors.length)
    throw ApiError.unprocessable(`Cannot ${transition.label}.`, validation.errors);
  ...
  if (code === 'TO_ASSIGNED') {
    const claimed = await DeliveryRequest.updateOne(
      { _id: doc._id, tenantId: actor.tenantId, status: DLV_REQUESTED, assignedTo: null },
      {
        $set: {
          ...
          assignedAt: now,
          timeline: result.delivery.timeline.map((t) => ({
            status: t.status,
            at: new Date(t.at),
            by: t.by,
            note: t.note,
    ...
    if (claimed.modifiedCount !== 1)
      throw ApiError.conflict('Another courier claimed this delivery first.');
  } else {
  ...
  if (code === 'TO_RELEASE_REQUESTED') {
    ...
    const courier = await User.findOne(
      { _id: actor.userId, tenantId: actor.tenantId },
      { fullName: 1 }
    ).lean();
    await raise({
      tenantId: actor.tenantId,
...
async function applyBookingEffects(
  ...
  if (code === 'TO_PICKED_UP') {
    ...
    booking.custody.push({
      ...
      to: 'PORTER',
      at: now,
      note: `Collected by courier for ${delivery._id}`,
    });
    ...
    if (delivery.destination?.kind === 'GATE') {
      ...
      booking.custody.push({
        ...
        to: 'CUSTOMER',
        at: now,
        note: `Delivered to ${delivery.destination.address}`,
      });
    }
  }
...
export async function collectStop(
  ...
  if (stops.length === 0) {
    return (
      await applyDeliveryTransition({
        ...
        id,
        code: 'TO_PICKED_UP',
        payload: { scannedBarcodes: input.scannedBarcodes },
      })
    ).delivery;
  }
  ...
  if (!current)
    throw ApiError.unprocessable('Every kiosk on this delivery has already been collected.');
