...
export async function listPricing(scope: ManagerScope) {
  const [products, assetTypes, tenant, stations, kiosks, unitPlaces, revenueAgg] =
    await Promise.all([
      ...
      AssetUnit.aggregate([
        ...
        {
          $group: {
            _id: { assetTypeId: '$assetTypeId', stationId: '$stationId', kioskId: '$kioskId' },
            count: { $sum: 1 },
          },
        },
      ]),
  ...
  type Place = {
    _id: { assetTypeId: string; stationId: string; kioskId: string | null };
    count: number;
  };
  ...
  const usage = new Map(
    revenueAgg.map((r: { _id: string; bookings: number }) => [r._id, r.bookings])
  );
  ...
  return {
    ...
    products: products.map((p) => ({
      ...
      unitsHere:
        p.assetTypeId && p.stationId
          ? countAtDesk(p.assetTypeId, p.stationId, p.kioskId ?? null)
          : null,
...
function validate(input: Partial<ProductInput>) {
  ...
  if (
    ...
    (!Number.isFinite(input.overtimeHourlyRate) || input.overtimeHourlyRate < 0)
  ) {
    throw ApiError.badRequest('Overtime rate must be zero or more.');
  }
  if (
    input.depositRequired !== undefined &&
    (!Number.isFinite(input.depositRequired) || input.depositRequired < 0)
  ) {
    throw ApiError.badRequest('Deposit must be zero or more.');
  }
  if (
    input.penaltyPrice !== undefined &&
    (!Number.isFinite(input.penaltyPrice) || input.penaltyPrice < 0)
  ) {
  ...
  if (input.saleType === 'SALE' && input.overtimeHourlyRate) {
    throw ApiError.badRequest(
      'A sale has no overtime — leave the hourly rate empty or make it a rental.'
    );
  }
}

/** How many of a kind are standing at a desk right now. */
export async function unitsAtDesk(
  tenantId: string,
  assetTypeId: string,
  stationId: string,
  kioskId: string | null
...
export async function updateProduct(scope: ManagerScope, id: string, patch: Partial<ProductInput>) {
  ...
  if (patch.overtimeHourlyRate !== undefined) {
    product.overtimeHourlyRate =
      patch.overtimeHourlyRate == null ? null : round2(patch.overtimeHourlyRate);
  }
  if (patch.hourlyPrice !== undefined)
    product.hourlyPrice = patch.hourlyPrice == null ? null : round2(patch.hourlyPrice);
  if (patch.tourPrice !== undefined)
    product.tourPrice = patch.tourPrice == null ? null : round2(patch.tourPrice);
