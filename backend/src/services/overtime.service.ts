...
export async function sweepExpiryWarnings(now: Date = new Date()): Promise<number> {
  ...
  for (const booking of due) {
    const minutesLeft = Math.max(
      1,
      Math.round((new Date(booking.session.expectedEndAt!).getTime() - now.getTime()) / 60_000)
    );
...
export async function runSessionSweeps(
  now: Date = new Date()
): Promise<{ warned: number; flipped: number; rested: number }> {
