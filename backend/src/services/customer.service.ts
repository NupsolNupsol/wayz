...
export async function rememberPhoneVerified(tenantId: string, destination: string) {
  ...
  const written = await Customer.updateMany(
    { tenantId, $or: match },
  ...
  if (written.matchedCount === 0) {
    logger.warn('confirmation matched no customer, so no proof was recorded', {
      tenantId,
      destination: value,
    });
  }
...
export async function phoneProofIsFresh(tenantId: string, phone: string, now = new Date()) {
  ...
  return (
    now.getTime() - new Date(customer.phoneVerifiedAt).getTime() <= PHONE_PROOF_TTL_MIN * 60_000
  );
}
