...
export async function ageBooking(scope: Scope, bookingId: string, minutes: number) {
  ...
  if (!Number.isFinite(forward) || forward === 0)
    throw ApiError.badRequest('Say how many minutes to move.');
  if (Math.abs(forward) > 60 * 24 * 7)
    throw ApiError.badRequest('A week either way is as far as this goes.');
  ...
  if (!atMyGate && !canWorkEngine(scope, booking.engineKind))
  ...
  const mine =
    kiosk === undefined ||
    booking.kioskId === kiosk ||
    (!!scope.gateId && booking.gateId === scope.gateId);
  if (!mine) throw ApiError.notFound('Booking not found.');

  if (!booking.session.startedAt)
    throw ApiError.unprocessable('That session has not started, so it has no clock to move.');
