...
export async function requestRefund(
  ...
  if (kiosk !== undefined && booking.kioskId !== kiosk)
  ...
  if (pending)
    throw ApiError.unprocessable(`${pending.ref} is already waiting for approval on this booking.`);
...
export async function reviewRefundRequest(
  ...
  if (!canApproveRefund(scope.role))
    throw ApiError.forbidden(
      'Only a manager, the project manager or the CEO may release a refund.'
    );
  ...
  if (!canWorkEngine(scope, request.engineKind))
    throw ApiError.notFound('Refund request not found.');
  if (request.requestedBy === scope.agentId)
    throw ApiError.forbidden('Someone else has to release a refund you asked for.');
  if (!decision.approve && !decision.note?.trim())
    throw ApiError.badRequest('Say why it is being refused.');
