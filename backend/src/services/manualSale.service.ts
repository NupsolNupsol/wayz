...
export async function listManualSales(scope: ManagerScope, filter: ManualSaleFilter = {}) {
  ...
  return {
    ...
    rows: rows.map((row) => ({
      ...row,
      stationName: stationName.get(row.stationId) ?? row.stationId,
    })),
    pendingTotal: round2(
...
export async function recordManualSale(scope: ManagerScope, input: ManualSaleInput) {
  if (!ENGINE_KINDS.includes(input.engineKind))
    throw ApiError.badRequest(`Unknown activity "${input.engineKind}".`);
  if (!PAYMENT_METHODS.includes(input.method))
    throw ApiError.badRequest(`Unknown payment method "${input.method}".`);
  if (!Number.isFinite(input.amount) || input.amount <= 0)
    throw ApiError.badRequest('Enter an amount greater than zero.');

  const occurredAt = new Date(input.occurredAt);
  if (Number.isNaN(occurredAt.getTime()))
    throw ApiError.badRequest('Give the date the money was actually taken.');
  if (occurredAt.getTime() > Date.now())
    throw ApiError.badRequest('A manual entry records revenue already taken, not future revenue.');
...
export async function reviewManualSale(
  ...
  if (sale.status !== 'PENDING')
    throw ApiError.badRequest(`${sale.ref} has already been ${sale.status.toLowerCase()}.`);
