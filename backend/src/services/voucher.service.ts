...
export async function createCampaign(scope: ManagerScope, input: CampaignInput) {
  ...
  if (expiresAt && Number.isNaN(expiresAt.getTime()))
    throw ApiError.badRequest('That expiry date is not a date.');
  if (expiresAt && expiresAt.getTime() < Date.now())
    throw ApiError.badRequest('That expiry date is already past.');
...
export async function listCampaigns(scope: ManagerScope) {
  const campaigns = await VoucherCampaign.find({ tenantId: scope.tenantId })
    .sort({ createdAt: -1 })
    .lean();
...
export async function campaignCodes(scope: ManagerScope, campaignId: string) {
  ...
  const vouchers = await Voucher.find({ tenantId: scope.tenantId, campaignId })
    .sort({ code: 1 })
    .lean();
...
export async function redeemVoucher(scope: Scope, bookingId: string, code: string) {
  ...
  const { voucher, campaign } = await usableVoucher(
    scope.tenantId,
    code,
    booking.engineKind as EngineKind
  );
