...
export async function activityBreakdown(scope: AccountingScope, filter: PeriodFilter) {
  ...
  const activities: ActivityFigures[] = lines
    .filter((k) => byActivity.has(k) || !filter.engineKind || filter.engineKind === k)
    .map((engineKind) => {
...
export async function zakatReturn(
...
export async function ledger(scope: AccountingScope, filter: PeriodFilter): Promise<LedgerRow[]> {
  ...
  const [payments, movements, expenses] = await Promise.all([
    ...
    filter.engineKind
      ? []
      : CashMovement.find(movementQuery).sort({ createdAt: 1 }).limit(5000).lean(),
  ...
  for (const p of payments) {
    ...
    rows.push({
      date: p.createdAt,
      processType:
        p.kind === 'REFUND' ? 'مرتجع' : p.method === 'CASH' ? 'مبيعات نقدية' : 'تسوية نقاط البيع',
...
export async function ledgerCsv(scope: AccountingScope, filter: PeriodFilter): Promise<string> {
  ...
  lines.push(
    [
      '',
      'اجمالي المبيعات بدون ضريبة',
      '',
      '',
      '',
      summary.salesBase.toFixed(2),
      summary.salesVat.toFixed(2),
      '',
    ]
      .map(csvCell)
      .join(',')
  ...
  lines.push(
    [
      '',
      'اجمالي المرتجعات بدون ضريبة',
      '',
      '',
      '',
      summary.returnsBase.toFixed(2),
      summary.returnsVat.toFixed(2),
      '',
    ]
      .map(csvCell)
      .join(',')
  ...
  lines.push(
    [
      '',
      'اجمالي المشتريات والمصروفات بدون ضريبة',
      '',
      '',
      '',
      summary.purchasesBase.toFixed(2),
      summary.purchasesVat.toFixed(2),
      '',
    ]
      .map(csvCell)
      .join(',')
  ...
  lines.push(
    [
      '',
      'صافي الوعاء الضريبي / Net taxable base',
      '',
      '',
      '',
      summary.netTaxableBase.toFixed(2),
      summary.dueVat.toFixed(2),
      '',
    ]
      .map(csvCell)
      .join(',')
