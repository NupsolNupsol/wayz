...
const PAYROLL_ROLES: Role[] = ['AGENT', 'DELIVERY_AGENT', 'CHIEF_CAPTAIN', 'SUPERVISOR', 'MANAGER'];
...
export async function recordExpense(scope: HrScope, input: ExpenseInput) {
  if (!EXPENSE_CATEGORIES.includes(input.category))
    throw ApiError.badRequest(`Unknown category "${input.category}".`);
  ...
  const tax = NO_VAT_CATEGORIES.includes(input.category)
    ...
    : input.vatInclusive === false
      ? {
          baseAmount: amount,
          vatAmount: round2(amount * rate),
          totalAmount: round2(amount * (1 + rate)),
          vatRate: rate,
        }
...
export async function seasonDetail(scope: HrScope, seasonId: string) {
  ...
  const [expenses, staff] = await Promise.all([
    Expense.find({ tenantId: scope.tenantId, seasonId: season._id })
      .sort({ incurredAt: -1 })
      .lean(),
    User.find({ tenantId: scope.tenantId }).lean(),
  ]);
...
export async function chargeSeasonPayroll(scope: HrScope, input: PayrollInput) {
  ...
  if (!chargeable.length)
    throw ApiError.unprocessable('There are no employees to charge for this season.');
...
export async function setShiftWindow(scope: HrScope, input: { startsAt: string; endsAt: string }) {
  ...
  if (input.startsAt === input.endsAt)
    throw ApiError.badRequest('A shift that starts and ends at the same minute is not a shift.');
...
export async function hoursWorked(scope: HrScope, range: { from?: string; to?: string } = {}) {
  ...
  const [shifts, staff, rules] = await Promise.all([
    Shift.find({ tenantId: scope.tenantId, openedAt: { $gte: from, $lte: to } })
      .sort({ openedAt: -1 })
      .lean(),
    User.find({ tenantId: scope.tenantId }).lean(),
  ...
  const byAgent = new Map<
    string,
    { minutes: number; shifts: number; open: number; lastSeen: Date | null }
  >();

  for (const shift of shifts) {
    const closedAt = shift.closedAt ? new Date(shift.closedAt) : null;
    const minutes = closedAt
      ? Math.max(0, Math.round((closedAt.getTime() - new Date(shift.openedAt).getTime()) / 60_000))
      : 0;
...
export async function peopleAudit(
  scope: HrScope,
  filter: { action?: string; agentId?: string; limit?: number } = {}
) {
