...
export async function updateCommissionRates(scope: AccountingScope, input: RateInput) {
  ...
  for (const [scheme, rate] of entries) {
    if (!CARD_SCHEMES.includes(scheme as CardScheme))
      throw ApiError.badRequest(`Unknown card scheme "${scheme}".`);
    if (typeof rate !== 'number' || Number.isNaN(rate))
      throw ApiError.badRequest(`Give ${scheme} a numeric rate.`);
    if (rate < 0) throw ApiError.badRequest('A commission rate cannot be negative.');
    if (rate > MAX_COMMISSION_RATE) {
      throw ApiError.badRequest(
        `A commission rate above ${(MAX_COMMISSION_RATE * 100).toFixed(0)}% is not a rate, it is a mistake.`
      );
    }
  }
...
export async function ingestTransactions(
  ...
  if (!Array.isArray(rows) || rows.length === 0)
    throw ApiError.badRequest('Send at least one transaction.');
  if (rows.length > 5000)
    throw ApiError.badRequest('Import at most 5000 transactions in one batch.');
  ...
  const existing = new Set(
    (await CardTransaction.find({ tenantId: scope.tenantId }, { externalRef: 1 }).lean()).map(
      (t) => t.externalRef
    )
  );
  ...
  for (const row of rows) {
    ...
    if (!externalRef) {
      result.rejected.push({
        externalRef: '',
        reason: 'Every transaction needs a reference from the terminal.',
      });
      continue;
    }
    ...
    if (!live.has(stale.reference) && stale.status === 'RECORDED') {
      await Expense.updateOne(
        { _id: stale._id },
        {
          $set: {
            status: 'VOID',
            voidReason: 'No card transactions remain for this day and scheme.',
          },
        }
      );
      written += 1;
...
async function allTransactions(scope: AccountingScope, filter: TransactionFilter) {
  return CardTransaction.find(queryFor(scope, filter))
    .sort({ capturedAt: -1 })
...
export async function reconcile(scope: AccountingScope, filter: TransactionFilter = {}) {
  ...
  const byPaymentId = new Map(
    live.filter((t) => t.paymentId).map((t) => [t.paymentId as string, t])
  );
  ...
  const count = (status: ReconciliationRow['status']) =>
...
export async function transactionDetail(scope: AccountingScope, id: string) {
  ...
  const amountAgrees =
    !payment || Math.abs(round2(transaction.grossAmount - payment.amount)) < 0.01;
  ...
  return {
    ...
    payment: payment
      ...
      : null,
    booking: booking
      ? { _id: booking._id, ref: booking.ref, customerName: booking.customerName }
      : null,
    reconciliation: {
...
export async function paymentLedger(scope: AccountingScope, filter: PaymentLedgerFilter = {}) {
  ...
  const [bookings, takers, matched] = await Promise.all([
    Booking.find({
      _id: { $in: payments.map((p) => p.bookingId).filter(Boolean) as string[] },
    }).lean(),
    User.find({
      _id: { $in: [...new Set(payments.map((p) => p.takenBy).filter(Boolean))] },
    }).lean(),
...
export async function paymentDetail(scope: AccountingScope, id: string) {
  ...
  const [booking, taker, transaction] = await Promise.all([
    payment.bookingId
      ? Booking.findOne({ _id: payment.bookingId, tenantId: scope.tenantId }).lean()
      : null,
  ...
  const schemeAgrees =
    !payment.cardScheme || !transaction || payment.cardScheme === transaction.scheme;
  const amountAgrees =
    !transaction || Math.abs(round2(transaction.grossAmount - payment.amount)) < 0.01;
