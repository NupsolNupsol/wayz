...
async function assertOwnsType(scope: AssetScope, assetTypeId: string) {
  const type = await AssetType.findOne(
    { _id: assetTypeId, tenantId: scope.tenantId },
    { engineKind: 1 }
  ).lean();
...
export const SETTABLE_STATUSES: AssetUnitStatus[] = [
  'AVAILABLE',
  'OUT_OF_SERVICE',
  'MAINTENANCE',
  'BLOCKED',
...
function bucket(statuses: Record<string, number>) {
  ...
  const inUse =
    ...
    (statuses.RESERVED ?? 0) +
    (statuses.HELD ?? 0) +
    (statuses.RETRIEVAL_PENDING ?? 0);
  const outOfService =
    (statuses.MAINTENANCE ?? 0) +
    (statuses.OUT_OF_SERVICE ?? 0) +
    (statuses.BLOCKED ?? 0) +
    (statuses.INSPECTION_REQUIRED ?? 0);
  return {
    total,
    inUse,
...
export async function listAssetTypes(scope: AssetScope, engineKind?: EngineKind) {
  ...
  const [types, unitAgg, products, unitStations] = await Promise.all([
    ...
    AssetUnit.aggregate([
      ...
      {
        $group: {
          _id: { assetTypeId: '$assetTypeId', stationId: '$stationId' },
          count: { $sum: 1 },
        },
      },
    ]),
  ...
  for (const product of [...products].sort(
    (a, b) => a.basePrice - b.basePrice || a._id.localeCompare(b._id)
  )) {
  ...
  return {
    ...
    assetTypes: types.map((type) => {
      ...
      return {
        ...
        stationNames: [...new Set(stationsByType.get(type._id) ?? [])].map(
          (id) => stationName.get(id) ?? id
        ),
        byStatus: statuses,
        ...bucket(statuses),
      };
...
export async function assetTypeDetail(scope: AssetScope, assetTypeId: string) {
  ...
  const [units, product, stations, kiosks, gates] = await Promise.all([
    AssetUnit.find({ tenantId: scope.tenantId, assetTypeId })
      .sort({ identifier: 1 })
      .limit(2000)
      .lean(),
...
export async function assetUnitDetail(scope: AssetScope, unitId: string) {
  ...
  const [type, product, station, kiosk, gate, booking] = await Promise.all([
    ...
    unit.currentBookingId
      ? Booking.findOne({ _id: unit.currentBookingId, tenantId: scope.tenantId }).lean()
      : null,
  ]);
...
export async function createAssetKind(scope: AssetScope, input: NewAssetKind) {
  ...
  const billing =
    input.billingModel ?? billingForSaleUnit(saleUnit, KIND_DEFAULTS[input.kind].billingModel);
  ...
  if (!saleUnitMatchesBilling(saleUnit, billing)) {
    throw ApiError.unprocessable(
      `Something sold by ${saleUnit.toLowerCase().replace(/_/g, ' ')} cannot be charged that way.`,
      [
  ...
  await AssetType.create({
    ...
    capacity: {
      ...capacity,
      capacityScore:
        capacity.capacityScore ?? capacity.maxRecommendedBagCount ?? capacity.seats ?? 1,
    },
  });
...
export async function updateAssetKind(
  ...
  if (input.name !== undefined && input.name.trim() && input.name.trim() !== type.name) {
    const name = input.name.trim();
    const clash = await AssetType.findOne({
      tenantId: scope.tenantId,
      name,
      _id: { $ne: type._id },
    }).lean();
...
export async function removeAssetKind(scope: AssetScope, assetTypeId: string) {
  ...
  if (units > 0) {
    throw ApiError.unprocessable(
      `${type.name} still has ${units} asset(s). Remove them before deleting the kind.`
    );
  }

  await Promise.all([
    AssetType.deleteOne({ _id: assetTypeId, tenantId: scope.tenantId }),
    CatalogueProduct.updateMany(
      { tenantId: scope.tenantId, assetTypeId },
      { $set: { active: false, assetTypeId: null } }
    ),
  ]);
...
export async function addUnits(
  ...
  if (input.count < 1 || input.count > 200)
    throw ApiError.badRequest('Add between 1 and 200 assets at a time.');
  ...
  if (livesAtAGate(type.kind)) {
    ...
    if (input.kioskId && !kiosk) {
      ...
      if (anywhere.stationId !== input.stationId)
        throw ApiError.badRequest('That desk does not belong to the chosen station.');
  ...
  const existing = await AssetUnit.find(
    { tenantId: scope.tenantId, assetTypeId },
    { identifier: 1 }
  ).lean();
  ...
  return {
    ...
    assetTypeId,
    identifiers: units.map((u) => u.identifier as string),
  };
}
...
export async function updateUnit(
  ...
  if (input.status && input.status !== unit.status) {
    ...
    if (input.status !== 'AVAILABLE' && BUSY.includes(unit.status)) {
      throw ApiError.unprocessable(
        `${unit.identifier} is in use — complete or reassign its booking first.`
      );
    }
  ...
  if (
    ...
    input.identifier.trim() !== unit.identifier
  ) {
    ...
    if (clash)
      throw ApiError.badRequest(`${input.identifier.trim()} is already used by another asset.`);
  ...
  if (input.stationId !== undefined || input.kioskId !== undefined) {
    if (BUSY.includes(unit.status)) {
      throw ApiError.unprocessable(
        `${unit.identifier} is in use — complete its booking before moving it.`
      );
    }
    ...
    const type = await AssetType.findOne(
      { _id: unit.assetTypeId, tenantId: scope.tenantId },
      { engineKind: 1 }
    ).lean();
    /*
    ...
    if (type?.engineKind && !station.engineKinds.includes(type.engineKind)) {
      throw ApiError.badRequest(
        `${station.name} does not run ${(type.engineKind ?? 'that activity').replaceAll('_', ' ').toLowerCase()}.`
      );
    }
...
export async function updateTypePrice(
  ...
  if (!product)
    throw ApiError.unprocessable(
      `${type.name} has no product to price. Create one under Pricing first.`
    );
  ...
  if (input.saleUnit !== undefined) {
    ...
    if (product.billingModel === 'DURATION_BASED')
      product.durationUnit = durationUnitFor(input.saleUnit) ?? 'HOUR';
  }
  ...
  if (input.overtimeHourlyRate !== undefined) {
    ...
    if (next !== null && next < 0)
      throw ApiError.badRequest('An overtime rate cannot be negative.');
