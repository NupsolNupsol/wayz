...
import { isWhatsAppConfigured } from './vonage.service.js';
...
function assertOpenForVerification(booking: BookingHydrated, purpose: VerificationPurpose) {
  if (
    ['RETRIEVAL', 'DELIVERY_REQUEST'].includes(purpose) &&
...
const maskDestination = (channel: OtpChannel, value: string) =>
  channel === 'EMAIL' ? maskEmail(value) : maskPhone(value);
  ...
  return [
    {
      ...
      hasDestination: !!booking.customerPhone,
    },
    {
      channel: 'EMAIL' as const,
      configured: isEmailConfigured(),
      hasDestination: !!booking.customerEmail,
    },
  ];
}
...
async function storeEvidence(
  ...
  if (sizeBytes > MAX_EVIDENCE_BYTES) {
    throw ApiError.badRequest(
      `The document image is too large (max ${MAX_EVIDENCE_BYTES / 1024 / 1024} MB).`
    );
  }
...
export async function confirmVerification(
  ...
  switch (input.method) {
    ...
    case 'EMAIL_OTP': {
      ...
      if (!ok)
        throw ApiError.unprocessable('Incorrect or expired code — the customer is not verified.');
      verification = {
        ...base,
        method: input.method,
        channel,
    ...
    case 'ID_DOCUMENT': {
      if (!input.reason?.trim())
        throw ApiError.unprocessable('State why the WhatsApp code could not be used.');
      const evidenceId = input.document.image
        ? await storeEvidence(scope, booking, input.purpose, input.document.image)
        : null;
      verification = {
        ...base,
        method: 'ID_DOCUMENT',
    ...
    case 'MANAGER_OVERRIDE': {
      ...
      const supervisor = await authenticateOverride(
        scope.tenantId,
        input.authoriserEmail,
        input.authoriserPassword,
        OVERRIDE_ROLES
      );
      const evidenceId = input.document?.image
        ? await storeEvidence(scope, booking, input.purpose, input.document.image)
