...
export async function sweepRestPeriods(now: Date = new Date()): Promise<number> {
  ...
  let fromNotes = 0;
  const older = await AssetUnit.find(
    { status: 'RESTING', restingUntil: null, note: NOTE_TIME },
    { note: 1 }
  ).lean();
  for (const unit of older) {
    ...
    if (until <= now) {
      await AssetUnit.updateOne(
        { _id: unit._id, status: 'RESTING' },
        { $set: { status: 'AVAILABLE', note: '' } }
      );
      fromNotes += 1;
    } else {
  ...
  if (total > 0)
    logger.info('Animals back from rest', { organisation: currentOrganisation(), count: total });
  return total;
}
