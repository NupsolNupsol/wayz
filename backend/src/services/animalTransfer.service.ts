...
export async function raiseTransfer(actor: TransferActor, input: RaiseTransferInput) {
  ...
  if (origin.siteId === destination.siteId) {
    throw ApiError.badRequest(
      'That is the same location — a transfer moves an animal between locations.'
    );
  }
  ...
  if (inFlight) {
    throw ApiError.conflict(
      `${animal.identifier} already has a transfer in progress (${inFlight.ref}).`
    );
  }
...
export async function approveTransfer(actor: TransferActor, id: string, note = '') {
  ...
  if (transfer.status !== 'REQUESTED') {
    throw ApiError.unprocessable(
      `${transfer.ref} is ${transfer.status.toLowerCase()} and cannot be approved.`
    );
  }
...
export async function rejectTransfer(actor: TransferActor, id: string, reason: string) {
  ...
  if (transfer.status !== 'REQUESTED') {
    throw ApiError.unprocessable(
      `${transfer.ref} is ${transfer.status.toLowerCase()} and cannot be rejected.`
    );
  }
  if (!may(rules.approvers, actor.role)) {
...
export async function departTransfer(actor: TransferActor, id: string) {
  ...
  if (!named && !may(rules.approvers, actor.role)) {
    throw ApiError.forbidden(
      `${transfer.ref} is assigned to ${transfer.driverName || 'another driver'}.`
    );
  }
  ...
  if (animal.currentBookingId) {
    throw ApiError.unprocessable(
      `${animal.identifier} is in a session — it cannot leave until that finishes.`
    );
  }
...
export async function receiveTransfer(
  actor: TransferActor,
  id: string,
  arriveAs: 'AVAILABLE' | 'RESTING' = 'RESTING'
) {
  const transfer = await AnimalTransfer.findById(id);
  if (!transfer) throw ApiError.notFound('No such transfer order.');
  if (transfer.status !== 'IN_TRANSIT') {
    throw ApiError.unprocessable(
      `${transfer.ref} is ${transfer.status.toLowerCase()} — nothing to receive.`
    );
  }
  ...
  if (!receiver) {
    throw ApiError.forbidden(
      'Receipt is confirmed by the driver who carried it or by a supervisor at the destination.'
    );
  }
  ...
  animal.restingUntil =
    arriveAs === 'RESTING'
      ? new Date(Date.now() + restMinutesFor(animal.assetTypeId) * 60_000)
      : null;
...
export async function cancelTransfer(actor: TransferActor, id: string, reason: string) {
  ...
  if (transfer.requestedBy !== actor.id && !may(rules.approvers, actor.role)) {
    throw ApiError.forbidden(
      'Only the person who raised a transfer, or somebody who may approve it, can withdraw it.'
    );
  }
