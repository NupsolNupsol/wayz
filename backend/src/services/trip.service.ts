...
export async function boatsWithRoom(scope: Scope, assetTypeId?: string): Promise<BoatSpace[]> {
  if (!canWorkEngine(scope, 'LAGOON'))
    throw ApiError.forbidden('You are not assigned to the lagoon.');
...
export async function seatOnBoat(
  scope: Scope,
  booking: {
    _id: string;
    ref: string;
...
export async function releaseTrip(scope: Scope, tripId: string) {
  ...
  if (trip.status !== 'FILLING')
    throw ApiError.unprocessable(`${trip.ref} is already ${trip.status.toLowerCase()}.`);
...
export async function tripBoard(scope: Scope, mine = false) {
  ...
  const [ready, running, done, filling] = await Promise.all([
    ...
    Trip.find({
      ...
      .sort({ createdAt: 1 })
      .lean(),
    Trip.find({ ...base, status: { $in: ['COMPLETED', 'CANCELLED'] } })
      .sort({ endedAt: -1 })
      .limit(25)
      .lean(),
    Trip.find({ ...base, status: 'FILLING' })
      .sort({ createdAt: 1 })
      .lean(),
  ]);
...
export async function tripDetail(scope: Scope, tripId: string) {
  ...
  if (!trip) throw ApiError.notFound('Trip not found.');
  const home = await Station.findOne(
    { _id: trip.stationId, tenantId: scope.tenantId },
    { siteId: 1 }
  ).lean();
  const map = await stationMap(scope.tenantId, home?.siteId);
  const stations = map.points.filter(
    (p) => p.active && (p.kind === 'STATION' || p.engineKinds.includes('LAGOON'))
  );
...
export async function setTripRoute(scope: Scope, tripId: string, stationIds: string[]) {
  ...
  if (!trip) throw ApiError.notFound('Trip not found.');
  if (trip.captainId !== scope.agentId)
    throw ApiError.forbidden('That trip belongs to another captain.');
  if (!['CLAIMED', 'RUNNING'].includes(trip.status)) {
    throw ApiError.unprocessable(
      `A route cannot be planned on a trip that is ${trip.status.toLowerCase()}.`
    );
  }
  ...
  if (missing.length)
    throw ApiError.badRequest(`Not a stop in this tenant: ${missing.join(', ')}.`);
...
export async function startTrip(scope: Scope, tripId: string, input: { unitId?: string } = {}) {
  ...
  if (trip.status !== 'CLAIMED')
    throw ApiError.unprocessable(`${trip.ref} cannot be started from ${trip.status}.`);
  ...
  for (const passenger of trip.passengers) {
    ...
    const booking = await Booking.findOne(
      { _id: passenger.bookingId, tenantId: scope.tenantId },
      { status: 1 }
    ).lean();
    if (!booking) continue;
...
export async function clockStation(scope: Scope, tripId: string, stationId: string) {
  ...
  if (trip.status !== 'RUNNING')
    throw ApiError.unprocessable('Only a trip that is under way can reach a station.');
  ...
  if (last?.stationId === stationId)
    throw ApiError.unprocessable(`${name} is already the last stop clocked.`);
...
export async function completeTrip(scope: Scope, tripId: string) {
  ...
  if (trip.captainId !== scope.agentId)
    throw ApiError.forbidden('That trip belongs to another captain.');
  if (trip.status !== 'RUNNING')
    throw ApiError.unprocessable('Only a trip under way can be finished.');
  ...
  if (trip.assetUnitId) {
    await AssetUnit.updateOne(
      { _id: trip.assetUnitId, tenantId: scope.tenantId },
