...
import {
  ...
  requestRefund,
} from '../services/refundRequest.service.js';
...
export const bookingController = {
  ...
  list: asyncHandler(async (req, res) => {
    const s = scopeFromReq(req);
    const list = await listBookings(s, {
      status: req.query.status as string | undefined,
      engineKind: req.query.engineKind as never,
    });
  ...
  reassign: asyncHandler(async (req, res) => {
    ...
    res.json({
      success: true,
      data: bookingDTO(await reassignBooking(s, req.params.id, body.unitId, body.reason)),
    });
  }),
  ...
  returnHere: asyncHandler(async (req, res) => {
    ...
    const { booking, wrongStation } = await returnAtStation(
      s,
      req.params.id,
      body.code,
      body.payload ?? {}
  ...
  transition: asyncHandler(async (req, res) => {
    ...
    res.json({
      success: true,
      data: bookingDTO(await transitionBooking(s, req.params.id, body.code, body.payload ?? {})),
    });
  }),
  ...
  sendVerification: asyncHandler(async (req, res) => {
    ...
    res.json({
      success: true,
      data: await sendVerificationChallenge(s, req.params.id, purpose, channel),
    });
  }),
  ...
  confirmVerification: asyncHandler(async (req, res) => {
    ...
    res.json({
      success: true,
      data: bookingDTO(await confirmVerification(s, req.params.id, body)),
    });
  }),
