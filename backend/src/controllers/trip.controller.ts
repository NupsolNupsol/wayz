...
export const tripController = {
  boats: asyncHandler(async (req, res) => {
    const assetTypeId =
      typeof req.query.assetTypeId === 'string' ? req.query.assetTypeId : undefined;
  ...
  route: asyncHandler(async (req, res) => {
    ...
    res.json({
      success: true,
      data: await setTripRoute(scopeFromReq(req), req.params.id, body.stationIds),
    });
  }),
  ...
  stop: asyncHandler(async (req, res) => {
    ...
    res.json({
      success: true,
      data: await clockStation(scopeFromReq(req), req.params.id, body.stationId),
    });
  }),
