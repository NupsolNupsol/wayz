...
const receiveSchema = z.object({
  received: z.array(z.object({ index: z.number().int().min(0), quantity: z.number().min(0) })).min(1),
})
...
export const procurementController = {
  ...
  receive: asyncHandler(async (req, res) => {
    ...
    res.json({ success: true, data: await receiveOrder(await actorOf(req), req.params.id, received) })
  }),
