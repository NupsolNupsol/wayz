...
const capacitySchema = z.object({
  internalDimensions: z
    .object({ w: z.number().min(1), h: z.number().min(1), d: z.number().min(1) })
    .optional(),
...
const kindPatchSchema = z
  ...
  .refine((v) => v.name !== undefined || v.capacity !== undefined, {
...
export const assetController = {
  types: asyncHandler(async (req, res) => {
    const engineKind = req.query.engineKind
      ? z.enum(ENGINE_KINDS).parse(req.query.engineKind)
      : undefined;
  ...
  createType: asyncHandler(async (req, res) => {
    ...
    const data = await createAssetKind(assetScope(req), {
      ...body,
  ...
  addUnits: asyncHandler(async (req, res) => {
    ...
    const data = await addUnits(assetScope(req), req.params.id, {
      ...body,
      kioskId: body.kioskId ?? null,
      gateId: body.gateId ?? null,
    });
  ...
  updateUnit: asyncHandler(async (req, res) => {
    ...
    const data = await updateUnit(
      assetScope(req),
      req.params.id,
      body as Parameters<typeof updateUnit>[2]
    );
