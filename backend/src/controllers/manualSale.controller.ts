...
import {
  ...
  recordManualSale,
  reviewManualSale,
} from '../services/manualSale.service.js';
...
export const manualSaleController = {
  list: asyncHandler(async (req, res) => {
    res.json({
      success: true,
      data: await listManualSales(scope(req), listQuery.parse(req.query)),
    });
  }),
  ...
  create: asyncHandler(async (req, res) => {
    res.status(201).json({
      success: true,
      data: await recordManualSale(scope(req), createSchema.parse(req.body)),
    });
  }),
