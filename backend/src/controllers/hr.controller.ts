...
export const hrController = {
  ...
  overview: asyncHandler(async (req, res) => {
    res.json({
      success: true,
      data: await hrOverview(hrScope(req), filterSchema.parse(req.query)),
    });
  }),
  ...
  expenses: asyncHandler(async (req, res) => {
    res.json({
      success: true,
      data: await listExpenses(hrScope(req), filterSchema.parse(req.query)),
    });
  }),
  ...
  createSeason: asyncHandler(async (req, res) => {
    res.status(201).json({
      success: true,
      data: await createSeason(hrScope(req), seasonSchema.parse(req.body)),
    });
  }),
