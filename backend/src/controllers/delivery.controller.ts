...
import {
  ...
  availableDeliveryTransitions,
  collectStop,
  courierBoard,
  customerBagsElsewhere,
  courierScopeFrom,
...
async function courierActor(req: Parameters<typeof scopeFromReq>[0]): Promise<DeliveryActor> {
  ...
  const s = await courierScopeFrom(
    req.auth.tenantId,
    req.auth.stationId,
    req.auth.sub,
    req.auth.role
...
export const deliveryController = {
  ...
  customerBags: asyncHandler(async (req, res) => {
    res.json({
      success: true,
      data: await customerBagsElsewhere(scopeFromReq(req), req.params.bookingId),
    });
  }),

  collectStop: asyncHandler(async (req, res) => {
    const body = z
      .object({ scannedBarcodes: z.array(z.string()).default([]) })
      .parse(req.body ?? {});
  ...
  detail: asyncHandler(async (req, res) => {
    ...
    res.json({
      ...
      data: {
        ...
        transitions: claimedByAnother
          ? []
          : availableDeliveryTransitions(data.delivery.status, req.auth.role),
      },
    });
  }),
};
