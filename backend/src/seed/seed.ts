...
function demoAccount(password: string): { passwordHash: string; demoCredential: string | null } {
  return {
    ...
    demoCredential: env.DEMO_LOGINS ? password : null,
  };
}

const HOUR = 3_600_000;
const MIN = 60_000;
const ALL_ENGINES: EngineKind[] = ['SHOP_AND_DROP', 'MOBILITY', 'LAGOON', 'COTE_RESTAURANT'];
...
function assetTypes(t: string) {
  ...
  const vehicle = (
    ...
    name: string,
    seats: number,
    engineKind: EngineKind = 'MOBILITY'
  ) => ({
    _id: `at_${t}_veh_${key}`,
    tenantId: t,
    engineKind,
...
const allGates = () => [
  ...
  gateDoc(
    ...
    'Boardwalk Gate',
    'BRD-GT-1',
    'Boardwalk entrance, locker hall'
  ),
];
    ...
    custody: [
      {
        from: 'CUSTOMER',
        to: 'AGENT',
        at: new Date(Date.now() - 45 * MIN),
        note: 'Bags received at desk',
      },
      {
        from: 'AGENT',
        to: 'LOCKER',
        at: new Date(Date.now() - 40 * MIN),
 ...
 */
function boulevardPlaceFor(
  assetTypeId: string,
  index: number
): { kioskId: string | null; gateId: string | null } {
...
const shopAndDropKiosks = () =>
  SHOP_AND_DROP_DESKS.map((d) => boulevardKiosk('SHOP_AND_DROP', 'SD', d));
...
type DemoUser = Omit<
  UserDoc,
  'comparePassword' | 'invite' | 'engineKinds' | 'gateId' | 'activityKeys' | 'roleKey'
> & {
...
export async function seedFresh() {
  ...
  await AssetUnit.insertMany(
    onlySeeded([
      ...
      ...assetUnits('wiqar', 'stn_wiqar_1').map((u) => {
        ...
        return {
          ...u,
          kioskId: type.includes('boat') ? 'ksk_wiqar_lagoon' : 'ksk_wiqar_1',
          gateId: null,
        };
      }),
    ...
    custody: [
      {
        from: 'CUSTOMER',
        to: 'AGENT',
        at: new Date(Date.now() - 55 * MIN),
        note: 'Bags received at desk',
      },
      {
        from: 'AGENT',
        to: 'LOCKER',
        at: new Date(Date.now() - 50 * MIN),
        note: 'Stored — timer started',
      },
    ],
  ...
  await AssetUnit.updateOne(
    { _id: demoMedium._id },
    { $set: { status: 'OCCUPIED', currentBookingId: 'bk-0002' } }
  );
  ...
  await DeliveryRequest.create({
    ...
    timeline: [
      {
        status: 'REQUESTED',
        at: new Date(Date.now() - 12 * MIN),
        by: 'usr_agent_wayz',
        note: 'Requested (at the desk)',
      },
    ],
  });
  ...
  await Booking.create({
    ...
    custody: [
      ...
      },
      {
        from: 'LOCKER',
        to: 'PORTER',
        at: new Date(Date.now() - 4 * HOUR),
        note: 'Collected by courier for dlv-0002',
      },
      {
        from: 'PORTER',
        to: 'CUSTOMER',
        at: new Date(Date.now() - 3 * HOUR),
        note: 'Delivered to Marriott Riyadh, room 812',
      },
    ],
  ...
  await DeliveryRequest.create({
    ...
    customerPhone: '0512223344',
    destination: {
      address: 'Marriott Riyadh, room 812',
      notes: 'Leave with the concierge.',
      contactPhone: '0512223344',
    },
    status: 'DELIVERED',
    ...
    timeline: [
      {
        status: 'REQUESTED',
        at: new Date(Date.now() - 4.5 * HOUR),
        by: 'usr_agent_wayz',
        note: 'Requested (by phone)',
      },
      {
        status: 'ASSIGNED',
        at: new Date(Date.now() - 4.4 * HOUR),
        by: 'usr_courier_wayz',
        note: 'Claimed by courier',
      },
      {
        status: 'RELEASE_REQUESTED',
        at: new Date(Date.now() - 4.1 * HOUR),
        by: 'usr_courier_wayz',
        note: 'Courier at the kiosk requesting the bags',
      },
      {
        status: 'RELEASE_APPROVED',
        at: new Date(Date.now() - 4 * HOUR),
        by: 'usr_agent_wayz',
        note: 'Kiosk agent confirmed the courier and released the compartment',
      },
      {
        status: 'PICKED_UP',
        at: new Date(Date.now() - 4 * HOUR),
        by: 'usr_courier_wayz',
        note: '1 bag(s) collected',
      },
      {
        status: 'DELIVERED',
        at: new Date(Date.now() - 3 * HOUR),
  ...
  for (const q of [
    {
      ...
      order: 'ord-0003',
      ref: 'SD-100013',
      customer: 'cust_wayz_3',
      name: 'Nora Hassan',
      phone: '0512223344',
      price: 45,
      product: 'pr_wayz_sd_m',
      label: 'Shop & Drop — M (2 bags)',
      bags: 2,
      minsAgo: 9,
    },
    {
      ...
      order: 'ord-0004',
      ref: 'SD-100014',
      customer: 'cust_wayz_1',
      name: 'Ahmed Saleh',
      phone: '0599709998',
      price: 30,
      product: 'pr_wayz_sd_s',
      label: 'Shop & Drop — S (1 bag)',
      bags: 1,
      minsAgo: 3,
    },
  ]) {
...
async function seedManualSales() {
  const gross = (total: number) => {
    ...
    return {
      amount: total,
      baseAmount: base,
      vatAmount: Math.round((total - base) * 100) / 100,
      vatRate: 0.15,
    };
  };

  await ManualSale.insertMany(
    onlySeeded([
      {
        _id: 'man-0001',
        ref: 'MAN-0001',
  ...
  for (let dayBack = 89; dayBack >= 0; dayBack--) {
    ...
    for (const a of activities) {
      ...
      for (let i = 0; i < count; i++) {
        const at = new Date(
          Math.min(day.getTime() + i * 7 * MIN, HISTORY_ENDS_AT - (count - i) * MIN)
        );
    ...
    if (dayBack % 3 === 0) {
      ...
      movements.push({
        ...
        vatAmount: etax.vat,
        vatRate: VAT_RATE,
        reason: ['Operating supplies', 'Equipment maintenance', 'POS consumables', 'Bank charges'][
          movSeq % 4
        ],
...
async function seedAuditTrail(
  ...
  for (const p of payments) {
    ...
    if (p.kind === 'REFUND') {
      add(
        ...
        'Payment',
        id,
        `${amount} SAR returned by ${method.toLowerCase()}`,
      ...
      add(
        ...
        'Payment',
        id,
        `${amount} SAR taken by ${method.toLowerCase()}`
      );
    }
  }
  ...
  for (const m of movements) {
    add(
      ...
      'CashMovement',
      String(m._id),
      `${Number(m.amount).toFixed(2)} SAR — ${String(m.reason)}`
    );
  }
  ...
  for (const b of bookings) {
    ...
    if (b.status === 'ACTIVE' || b.status === 'OVERTIME') {
      add(
        new Date(new Date(b.createdAt as Date).getTime() + 4 * MIN),
        b.agentId ?? 'usr_agent_wayz',
        'BOOKING_STORED',
        'Booking',
        b._id,
  ...
  for (const d of deliveries) {
    add(
      d.createdAt as Date,
      d.requestedBy ?? 'usr_agent_wayz',
      'DELIVERY_REQUESTED',
      'Delivery',
      d._id,
      `to ${String(d.destination?.address ?? '').slice(0, 40)}`
    );
  }
  ...
  add(
    ...
    'Tenant',
    'wayz',
    'VAT rate and legal identity confirmed'
  );
  add(
    ...
    'CommissionRate',
    'wayz',
    'Contract rates loaded from the acquiring bank'
  );
  add(
    new Date(Date.now() - 45 * 86400000),
    'usr_hr_wayz',
    'SEASON_PAYROLL_CHARGED',
    'Season',
    'ssn-0001',
    'Riyadh Season 2026 — H1 · 6 months charged'
  );
...
async function seedCardTransactions(payments: Record<string, unknown>[], next: () => number) {
  ...
  const commissionByDay = new Map<
    string,
    { day: string; scheme: CardScheme; amount: number; count: number }
  >();
  ...
  for (let i = 0; i < 4; i++) {
    ...
    record(
      ...
      scheme,
      bin,
      Math.round((60 + next() * 900) * 100) / 100,
      at,
      null,
      'LAGOON',
      'TPE'
  ...
  for (const { scheme, bin } of SCHEME_MIX) {
    ...
    record(
      ...
      scheme,
      bin,
      Math.round((120 + next() * 400) * 100) / 100,
      at,
      null,
      'MOBILITY',
      'TPE'
...
async function seedCostHistory() {
  ...
  const operating: {
    ...
    {
      category: 'RENT_VENUE',
      description: 'Lagoon site rent — quarter',
      supplier: 'Riyadh Season Authority',
      engineKind: 'LAGOON',
      amount: 172500,
      daysAgo: 70,
    },
    {
      category: 'RENT_ACCOMMODATION',
      description: 'Staff accommodation — quarter',
      supplier: 'Al Waha Housing',
      engineKind: null,
      amount: 86250,
      daysAgo: 68,
    },
    {
      category: 'REPAIR',
      description: 'Boat hull repair after collision',
      supplier: 'Marine Works Co.',
      engineKind: 'LAGOON',
      amount: 14375,
      daysAgo: 41,
    },
    {
      category: 'REPAIR',
      description: 'Scooter battery replacement — 6 units',
      supplier: 'Mobility Parts KSA',
      engineKind: 'MOBILITY',
      amount: 9200,
      daysAgo: 33,
    },
    {
      ...
      description: 'Monthly boat servicing',
      supplier: 'Marine Works Co.',
      engineKind: 'LAGOON',
      amount: 6900,
      daysAgo: 27,
    ...
    {
      category: 'FUEL_OIL',
      description: 'Fuel and engine oil',
      supplier: 'Petro Supply',
      engineKind: 'LAGOON',
      amount: 11500,
      daysAgo: 20,
    },
    {
      category: 'MAINTENANCE',
      description: 'Locker lock servicing',
      supplier: 'SecureLock',
      engineKind: 'SHOP_AND_DROP',
      amount: 3450,
      daysAgo: 16,
    ...
    {
      category: 'SUPPLIER',
      description: 'Barcode label rolls and printer ribbon',
      supplier: 'PrintPro',
      engineKind: 'SHOP_AND_DROP',
      amount: 2300,
      daysAgo: 12,
    ...
    {
      category: 'FUEL_OIL',
      description: 'Scooter charging electricity',
      supplier: 'SEC',
      engineKind: 'MOBILITY',
      amount: 4600,
      daysAgo: 9,
    ...
    {
      category: 'ADMIN',
      description: 'Season administrative charges',
      supplier: '',
      engineKind: null,
      amount: 23000,
      daysAgo: 6,
