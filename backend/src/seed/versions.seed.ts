...
const RELEASE_ONE: Draft[] = [
  change(
    ...
    [
      CEO,
      'Open Employees, accounts & roles',
      'Press Add member and read the role list',
      'Add a chief captain to a lagoon kiosk, and an agent to another lagoon kiosk',
    ],
  ...
  change(
    ...
    [
      CEO,
      'Open Estate → Sites & stations',
      'Add a station to a site, then add a Shop & Drop kiosk and a Mobility kiosk under it',
      'Try to deactivate a station that still has live bookings',
    ],
  ...
  change(
    ...
    [
      MANAGER,
      'Open Team and edit an agent',
      'Move them from the Iran (Shop & Drop) desk to Gate 1 (Mobility) and save',
      'Sign in as that agent',
    ],
  ...
  change(
    ...
    [
      AGENT,
      'Open a booking that belongs to your desk',
      'Then edit the address bar to a booking id from another kiosk',
    ],
  ...
  change(
    ...
    [
      MANAGER,
      'Open Team → Add member and create somebody with your own email',
      'Open the invitation link from the email (or the one shown when email is not configured)',
...
const RELEASE_TWO: Draft[] = [
  change(
    ...
    [
      AGENT,
      'Without opening the till, start a Shop & Drop sale and try to take payment',
      'Open the till from the header and count in a float',
      'Take the payment again, then open My till',
    ],
  ...
  change(
    ...
    [
      MANAGER,
      'Open Shifts and find an open till',
      'Force-close it with a counted amount that does not match',
      'Open the shift and record a resolution',
    ],
  ...
  change(
    ...
    [
      AGENT,
      'Open a paid booking and press Refund, enter an amount and a reason',
      MANAGER,
      'Open Refund approvals and release it',
      'Then sign in as the accountant and look for the same queue',
    ],
  ...
  change(
    ...
    [
      ACCOUNTANT,
      'Open Commissions',
      'Read the rates against the contract, then change the Visa rate and save',
    ],
  ...
  change(
    ...
    [
      AGENT,
      'Take a payment for a customer whose WhatsApp you can read',
      'Press Sales invoice on the booking and print it',
    ],
  ...
  change(
    ...
    [
      CEO,
      'Open Operating rules and read the grace period and the hourly rate',
      AGENT,
      'Store a bag, then use the testing clock to push it past the grace period',
    ],
    ...
    [
      ...
      { label: 'Active operations', to: '/operations' },
    ]
  ),
  change(
    'Notifications',
...
const RELEASE_THREE: Draft[] = [
  change(
    ...
    [
      HR,
      'Open Shifts & hours',
      'Change the shift window and save',
      'Read hours worked per person, then open the people log and filter by one person',
    ],
    ...
    [
      { label: 'Shifts & hours', to: '/hr/shifts' },
      { label: 'Costs', to: '/hr/costs' },
    ]
  ),
  change(
    'Money',
    ...
    [
      HR,
      'Open Pricing and give a scooter an hourly price and a tour price',
      'Sign in as agent.gate1.wayz@lockerflow.demo / Agent@123',
      'Start a Mobility sale and switch between By the hour and By the tour',
    ],
  ...
  change(
    ...
    [
      LAGOON_DESK,
      'Open the till and sell three Abra trips (5, 4 and 3 people)',
      'Open Boats & trips and press Group them into boats',
      CAPTAIN,
      'Open My trips, take a waiting trip, and try to find anything that sells',
    ],
    ...
    [
      { label: 'Boats & trips', to: '/lagoon/trips' },
      { label: 'My trips', to: '/lagoon/captain' },
    ]
  ),
  change(
    'Counter',
    ...
    [
      AGENT,
      'Start a Shop & Drop sale, get as far as the payment step, and leave the page',
      'Open Bookings and click that draft row',
    ],
...
const RELEASE_FOUR: Draft[] = [
  change(
    ...
    [
      CEO,
      'Open Estate then Station map',
      'Drag the Egypt jetty somewhere else and press Save the map',
      'Reload the page',
      'Take a marker off the map with the small circular arrow, then drag it back on from the tray',
    ],
  ...
  change(
    ...
    [
      LAGOON_DESK,
      'Open the till and sell two Abra trips (5 people, then 4)',
      'Open Boats and trips, press Group them into boats',
      CAPTAIN,
      'Open My trips, take the waiting trip, and press Plan the road',
      'Tap Egypt, France and Mountain on the chart, press Save the road, then Cast off',
      'Press Reached Egypt, then Reached France, then Reached Mountain',
    ],
    ...
    [
      { label: 'Boats and trips', to: '/lagoon/trips' },
      { label: 'Chart and sail', to: '/lagoon/voyage' },
    ]
  ),
];
...
const RELEASE_FOUR_MORE: Draft[] = [
  change(
    ...
    [
      AGENT,
      'Open booking SD-100011 from Bookings and press Send to customer',
      'Read the list of other kiosks holding this customer bags, tick Bring everything',
      'Choose They called or messaged, verify the customer, set a charge of 40 and create the delivery',
      COURIER,
      'Open the till, pick the task up, then for each kiosk: Request the bags, the desk approves with a code, scan and confirm',
      'At the door, try Mark delivered before paying, then take the payment and mark it delivered',
    ],
    ...
    [
      ...
      { label: 'Courier board', to: '/courier' },
    ]
  ),
  change(
    'Delivery',
    ...
    [
      AGENT,
      'Raise a delivery from any stored booking',
      COURIER,
      'Open the task from the bell without pressing Pick up this task',
      'Look at the top of the page, then press Pick up this task and ask the kiosk for the bags',
      AGENT,
      'Open the bell',
    ],
  ...
  change(
    ...
    [
      AGENT,
      'Store a bag, then press +60 min and move the clock 200 minutes',
      'Try Begin retrieval and read the amber line above the buttons',
      'Verify the customer, take the payment, then begin the retrieval',
    ],
  ...
  change(
    ...
    [
      AGENT,
      'Take an overtime payment on a booking that ran over',
      'Press Sales invoice on the same booking',
    ],
  ...
  change(
    ...
    [
      LAGOON_DESK,
      'Open the till, then start a Lagoon sale and pick the Abra Trip',
      'Read the boats offered and how many seats each has left',
      'Pick one, try to type more visitors than it has seats, then sell a smaller party',
      'Open Boats and trips and watch that boat fill',
      'Sell the remaining seats on the same boat',
      CAPTAIN,
      'Open the trip board',
    ],
    ...
    [
      { label: 'Lagoon', to: '/lagoon' },
      { label: 'Boats and trips', to: '/lagoon/trips' },
      { label: 'Trip board', to: '/lagoon/captain' },
    ]
  ),
  change(
    'Lagoon',
    ...
    [
      CAPTAIN,
      'Look at the menu on the left and at where you landed',
      'Take a trip and read the boat, the seats and the manifest',
    ],
  ...
  change(
    ...
    ['Everyone'],
    [
      'Open /versions with no account at all',
      'Open this release and press Mark as checked on a change you have tested',
      'Press Report an issue on another, type what went wrong, and send it',
    ],
...
const withVerdicts = (changes: Draft[]): VersionChange[] =>
  changes.map((c) => ({ ...c, checks: [], issues: [] }));
...
export async function seedVersions() {
  ...
  await Version.insertMany(
    RELEASES.map((release) => ({
      ...
      changes: withVerdicts(release.changes),
    }))
  );
