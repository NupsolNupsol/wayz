...
export function packBags(bags: BagItem[], assetType: AssetTypeDoc): PackingResult {
  ...
  return {
    ...
    allocations: bins.map((b, i) => ({
      compartmentIndex: i,
      bagIndexes: b.bagIndexes.sort((a, c) => a - c),
    })),
  };
}
...
export function priceQuote(
  ...
  switch (billing) {
    case 'PER_BAG':
      return {
        amount: unitPrice * bagCount,
        summary: `${bagCount} bag(s) × ${unitPrice} (PER_BAG)`,
      };
    case 'PER_COMPARTMENT':
      return {
        amount: unitPrice * compartments,
        summary: `${compartments} compartment(s) × ${unitPrice} (PER_COMPARTMENT)`,
      };
    case 'PACKAGE':
      return {
        amount: packagePrice || unitPrice,
        summary: `Flat package covering ${bagCount} bag(s)`,
      };
    case 'DURATION_BASED':
      return {
        amount: unitPrice * Math.max(1, Math.ceil(durationPeriods)),
        summary: `${Math.ceil(durationPeriods)} period(s) × ${unitPrice} (DURATION_BASED)`,
      };
    default:
