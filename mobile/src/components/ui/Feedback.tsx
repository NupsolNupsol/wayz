...
export function EmptyState({
  ...
  return (
    <View
      className="items-center justify-center gap-2 rounded-xl2 border border-line bg-surface px-6 py-12"
      testID={testID}
    >
      {icon ? (
        <View className="mb-1 h-14 w-14 items-center justify-center rounded-2xl bg-canvas">
          {icon}
        </View>
      ) : null}
...
export function ErrorState({ message, onRetry }: { message: string; onRetry?: ReactNode }) {
  return (
    <View
      className="gap-2 rounded-xl2 border border-danger/30 bg-danger-soft p-4"
      testID="error-state"
    >
