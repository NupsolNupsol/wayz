...
export default function More() {
  ...
  return (
    <Screen scroll testID="more">
      ...
      <Section title="Your session" className="mb-5">
        <ListGroup>
          <ListRow
            ...
            trailing={shift.data ? <StatusPill status={shift.data.status} size="sm" /> : undefined}
          />
          <Divider />
          <ListRow
            testID="more-profile"
            ...
            trailing={
              openIncidents > 0 ? (
                <View className="rounded-full bg-danger px-2 py-0.5">
                  <Body className="text-[11px] font-bold text-white">{openIncidents}</Body>
                </View>
              ) : undefined
            }
          />
      ...
      <Muted className="px-1 text-center">WAYZ · kiosk agent · {me?.tenant?.name ?? ''}</Muted>
    </Screen>
  );
}
