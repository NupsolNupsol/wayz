...
export default function Assets() {
  ...
  const rows = useMemo(() => {
    ...
    return term ? byFilter.filter((u) => u.identifier.toLowerCase().includes(term)) : byFilter;
  }, [data, filter, query]);

  return (
    <Screen padded={false} testID="assets">
      <View className="gap-3 px-4 pb-3 pt-2">
        <AppHeader back title="Assets" subtitle="What is free, busy or out of service" />

        <View className="flex-row gap-3">
          <Tile label="Free" value={counts.free} tone="success" />
          <Tile label="In use" value={counts.busy} tone="info" />
          <Tile label="Down" value={counts.down} tone={counts.down ? 'danger' : 'neutral'} />
        </View>
          ...
          renderItem={({ item, index }) => (
            <ListGroup className={index === 0 ? '' : 'mt-2'}>
              <ListRow
                ...
                subtitle={item.currentBookingId ? 'Holding a booking' : undefined}
