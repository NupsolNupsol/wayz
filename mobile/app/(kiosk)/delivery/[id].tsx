...
/**
 * One delivery, from the desk's side.
 *
 * The agent's job here is a gate: they hold bags a courier wants, and the only thing standing
 * between a stranger and someone's luggage is the check on this screen. So the identity tick and
 * the unlock code sit together in one sheet, and neither alone opens anything.
 */
export default function DeliveryDetail() {
  ...
  const header = (
    <ScreenHeader
      title={d?.bookingRef ?? 'Delivery'}
      subtitle={d?.customerName}
      fallback={KIOSK_TABS.deliveries}
      right={d ? <StatusPill status={d.status} /> : undefined}
    />
  )

  if (detail.isLoading) {
    return (
      <Screen testID="delivery" header={header}>
        <Loading />
      </Screen>
    )
  }
  ...
  const can = (transition: string) => data.transitions.some((t) => t.code === transition)
  const waiting = d.status === 'RELEASE_REQUESTED'

  const release = () => {
    move.mutate(
      { id: d._id, code: 'TO_RELEASE_APPROVED', payload: { compartmentCode: code.trim() } },
      {
        onSuccess: () => {
    ...
    >
        ...
        <Notice tone="warn" testID="delivery-waiting">
          <View className="gap-3">
            <View className="flex-row items-center gap-2">
              ...
              <Body className="flex-1 font-semibold">A courier is at your desk for these bags.</Body>
            </View>
            <Body>
              Check that the person in front of you is{' '}
              <Body className="font-bold">{d.assignedToName ?? 'the assigned courier'}</Body> before you open
              anything.
            </Body>
          </View>
        </Notice>
      ...
      <Section title="The job" className="mt-4">
        <Card>
          <View className="flex-row flex-wrap gap-4">
            <KeyValue label="Courier" value={d.assignedToName ?? 'Not claimed yet'} className="min-w-[45%]" />
            <KeyValue label="Compartment" value={d.assetUnitIdentifier ?? '—'} className="min-w-[45%]" />
        ...
        <Section title={`Bags (${data.bags.length})`} className="mt-4">
          <Card>
            <View className="gap-2">
              {data.bags.map((bag) => (
                <View key={bag.barcode} className="flex-row items-center gap-3 rounded-2xl border border-line p-3">
      ...
      <View className="mt-5 gap-2">
        {can('TO_RELEASE_APPROVED') ? (
          <Button
            ...
            size="lg"
            full
            icon={<Icon name="ShieldCheck" size={17} color={COLORS.white} />}
      ...
      {d.compartmentCode && d.status === 'RELEASE_APPROVED' ? (
        <Notice tone="info" testID="delivery-code-issued">
          <Body>
            The code is on the courier&apos;s phone until {formatDateTime(d.compartmentCodeExpiresAt)}. If it expires,
            approve again.
          </Body>
        </Notice>
      ) : null}
