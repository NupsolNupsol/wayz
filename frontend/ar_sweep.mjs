...
const ROUTES = {
  ...
  accountant: {
    ...
    paths: [
      ...
      '/accounting/settlement/transactions',
      '/accounting/settlement/payments',
    ],
  },
};
...
const ALLOWED =
  /^(WAYZ|WIQAR|LockerFlow|SAR|VAT|CSV|QR|OTP|KDS|ZATCA|Mada|SPAN|Visa|Master ?Card|GCC|EN|AR|[A-Z]{2,5}-[A-Z0-9-]+|[a-z0-9.]+@[a-z.]+|pay-\d+|bk-\d+|dlv-\d+|cm-\d+|exp-\d+|txn-\d+|usr[_-][a-z0-9_]+|ssn-\d+|SD-\d+|MB-\d+|LG-\d+|INC-\d+|[XSML]-\d+|[0-9.,:%+−-]+|[A-Z]{1,3}\d*)$/;
...
async function tenantContent(token) {
  ...
  for (const t of estate?.assetTypes ?? []) {
    add(t.name);
    for (const s of t.stationNames ?? []) add(s);
  }
