...
export function useTillOverview(enabled = true) {
  return useQuery({
    queryKey: qk.till.overview,
    queryFn: tillApi.overview,
    enabled,
    refetchInterval: TILL_POLL_MS,
  });
}

export function useTillQueue(enabled = true) {
  return useQuery({
    queryKey: qk.till.queue,
...
export function useTillTransactions(params?: {
  ...
  return useQuery({
    queryKey: qk.till.transactions(params ?? {}),
...
export function useTillDrawer(shiftId?: string) {
  return useQuery({
    queryKey: qk.till.drawer(shiftId ?? 'open'),
    queryFn: () => tillApi.drawer(shiftId),
  });
}
