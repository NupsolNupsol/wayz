...
export const useIncidentCatalogue = () =>
  useQuery({ queryKey: qk.incidentTypes, queryFn: engineApi.incidentTypes, staleTime: Infinity });
...
export function useUpdateIncident() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (v: { id: string; status: string }) => incidentApi.updateStatus(v.id, v.status),
    onSuccess: () => qc.invalidateQueries({ queryKey: qk.incidents }),
  });
}
