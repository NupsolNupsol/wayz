...
export const useProducts = (engineKind?: EngineKind) =>
  useQuery({ queryKey: qk.products(engineKind), queryFn: () => catalogueApi.products(engineKind) });
export const useUnits = () =>
  useQuery({
    queryKey: qk.units,
    queryFn: catalogueApi.units,
    refetchInterval: OPERATIONS_POLL_MS,
    staleTime: 0,
  });
export const useAssetTypes = () =>
  useQuery({ queryKey: ['assetTypes'], queryFn: catalogueApi.assetTypes, staleTime: Infinity });
