...
export const useTripBoard = (mine = false, enabled = true) =>
  useQuery({
    queryKey: [...KEY, 'board', mine],
    queryFn: () => tripApi.board(mine),
    enabled,
    refetchInterval: OPERATIONS_POLL_MS,
    staleTime: 0,
  });
...
export const useStartTrip = () =>
  useTripMutation((v: { id: string; unitId?: string }) => tripApi.start(v.id, v.unitId));
export const useClockStation = () =>
  useTripMutation((v: { id: string; stationId: string }) => tripApi.clock(v.id, v.stationId));
export const useCompleteTrip = () => useTripMutation((id: string) => tripApi.complete(id));
export const useSetTripRoute = () =>
  useTripMutation((v: { id: string; stationIds: string[] }) =>
    tripApi.setRoute(v.id, v.stationIds)
  );
