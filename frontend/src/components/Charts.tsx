    ...
    <div className="overflow-x-auto scroll-thin">
      <div className="flex items-end gap-3 min-w-full" style={{ height }}>
        {data.map((d) => (
          ...
          >
            <span className="text-xs font-bold text-navy dark:text-dk-text mb-1 tabular-nums">
              {d.value > 0 ? d.value : ''}
            </span>
            <div
              className={clsx(
                'w-full max-w-[46px] rounded-t-lg transition-all',
                d.value > 0 ? 'bg-brand' : 'bg-line'
              )}
      ...
      <div className="flex gap-3 mt-2 border-t border-line pt-2 min-w-full">
        {data.map((d) => (
          <span
            key={d.label}
            className="flex-1 min-w-[26px] text-center text-[11px] font-medium text-muted truncate"
          >
            {d.label}
          </span>
        ))}
  ...
  return (
    <div className="flex items-center gap-5">
      ...
      >
        ...
        <text x="50" y="60" textAnchor="middle" fontSize="6.5" fill="rgb(var(--muted))">
          {t('table.total')}
        </text>
      </svg>
      <ul className="text-sm flex flex-col gap-2 flex-1 min-w-0">
        {data.map((d) => (
          <li key={d.label} className="flex items-center gap-2">
            ...
            <span className="ms-auto font-semibold text-navy dark:text-dk-text tabular-nums">
              {d.value}
            </span>
          </li>
        ))}
