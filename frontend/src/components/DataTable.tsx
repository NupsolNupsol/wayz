    ...
    for (const col of columns) {
      ...
      out =
        col.filter.kind === 'text'
          ? out.filter((r) => val(r).toLowerCase().includes(active.toLowerCase()))
          : out.filter((r) => val(r) === active);
    }
  ...
  return (
    <>
      ...
      <div
        data-testid={testId}
        className={clsx(
          'bg-surface dark:bg-dk-surface rounded-card border border-line shadow-card overflow-hidden',
          className
        )}
      >
        <div className="overflow-x-auto scroll-thin">
          <table className="w-full text-sm border-collapse">
                ...
                {columns.map((c) => {
                  ...
                  return (
                    <th
                      key={c.key}
                      className={clsx(
                        'px-4 py-3 font-bold text-[11.5px] uppercase tracking-wide text-navy dark:text-dk-texthi whitespace-nowrap',
                        alignCls(c.align)
                      )}
                    >
                      <div
                      ...
                      >
                        {c.sortValue ? (
                          <button
                            ...
                            data-testid={testId ? `sort-${c.key}` : undefined}
                          >
                            {c.header}
                            {sorted ? (
                              sort!.dir === 'asc' ? (
                        ...
                        {c.filter && (
                          <button
                            ...
                            className={clsx(
                              ...
                              isFiltered ? 'text-brand' : 'text-muted/50 hover:text-brand'
                            )}
              ...
              {paged.map((row, i) => (
                <tr
                  ...
                  className={clsx(
                    ...
                    i % 2 === 1 && 'bg-canvas/40 dark:bg-dk-elevated/30',
                ...
                >
                  {columns.map((c) => (
                    <td
                      key={c.key}
                      className={clsx('px-4 py-3 align-middle', alignCls(c.align), c.className)}
                    >
                      {c.render(row)}
                    </td>
                  ))}
            ...
            {footer && visible.length > 0 && (
              <tfoot>
                <tr
                  className="border-t-2 border-brand/25 bg-canvas/60 dark:bg-dk-elevated/40 font-bold"
        ...
        {visible.length === 0 && (
          <div className="p-6 text-center text-sm text-muted">
            {t('table.noRows')}
            <button className="text-brand font-medium" onClick={() => setFilters({})}>
        ...
        {visible.length > 0 && (
          <div
            className="flex items-center justify-between gap-3 px-4 py-3 border-t border-line bg-canvas/40 dark:bg-dk-elevated/30"
            data-testid={testId ? `${testId}-pagination` : undefined}
          >
            <p className="text-xs text-muted">
              {t('table.showing')}
              <strong className="text-navy dark:text-dk-text">{from}</strong>–
              <strong className="text-navy dark:text-dk-text">{to}</strong> {t('table.of')}{' '}
              <strong className="text-navy dark:text-dk-text">{visible.length}</strong>
            </p>
            {pageCount > 1 && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="w-8 h-8 rounded-lg border border-line flex items-center justify-center text-muted hover:text-brand hover:border-brand disabled:opacity-40 disabled:hover:text-muted disabled:hover:border-line"
                >
                  <ChevronLeft size={16} />
                </button>
                {Array.from({ length: pageCount })
                  .slice(Math.max(0, current - 2), Math.max(0, current - 2) + 5)
                  .map((_, idx) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => setPage(p)}
                        className={clsx(
                          'min-w-8 h-8 px-2 rounded-lg text-sm font-semibold',
                          p === current
                            ? 'bg-brand text-brand-fg'
                            : 'border border-line text-muted hover:text-brand hover:border-brand'
                        )}
                      >
                        {p + 1}
                      </button>
                ...
                {openCol.filter.kind === 'text' ? (
                  <div className="relative">
                    ...
                    {filters[open] && (
                      <button
                        ...
                        className="absolute right-2 top-1/2 -translate-y-1/2 text-muted hover:text-danger-strong"
                      >
                        <X size={14} />
                      </button>
                    )}
                  ...
                  <div className="max-h-56 overflow-y-auto scroll-thin flex flex-col gap-0.5">
                    <button
                      ...
                      className={clsx(
                        'px-2.5 py-1.5 rounded-lg text-sm text-left hover:bg-brand/5',
                        !filters[open] && 'bg-brand/10 text-brand font-semibold'
                      )}
                    >
                      {t('table.all')}
                    </button>
                    {openCol.filter.options.map((o) => (
                      <button
                        key={o.value}
                        onClick={() => {
                          setFilter(open, o.value);
                          setOpen(null);
                        }}
                        className={clsx(
                          'px-2.5 py-1.5 rounded-lg text-sm text-left hover:bg-brand/5',
                          filters[open] === o.value && 'bg-brand/10 text-brand font-semibold'
                        )}
                      >
                        {o.label}
                      </button>
