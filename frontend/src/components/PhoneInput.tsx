  ...
  return (
    <div className="relative flex items-stretch gap-2" ref={ref} data-testid={testId}>
      <div className="relative">
        ...
        {open && (
          <div
            className="absolute z-[9999] top-[calc(100%+6px)] start-0 w-72 lf-card p-0 shadow-pop overflow-hidden"
            ...
            <div className="max-h-64 overflow-y-auto scroll-thin">
              {filtered.map((c) => (
                <button
                  ...
                  className={clsx(
                    'w-full flex items-center gap-2 px-3 py-2 text-sm text-start hover:bg-canvas dark:hover:bg-dk-elevated',
                    c.iso2 === country.iso2 && 'bg-brand/5'
                  )}
                >
              ...
              {filtered.length === 0 && (
                <p className="p-3 text-sm text-muted text-center">No match</p>
              )}
            </div>
          </div>
        )}
