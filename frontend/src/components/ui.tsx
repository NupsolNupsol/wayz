...
export function FieldGroupTitle({
  ...
  return (
    <p
      className={clsx(
        'flex items-center gap-1.5 lf-label mt-1 mb-2 border-b border-line dark:border-dk-border pb-1.5',
        className
      )}
    >
      {children}
...
export function Badge({
  ...
  return (
    <span className={clsx('lf-chip', BADGE_TONES[tone], className)} data-testid={testId}>
      {children}
    </span>
  );
}
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center py-14 px-6">
      <div className="w-14 h-14 rounded-2xl bg-navy-50 dark:bg-dk-elevated flex items-center justify-center text-brand mb-4">
        {icon}
      </div>
...
export function Field({
  ...
  htmlFor?: string;
  hint?: string;
  /** For a hint that carries a live figure worth asserting on, e.g. seats left. */ hintTestId?: string;
  error?: string;
  required?: boolean;
  children: ReactNode;
  className?: string;
  ...
  return (
    <div className={clsx('mb-4', className)}>
      ...
      {children}
      {hint && !error && (
        <p className="text-xs text-muted mt-1" data-testid={hintTestId}>
          {hint}
        </p>
      )}
      {error && (
        <p className="text-xs text-danger-strong mt-1" role="alert">
          {error}
        </p>
      )}
    </div>
...
export function StatCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={clsx(
        'lf-card lf-card-hover p-4 text-start w-full',
