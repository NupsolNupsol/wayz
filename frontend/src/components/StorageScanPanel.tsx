    ...
    <div data-testid={`${testIdPrefix}-scan-panel`}>
      <button
        ...
        className={clsx(
          ...
          compartmentScanned
            ? 'border-success bg-emerald-50 dark:bg-emerald-900/20 text-success'
            : 'hover:border-brand'
        )}
      >
        {compartmentScanned ? <Check size={18} /> : <ScanLine size={18} />} Scan compartment{' '}
        {unitIdentifier}
      </button>
          ...
          return (
            ...
            >
              <span className="flex items-center gap-2">
                {done ? <Check size={16} className="text-success" /> : <ScanLine size={16} />} Bag{' '}
      ...
      <div className="flex gap-2 mb-3">
        ...
        <Button
          ...
          data-testid={`${testIdPrefix}-scan-manual-add`}
        >
          {t('storage.addScan')}
        </Button>
      </div>
      {[...scanned].some((s) => !known.has(s)) && (
        <p className="text-xs text-danger-strong mb-2" data-testid={`${testIdPrefix}-scan-foreign`}>
          {t('storage.wrongBag')}
        </p>
      )}
      ...
      <Button
        onClick={() =>
          onConfirm({ scannedUnitId: unitId, scannedBarcodes: [...scanned], durationMin })
        }
        loading={pending}
      ...
      {!ready && <p className="text-xs text-amber-600 mt-2">{t('storage.hint')}</p>}
    </div>
  );
}
