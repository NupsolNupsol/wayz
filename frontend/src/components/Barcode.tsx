  ...
  return (
    <div className={className} aria-label={`Barcode ${value}`}>
      <svg
        width="100%"
        height={height}
        viewBox={`0 0 ${bars.reduce((a, b) => a + b + 1, 0)} ${height}`}
      ...
      >
        {(() => {
          ...
          return bars.map((w, i) => {
            const rect =
              i % 2 === 0 ? (
                <rect key={i} x={x} y={0} width={w} height={height} fill="#0f214a" />
              ) : null;
            x += w + 1;
            return rect;
          });
      ...
      */}
      <div className="text-center text-[13px] tracking-[2px] font-mono text-navy dark:text-dk-text mt-1">
        {value}
      </div>
    </div>
  );
