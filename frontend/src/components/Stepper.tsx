...
const revisitStyle = (on: boolean) =>
  on
    ? 'cursor-pointer hover:border-brand hover:text-brand focus:outline-none focus:ring-2 focus:ring-brand/30'
    : '';
        ...
        return (
          <li key={s.key} className="flex items-center gap-1 shrink-0">
            <div
              ...
              onKeyDown={
                revisitable ? (e) => (e.key === 'Enter' || e.key === ' ') && onStep(i) : undefined
              }
              ...
              className={clsx(
                ...
                active && 'bg-brand text-white border-brand',
                  ...
                  'bg-emerald-50 text-success border-emerald-200 dark:bg-emerald-900/30 dark:border-emerald-800',
                !active &&
                  !done &&
                  'bg-white text-muted border-line dark:bg-dk-elevated dark:border-dk-border',
            ...
            >
              <span
                className={clsx(
                  ...
                  active ? 'bg-white/20' : done ? '' : 'bg-slate-100 dark:bg-dk-muted'
                )}
              >
