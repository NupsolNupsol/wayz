...
export function Toaster() {
  ...
  return createPortal(
    <div
      className="fixed bottom-5 end-5 z-[3000] flex flex-col gap-2 w-[340px] max-w-[calc(100vw-2rem)]"
      data-testid="toaster"
    >
      {toasts.map((t) => (
        <div
          key={t.id}
          role="status"
          className="lf-card p-3.5 flex items-start gap-3 shadow-pop animate-[toastIn_.18s_ease]"
        >
