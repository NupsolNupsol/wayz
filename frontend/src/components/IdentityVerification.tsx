...
const CODED: Record<
  'WHATSAPP' | 'EMAIL',
  { channel: OtpChannel; method: 'WHATSAPP_OTP' | 'EMAIL_OTP'; labelKey: string; nounKey: string }
> = {
  WHATSAPP: {
    channel: 'WHATSAPP',
    method: 'WHATSAPP_OTP',
    labelKey: 'ui:identity.whatsappTab',
    nounKey: 'ui:identity.phoneNoun',
  },
  EMAIL: {
    channel: 'EMAIL',
    method: 'EMAIL_OTP',
    labelKey: 'ui:identity.emailTab',
    nounKey: 'ui:identity.emailNoun',
  },
};
...
export function IdentityVerificationModal({
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      if (res.delivered === 'FAILED') {
        ...
        toast(
          'success',
          `Code sent — ${t(coded.labelKey)}`,
          `Sent to ${res.destinationMasked} — ask the customer to read it back.`
        );
      }
    } catch (e) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast(
        'success',
        t('identity.verified'),
        coded ? `${t(coded.labelKey)} confirmed.` : 'Fallback recorded in the audit trail.'
      );
      reset();
      onVerified();
    } catch (e) {
  ...
  const nameMismatch =
    ...
    holderName.trim().toLowerCase() !== customerName.trim().toLowerCase();
      ...
      {coded && (
        <div data-testid="verify-otp-panel">
          <p className="text-sm text-muted mb-3">
            The code goes to the {t(coded.nounKey)} recorded on this booking — it cannot be
            redirected to another
            {coded.channel === 'EMAIL' ? ' inbox' : ' handset'}.
          </p>
          {coded.channel === 'EMAIL' && !customerEmail ? (
            <div
              className="lf-card p-3 border-amber-300 bg-amber-50 dark:bg-amber-900/20"
              data-testid="verify-no-email"
            >
              ...
              <p className="text-xs text-muted mt-1">
                No address was recorded for this customer when the booking was created, so a code
                cannot be emailed. Use WhatsApp, or verify with an ID document. Capture an email
                next time on the customer form to enable this channel.
              </p>
            </div>
          ) : !challenge ? (
            <Button
            ...
            <div
              className="lf-card p-3 border-amber-300 bg-amber-50 dark:bg-amber-900/20"
              data-testid="verify-delivery-failed"
            >
              <p className="flex items-center gap-2 text-sm font-semibold text-amber-700 dark:text-amber-300">
                <MessageSquareWarning size={16} /> {t(coded.labelKey)} could not be delivered
              </p>
              <p className="text-xs text-muted mt-1">
                {challenge.error ?? 'The provider is unavailable.'}
              </p>
              <p className="text-xs text-muted mt-2">
                The code was <strong>not</strong> shown to you on purpose — typing a code you were
                just shown proves nothing. Verify with the <strong>ID document</strong> tab, or ask
                a manager to override.
              </p>
              <div className="flex gap-2 mt-3">
                <Button
                  variant="secondary"
                  onClick={() => setTab('DOCUMENT')}
                  data-testid="verify-goto-document"
                >
                  <IdCard size={15} /> {t('identity.useDocument')}
                </Button>
                <Button variant="ghost" onClick={sendCode} loading={sendMut.isPending}>
                  {t('identity.retrySend')}
                </Button>
              </div>
            </div>
          ) : (
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <button type="button" className="text-xs text-brand" onClick={sendCode}>
                  {t('identity.resend')}
                </button>
              </div>
            </div>
        ...
        <div data-testid={tab === 'DOCUMENT' ? 'verify-document-panel' : 'verify-override-panel'}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
            <Field label={t('identity.docType')} required={tab === 'DOCUMENT'}>
              <Select
                value={docType}
                onChange={(v) => setDocType(v as IdDocumentType)}
                options={DOC_TYPES.map((d) => ({ label: t(d.labelKey), value: d.value }))}
                testId="verify-doc-type"
              />
            </Field>
            <Field
              label={t('identity.docNumber')}
              required={tab === 'DOCUMENT'}
              hint={t('identity.lastFour')}
            >
              <input
                className="lf-input"
                value={docNumber}
          ...
          <Field
            label={t('identity.docName')}
            required={tab === 'DOCUMENT'}
            error={
              nameMismatch
                ? `Does not match the booking (${customerName}) — explain below or escalate.`
                : undefined
            }
          >
            <input
            ...
            {image ? (
              <div className="lf-card p-2 flex items-center gap-3" data-testid="verify-doc-preview">
                ...
                <button
                  type="button"
                  onClick={() => setImage('')}
                  className="lf-btn-ghost !h-8 !px-2 text-xs"
                  data-testid="verify-doc-clear"
                >
                  <X size={14} /> Remove
                </button>
              </div>
              ...
              <>
                ...
                <Button
                  ...
                  data-testid="verify-doc-capture"
                >
          ...
          <Field
            label={
              tab === 'DOCUMENT' ? 'Why was the WhatsApp code not used?' : 'Reason for the override'
            }
            required
          >
            <textarea
              ...
              placeholder={
                tab === 'DOCUMENT'
                  ? 'e.g. Customer lost their phone; ID checked against the booking name'
                  : 'e.g. No phone and no ID; recognised by the desk manager'
              }
...
export function VerificationTrail({ verifications }: { verifications: IdentityVerification[] }) {
  ...
  if (!verifications.length)
    return <p className="text-sm text-muted">{t('identity.noneRecorded')}</p>;
  return (
        ...
        <li key={i} className="lf-card p-3">
          <div className="flex items-center justify-between gap-2">
            <span className="font-medium text-sm flex items-center gap-2">
              <ShieldCheck
                size={15}
                className={v.method === 'WHATSAPP_OTP' ? 'text-success' : 'text-amber-500'}
              />
              {t(`identity.method.${v.method}`, { defaultValue: v.method.replaceAll('_', ' ') })}
            </span>
            <Badge tone={v.status === 'CONSUMED' ? 'neutral' : 'success'}>
              {statusLabel(v.status, 'verification')}
            </Badge>
          </div>
          <p className="text-xs text-muted mt-1">
          ...
          {v.reason && (
            <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
              {t('identity.reason', { reason: v.reason })}
            </p>
          )}
        </li>
      ))}
