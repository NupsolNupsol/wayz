...
export function DiscountInline({
  ...
  const [applied, setApplied] = useState<{ amount: number; label: string; free: boolean } | null>(
    null
  );
  ...
  const apply = () => {
    ...
    discount.mutate(
      ...
      // is what they should pay — not that rounded to the nearest percent of the sale.
      {
        ...
        ...(free ? { percent: 100 } : { amount: amountOff }),
      },
      {
        onSuccess: () => {
        ...
        onError: (e) =>
          setProblem(
            e instanceof ApiError ? (e.errors?.join(' ') ?? e.message) : t('ui:discount.refused')
          ),
      }
    );
  };
      ...
      >
        ...
        <span className="text-navy dark:text-dk-text">
          {applied.free
            ...
            : t('ui:discount.someApplied', {
                amount: money(applied.amount),
                reason: applied.label,
              })}
        </span>
      </div>
    );
  ...
  return (
    <div className="mb-3" data-testid="discount-inline">
      ...
      {open && (
        <div
          className="mt-2 rounded-xl2 border border-line dark:border-dk-border p-3"
        ...
        >
          ...
          <Select
            ...
            options={[
              ...
              ...reasons.map((r) => ({
                label: `${r.label} · up to ${r.maxPercent}%`,
                value: r.code,
              })),
            ]}
