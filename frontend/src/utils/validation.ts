...
export function customerProblems(input: {
  name: string;
  phone: string;
  email?: string;
}): FieldProblem[] {
  const problems: FieldProblem[] = [];
  if (!isCompleteName(input.name))
    problems.push({ field: 'name', messageKey: 'ui:customer.nameTooShort' });
  if (!isCompletePhone(input.phone))
    problems.push({ field: 'phone', messageKey: 'ui:customer.phoneIncomplete' });
  if (!isCompleteEmail(input.email ?? ''))
    problems.push({ field: 'email', messageKey: 'ui:customer.emailInvalid' });
  return problems;
}

export const isCustomerComplete = (input: {
  name: string;
  phone: string;
  email?: string;
}): boolean => customerProblems(input).length === 0;
