...
import {
  ...
  useReassign,
  useCreateIncident,
  useIncidentCatalogue,
  useUnits,
  useRefundPosition,
  useRefundBooking,
  useStationDeliveries,
} from '@/hooks';
...
export function BookingDetailPage() {
  ...
  const { data: stationDeliveries } = useStationDeliveries(
    { bookingId: id },
    !!id && mayArrangeDelivery
  );
  ...
  const heldUnit = heldUnitDetail
    ? { _id: heldUnitDetail._id, identifier: heldUnitDetail.identifier }
    : undefined;
  ...
  const outstanding = Math.max(
    0,
    Math.round(((order?.total ?? 0) - (refundPosition?.paid ?? 0)) * 100) / 100
  );
    ...
    return (
      <div>
        <PageHeader
          ...
          title={t('page.booking')}
          crumbs={[
            { label: t('common:crumb.bookings'), to: '/bookings' },
            { label: t('common:crumb.notfound') },
          ]}
        />
        <Card>
          <EmptyState title={t('page.notFound')} />
        </Card>
      </div>
    );
  }
  ...
  const incidentOptions = engineTypes.map((type) => ({
    label: t(`status:incidentType.${type}`, {
      defaultValue: catalogue?.labels?.[type] ?? type.replaceAll('_', ' '),
    }),
    value: type,
  }));
  ...
  const runTransition = (transition: AvailableTransition) => {
    ...
    if (transition.code === 'TO_REASSIGNED') {
      setReassignUnit(
        units.find(
          (u) =>
            u.assetTypeId === (booking.metadata?.assetTypeId as string) && u.status === 'AVAILABLE'
        )?._id ?? ''
      );
  ...
  const scanOut = (barcode: string) => {
    scanMut.mutate(
      ...
      {
        onSuccess: (b) => {
          toast('success', t('toast.scanned'));
          setManualScan('');
          void b;
        },
        onError: (e) =>
          toast('danger', t('toast.scanRejected'), e instanceof ApiError ? e.message : ''),
      }
    );
  };
  ...
  const doReassign = () => {
    reassignMut.mutate(
      { id, unitId: reassignUnit, reason: reassignReason },
      {
        onSuccess: () => {
          toast('warning', t('toast.reassigned'));
        ...
        onSuccess: (res) => {
          ...
          if (res.approved) {
            toast(
              'success',
              t('toast.refundDone'),
              t('toast.refundDetail', { amount: money(res.refunded), ref: booking.ref })
            );
          } else {
            toast(
              'info',
              t('toast.refundAsked'),
              t('toast.refundAskedDetail', {
                ref: res.request?.ref ?? '',
  ...
  const submitIncident = () => {
    incidentMut.mutate(
      { type: incType, description: incDesc, bookingId: id },
      {
        onSuccess: () => {
          toast('warning', t('toast.incidentReported'));
  ...
  const availableAssetUnits = units.filter(
    (u) => u.assetTypeId === (booking.metadata?.assetTypeId as string) && u.status === 'AVAILABLE'
  );
  const inRetrieval = booking.status === 'RETRIEVAL_IN_PROGRESS';
  const canDeliver =
    mayArrangeDelivery &&
    booking.engineKind === 'SHOP_AND_DROP' &&
    ['ACTIVE', 'OVERTIME'].includes(booking.status);
  ...
  return (
    <div data-testid="booking-detail">
          ...
          <>
            {order && (
              <Button
                ...
                data-testid="booking-invoice-slip"
              >
            ...
            {booking.bags.length > 0 && (
              <Button
                ...
                data-testid="booking-labels"
              >
                <Printer size={16} /> {t('page.labels')}
              </Button>
            )}
            {canDeliver && !openDelivery && (
              <Button
                ...
                data-testid="booking-request-delivery"
              >
            ...
            <Button
              ...
              disabled={!order || invoiceXml.loading}
            >
      ...
      {openDelivery && (
        <Card
          className="mb-5 p-4 border-brand/40 bg-brand/5 flex flex-wrap items-center justify-between gap-3"
          data-testid="booking-open-delivery"
        >
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-navy dark:text-dk-texthi">
                {t('page.deliveryLine', {
                  id: openDelivery._id,
                  status: statusLabel(openDelivery.status, 'delivery'),
                })}
              </p>
              <p className="text-xs text-muted truncate">
                {t('page.goingTo', { address: openDelivery.destination.address })}
              </p>
            </div>
          </div>
          <Button
            ...
            data-testid="booking-open-delivery-link"
          >
      ...
      {awaitingApproval && (
        <Card
          className="mb-5 p-4 border-amber-400/50 bg-amber-50/60 dark:bg-amber-900/10"
          data-testid="booking-refund-pending"
        >
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-start gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-navy dark:text-dk-texthi">
                  {t('page.refundWaiting', {
                    ref: awaitingApproval.ref,
                    amount: money(awaitingApproval.amount),
                  })}
                </p>
                <p className="text-xs text-muted">
                  {t('page.refundWaitingWho', { name: awaitingApproval.requestedByName })} ·{' '}
                  {awaitingApproval.reason}
                </p>
              </div>
            </div>
            {refundPosition?.canApprove && (
              <Button
                ...
                data-testid="booking-refund-review"
              >
                {t('page.refundReview')}
              </Button>
            )}
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <span
              ...
              data-testid="booking-refunded-total"
            >
              {money(refundedSoFar)}
            </span>
          </div>
          <div className="flex flex-col gap-2">
            {booking.refunds.map((r, i) => (
              <div
                ...
                data-testid={`booking-refund-${i}`}
              >
                <span className="font-semibold tabular-nums text-navy dark:text-dk-texthi">
                  {money(r.amount)}
                </span>
          ...
          {refundable > 0 && (
            <p className="text-xs text-muted mt-2">
              {t('page.stillRefundable', { amount: money(refundable) })}
            </p>
          )}
        </Card>
      )}
      ...
      <Card className="mb-5">
        ...
        {awaitingRetrievalCheck && (
          <div
            className="lf-card p-3 mb-3 flex flex-wrap items-center justify-between gap-3 border-brand/40 bg-brand/5"
            data-testid="verify-required"
          >
        ...
        {canBeginRetrieval && hasFreshVerification && (
          <div
            className="lf-card p-3 mb-3 flex items-center gap-2 border-success/50 bg-emerald-50 dark:bg-emerald-900/20"
        ...
        {owed > 0 && workflowActions.some((tr) => HANDS_BACK.includes(tr.code)) && (
          ...
          >
            ...
            <span className="text-sm text-navy dark:text-dk-texthi flex-1 min-w-0">
              {awaitingRetrievalCheck
                ? t('page.verifyBeforePayment', { amount: money(owed) })
                : t('page.payFirst', { amount: money(owed) })}
            </span>
            <Button
        ...
        <div className="flex flex-wrap gap-2" data-testid="transition-actions">
          {workflowActions.length === 0 && (
            <p className="text-sm text-muted">{t('page.noActions')}</p>
          )}
          {workflowActions.map((transition) => (
            <button
              ...
              title={
                blockedByMoney(transition.code)
                  ? t('page.payFirst', { amount: money(owed) })
                  : undefined
              }
            ...
            >
              {transition.code === 'TO_REASSIGNED' && <RefreshCw size={15} />}{' '}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 flex flex-col gap-5">
          <Card>
            <div className="flex items-center justify-between mb-3">
              <SectionTitle>{t('page.session')}</SectionTitle>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              ...
              <Meta
                ...
                value={formatDateTime(s.startedAt ? new Date(s.startedAt).getTime() : null)}
              />
              <Meta
                label={t('meta.expectedEnd')}
                value={formatDateTime(s.expectedEndAt ? new Date(s.expectedEndAt).getTime() : null)}
              />
            </div>
            {s.startedAt ? (
              <div className="mt-3 flex items-center gap-2 text-sm">
                <span className="text-muted">{t('page.remaining')}</span>{' '}
                <Timer
                  expectedEndAt={s.expectedEndAt}
                  endedAt={s.endedAt ?? s.chargeableEndedAt}
          ...
          {booking.bags.length > 0 && (
            <Card>
              <SectionTitle className="mb-3 flex items-center gap-2">
                <Boxes size={18} /> {t('page.bags', { count: booking.bags.length })}
              </SectionTitle>
              {inRetrieval && (
                <Field label={t('page.manualBarcode')}>
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder={t('page.enterBarcode')}
                      data-testid="retrieval-manual"
                    />
                    <Button
                      variant="secondary"
                      onClick={() => scanOut(manualScan)}
                      disabled={!manualScan.trim()}
                      data-testid="retrieval-manual-scan"
                    >
                      {t('page.scan')}
                    </Button>
                  </div>
                    ...
                    <div key={b.index} className="lf-card p-3" data-testid={`bag-${b.index}`}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-semibold text-sm">
                          {t('page.bagLine', { index: b.index, description: b.description })}
                        </span>
                      ...
                      {inRetrieval && (
                        <button
                          onClick={() => scanOut(b.barcode)}
                          disabled={out}
                          data-testid={`retrieval-scan-${b.index}`}
                          className={`mt-2 w-full lf-btn ${out ? 'bg-success/10 text-success' : 'lf-btn-secondary'}`}
                        >
                          {out ? (
                            <>
                              <Check size={15} /> {t('page.scannedOut')}
                            </>
                          ) : (
                            <>
                              <ScanLine size={15} /> {t('page.scanOut')}
                            </>
                          )}
                        </button>
                      )}
          ...
          {booking.packingPlan && (
            <Card>
              ...
              <p className="text-sm">
                <strong>{booking.packingPlan.numberOfCompartmentsRequired}</strong>{' '}
                {t('page.compartmentsFor', { count: booking.bags.length })}
              </p>
              <p className="text-xs text-muted mt-1">
                {booking.packingPlan.priceCalculationSummary}
              </p>
            </Card>
          )}
        </div>
        ...
        <div className="flex flex-col gap-5">
          <Card>
            <SectionTitle className="mb-3 flex items-center gap-2">
              <User size={18} /> {t('page.customer')}
            </SectionTitle>
            <p className="text-base font-semibold text-navy dark:text-dk-text">
          ...
          {order && (
            <Card data-testid="booking-charges">
              <SectionTitle className="mb-3 flex items-center gap-2">
                <Receipt size={18} /> {t('page.charges')}
              </SectionTitle>
              <div className="flex flex-col gap-1.5 text-sm">
                ...
                <div className="flex items-baseline justify-between gap-3 font-bold text-navy dark:text-dk-texthi text-base">
                  ...
                  <span className="flex items-baseline gap-2">
                    {discountOff > 0 && (
                      <span
                        className="text-sm font-normal text-muted line-through tabular-nums"
                        data-testid="booking-before-discount"
                      >
                        {money(order.total + discountOff)}
                      </span>
                    )}
                    <span className="tabular-nums" data-testid="booking-charged-total">
                      {money(order.total)}
                    </span>
                  </span>
                </div>
                ...
                <div className="flex items-baseline justify-between gap-3 text-xs text-muted">
                  ...
                  <span className="tabular-nums" data-testid="booking-paid-total">
                    {money(refundPosition?.paid ?? 0)}
                  </span>
                </div>
                {outstanding > 0 && (
                  <div className="flex items-baseline justify-between gap-3 text-xs font-semibold text-amber-700 dark:text-amber-300">
                    ...
                    <span className="tabular-nums" data-testid="booking-outstanding">
                      {money(outstanding)}
                    </span>
                  </div>
                )}
          ...
          {verifications.length > 0 && (
            <Card>
              <SectionTitle className="mb-3 flex items-center gap-2">
                <ShieldCheck size={18} /> {t('page.identityChecks')}
              </SectionTitle>
          ...
          <AmountDuePanel
            ...
            blockedReason={
              awaitingRetrievalCheck
                ? t('page.verifyBeforePayment', { amount: money(dueNow) })
                : null
            }
          ...
          <Card>
            <SectionTitle className="mb-3 flex items-center gap-2">
              <Package size={18} /> {t('page.custody')}
            </SectionTitle>
            {booking.custody.length === 0 ? (
              <p className="text-sm text-muted">{t('page.noCustody')}</p>
            ) : (
              <ol className="relative border-s border-line ms-2">
                {booking.custody.map((c, i) => (
                  <li key={i} className="ms-4 pb-4 last:pb-0">
                    ...
                    <p className="text-xs text-muted">
                      {c.note} · {formatDateTime(new Date(c.at).getTime())}
                    </p>
                  </li>
                ))}
              </ol>
      ...
      <Modal
        ...
        title={t('labelsModal.title')}
        subtitle={t('labelsModal.subtitle')}
        size="md"
        footer={
          <>
            ...
            <Button onClick={() => window.print()} className="no-print">
              <Printer size={15} /> {t('labelsModal.print')}
            </Button>
          </>
        }
      >
          ...
          <StorageScanPanel
            ...
            onConfirm={(payload) =>
              transitionMut.mutate(
                ...
                {
                  onSuccess: () => {
                    toast('success', t('toast.stored'), t('toast.storedDetail'));
                    setStoreOpen(false);
                  },
                  onError: (e) =>
                    toast(
      ...
      <ReasonDialog
        ...
        onConfirm={(reason) => {
          ...
          transitionMut.mutate(
            ...
            {
              onSuccess: () =>
                toast('warning', t('cancel.done'), t('cancel.doneDetail', { ref: booking.ref })),
              onError: (e) =>
                toast(
                  'danger',
                  t('cancel.failed'),
        ...
        footer={
          <>
            ...
            <Button
              ...
              disabled={!reassignReason.trim() || !reassignUnit}
      ...
      >
        <Field label={t('reassign.unit')}>
          <Select
            value={reassignUnit}
            onChange={setReassignUnit}
            searchable
            placeholder={t('reassign.choose')}
            options={availableAssetUnits.map((u) => ({ label: u.identifier, value: u._id }))}
            testId="reassign-unit-select"
          />
        </Field>
        <Field label={t('reassign.reason')} required>
          <input
            className="lf-input"
      ...
      <Modal
        ...
        subtitle={t('refundModal.subtitle', {
          paid: money(refundPosition?.paid ?? 0),
        ...
        footer={
          <>
            ...
            <Button
              ...
              disabled={
                refundReason.trim().length < 3 || refundAmount <= 0 || refundAmount > refundable
              }
      ...
      >
        ...
        {refundAmount > refundable && (
          <p className="text-sm text-danger-strong -mt-2 mb-3">
            {t('refundModal.tooMuch', { amount: money(refundable) })}
          </p>
        )}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            >
              {t('incidentModal.submit')}
            </Button>
          </>
        }
      >
        <Field
          label={t('incidentModal.type')}
          hint={t('incidentModal.typeHint', { engine: t(`common:engine.${booking.engineKind}`) })}
        >
          <Select
            value={incType}
            onChange={(v) => setIncType(v as IncidentType)}
            options={incidentOptions}
            testId="booking-incident-type"
          />
        </Field>
        <Field label={t('incidentModal.description')} required>
          <textarea
            className="lf-input h-24 py-2"
            value={incDesc}
...
function Meta({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="font-semibold text-navy dark:text-dk-text">{value}</p>
    </div>
  );
}
