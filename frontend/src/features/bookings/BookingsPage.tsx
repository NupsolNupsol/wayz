...
export function BookingsPage() {
  ...
  const columns: Column<Booking>[] = [
    ...
    },
    {
      ...
      render: (b) => <span className="text-muted">{engineLabel(b.engineKind)}</span>,
    },
    {
      key: 'product',
    ...
    },
    {
      ...
      render: (b) => (
        <div className="tabular-nums">
          <p
            className="font-medium text-navy dark:text-dk-text"
            data-testid={`booking-price-${b.id}`}
          >
            {money(b.amountCharged ?? b.totalAmount ?? 0)}
          </p>
          {(b.amountDue ?? 0) > 0 && (
            <p
              className="text-xs text-amber-600 dark:text-amber-300"
              data-testid={`booking-due-${b.id}`}
            >
    ...
    {
      key: 'date',
      header: t('common:column.created'),
      align: 'right',
      sortValue: (b) => new Date(b.createdAt).getTime(),
      render: (b) => (
        <span className="text-muted">{formatDateTime(new Date(b.createdAt).getTime())}</span>
      ),
    },
  ];
