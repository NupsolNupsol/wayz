...
export function AmountDuePanel({
      ...
      },
      {
        onSuccess: (r) => {
          toast(
            'success',
            t('due.collected'),
            t('due.collectedDetail', { amount: money(r.collected) })
          );
          setOpen(false);
  ...
  const extras = (order?.lines ?? []).filter(
    (l) =>
      ['OVERTIME_PENALTY', 'WRONG_STATION_PENALTY'].includes(l.productId) ||
      l.productId.startsWith('EXTENSION')
  );
  ...
  return (
    <>
      {due > 0 && (
        <Card
          key="amount-due"
          className="border-amber-400 bg-amber-50 dark:bg-amber-900/20"
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p
                className="text-2xl font-bold text-navy dark:text-dk-texthi mt-1 tabular-nums"
              ...
              {(extras.length > 0 || pendingOvertime > 0) && (
                <ul
                  className="text-xs text-muted mt-2 flex flex-col gap-0.5"
                  data-testid="amount-due-lines"
                >
                  {pendingOvertime > 0 && (
                    <li className="flex justify-between gap-4">
                      <span>
                        {t('due.overtimeRunning', { count: overtime?.chargeableHours ?? 1 })}
                      </span>
