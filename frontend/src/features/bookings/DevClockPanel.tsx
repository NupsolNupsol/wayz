...
export function DevClockPanel({
    ...
    age.mutate(
      { id: bookingId, minutes: by },
      {
        onSuccess: (r) =>
          toast(
            'info',
            t('devClock.moved', { minutes: by }),
            t('devClock.movedDetail', { ref: r.ref })
          ),
        onError: (e) =>
          toast(
            'danger',
  ...
  return (
    <Card className="border-dashed" data-testid="dev-clock">
      ...
      {!hasStarted ? (
        <p className="text-xs text-amber-600" data-testid="dev-clock-not-started">
          {t('devClock.notStarted')}
        </p>
      ) : (
        <>
          <div className="flex flex-wrap gap-2 mb-3">
            {PRESETS.map((by) => (
              <Button
                ...
                variant="secondary"
                onClick={() => move(by)}
                loading={age.isPending}
                data-testid={`dev-clock-${by}`}
              >
          ...
          <div className="flex items-end gap-2">
            <Field label={t('devClock.custom')} className="mb-0 flex-1">
              <NumberInput
                ...
                min={-1440}
                max={1440}
                testId="dev-clock-minutes"
              />
            </Field>
            <Button
              onClick={() => move(minutes)}
              loading={age.isPending}
              data-testid="dev-clock-apply"
            >
              {t('devClock.apply')}
            </Button>
          </div>
