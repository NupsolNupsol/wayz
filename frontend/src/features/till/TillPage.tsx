    ...
    <div className="flex items-baseline justify-between gap-3 py-1.5">
      ...
      <p
        className={clsx(
          ...
          sign === '−' ? 'text-danger-strong' : 'text-navy dark:text-dk-texthi'
        )}
      >
        {sign === '−' ? '−' : sign === '+' ? '+' : ''}
        {money(value)}
    ...
    <div data-testid="till-overview">
      <PageHeader
        ...
        actions={
          <Button
            ...
            data-testid="till-go-queue"
          >
            <ClipboardCheck size={16} />
            {t('till.awaitingPayment')}
          </Button>
        }
      />
          ...
          <Button
            onClick={() =>
              openShift.mutate(undefined, {
                ...
                onError: (e) =>
                  toast('danger', t('till.couldNotOpen'), e instanceof ApiError ? e.message : ''),
              })
            }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <StatCard
          ...
          sublabel={
            data.shift
              ? `${waitedFor(Date.now() - new Date(data.shift.openedAt).getTime())}`
              : t('till.noTillOpen')
          }
        ...
        <StatCard
          ...
          sublabel={
            data.queue.count
              ? `${money(data.queue.value)} · ${waitedFor(data.queue.oldestWaitingMs)}`
              : t('till.nothingWaiting')
          }
          onClick={() => navigate('/till/queue')}
        ...
        <section>
          ...
          <Card className="p-4" data-testid="till-drawer">
            {!d ? (
              <EmptyState
                icon={<Wallet size={26} />}
                title={t('till.noOpenTill')}
                message={t('till.openOneToTrack')}
              />
            ) : (
              <>
                <DrawerLine
                  label={t('till.openingFloat')}
                  value={d.floatIn}
                  sign="+"
                  hint={t('till.cashPutIn')}
                />
                <DrawerLine
                  label={t('till.cashTaken')}
                  value={d.cashSales}
                  sign="+"
                  hint={t('till.paymentsInCash')}
                />
                <DrawerLine
                  label={t('till.cashRefunded')}
                  value={d.cashRefunds}
                  sign="−"
                  hint={t('till.givenBack')}
                />
                <DrawerLine
                  label={t('till.paidOut')}
                  value={d.paidOut}
                  sign="−"
                  hint={t('till.expensesFromDrawer')}
                />
                <DrawerLine
                  label={t('till.banked')}
                  value={d.dropped}
                  sign="−"
                  hint={t('till.removedToSafe')}
                />
                ...
                <div className="border-t border-line dark:border-dk-border mt-2 pt-3 flex items-baseline justify-between">
                  <p className="font-semibold text-navy dark:text-dk-texthi">
                    {t('till.shouldBeInDrawer')}
                  </p>
                  <p
                    className="text-xl font-bold tabular-nums text-navy dark:text-dk-texthi"
                    data-testid="till-drawer-total"
                  >
                    {money(d.derived)}
                  </p>
                </div>
                ...
                {Math.abs(d.drift) > 0.009 && (
                  <p className="mt-2 text-xs text-danger-strong" data-testid="till-drift">
                    This does not match the running total ({money(d.expected)}). Report it — do not
                    adjust your count.
                  </p>
                )}
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <Button
                    variant="secondary"
                    onClick={() => navigate('/till/drawer')}
                    data-testid="till-go-drawer"
                  >
                    <Banknote size={16} />
                    {t('till.movements')}
                  </Button>
                  <Button
                    variant="ghost"
                    onClick={() => navigate('/till/shift')}
                    data-testid="till-go-shift"
                  >
                    <Clock size={16} />
                    {t('till.countAndClose')}
                  </Button>
                </div>
              </>
            )}
          ...
          <Card className="p-4" data-testid="till-today">
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="lf-card p-3">
                ...
                <p className="text-lg font-bold tabular-nums text-navy dark:text-dk-texthi">
                  {money(data.today.cash)}
                </p>
              </div>
              <div className="lf-card p-3">
                <p className="text-xs text-muted flex items-center gap-1.5">
            ...
            <Button
              ...
              className="mt-4"
              onClick={() => navigate('/till/transactions')}
              data-testid="till-go-transactions"
            >
              {t('till.everyTransaction')}
              <ArrowRight size={15} />
            </Button>
          </Card>
