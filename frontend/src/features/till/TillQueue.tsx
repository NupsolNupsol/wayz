...
function QueueCard({
  ...
  return (
    <Card
      className={clsx(
        'p-4 transition-shadow hover:shadow-pop',
        stale && 'border-amber-300 bg-amber-50/50 dark:bg-amber-900/10'
      )}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {stale && (
              <Badge tone="warning">
                <Clock size={11} className="me-1 inline" />{' '}
                {t('queue.waitingFor', { duration: waitedFor(row.waitingMs) })}
              </Badge>
            )}
          </div>
        ...
        <div className="text-end shrink-0">
          ...
          {row.depositTotal > 0 && (
            <p className="text-[11px] text-muted">
              {t('queue.inclDeposit', { amount: money(row.depositTotal) })}
            </p>
          )}
          <p className="text-[11px] text-muted">
            {t('queue.vat')} {money(row.vat)}
          </p>
        </div>
      </div>
    ...
    pay.mutate(
      { id: taking.bookingId, splits },
      {
        onSuccess: () => {
          toast(
            'success',
            `${money(taking.total)} taken`,
            `${taking.ref} is paid — the agent can assign a compartment.`
          );
          setTaking(null);
        },
        onError: (e) =>
  ...
  return (
    <div data-testid="till-queue">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
        <StatCard
          label={t('queue.inTheQueue')}
          value={rows.length}
          icon={<ClipboardCheck size={18} />}
          tone={rows.length ? 'warning' : 'neutral'}
          testId="queue-stat-count"
        />
        <StatCard
          label={t('queue.value')}
          value={money(value)}
          ...
          tone="info"
          testId="queue-stat-value"
        />
        <StatCard
          label={t('queue.waitingTooLong')}
          value={overdue}
          icon={<Clock size={18} />}
          tone={overdue ? 'danger' : 'neutral'}
          sublabel={t('queue.overMinutes', { count: STALE_QUEUE_MS / 60000 })}
      ...
      {!tillOpen && (
        <Card
          className="mb-5 p-4 flex flex-wrap items-center justify-between gap-3 border-amber-300 bg-amber-50 dark:bg-amber-900/20"
          data-testid="queue-till-closed"
        >
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-navy dark:text-dk-text">
              Your till is {overview?.shift ? 'awaiting a supervisor' : 'closed'}. Card payments
              still work; cash will be refused until it is open.
            </p>
          </div>
          {!overview?.shift && (
            <Button
              onClick={() =>
                openShift.mutate(undefined, {
                  onSuccess: () => toast('success', t('queue.tillOpen')),
                })
              }
            ...
            { id: row.bookingId, code: 'TO_CANCELLED', payload: { reason } },
            {
              onSuccess: () => {
                ...
                toast(
                  'warning',
                  t('queue.cancelled'),
                  t('queue.cancelledDetail', { ref: row.ref })
                );
              },
              onError: (e) =>
                toast(
                  'danger',
                  t('queue.notCancelled'),
                  e instanceof ApiError ? (e.errors?.join(' ') ?? e.message) : ''
                ),
            }
          );
          ...
          <>
            <div className="lf-card p-3 mb-4">
              ...
              {(taking.discountOff ?? 0) > 0 && (
                <div
                  className="flex items-baseline justify-between py-1 text-success font-medium"
                  data-testid="queue-discount-off"
                >
              ...
              <div className="border-t border-line dark:border-dk-border mt-1 pt-2 flex items-baseline justify-between">
                ...
                <span className="text-lg font-bold tabular-nums text-navy dark:text-dk-texthi">
                  {money(taking.total)}
                </span>
              </div>
            </div>
            ...
            <div className="mb-3 flex flex-wrap justify-end gap-2">
              <DiscountButton
                bookingId={taking.bookingId}
                total={taking.total}
                onDone={() => startTaking(null)}
              />
            </div>
            ...
            {!!taking.customerPhone && !confirmed && (
              <p
                className="text-xs text-amber-700 dark:text-amber-300 mt-3"
                data-testid="queue-needs-otp"
              >
            ...
            <p className="text-xs text-muted mt-3 flex items-start gap-1.5">
              <Printer size={13} className="mt-0.5 shrink-0" />A receipt is issued automatically.
              Paying confirms the booking — it does not start the storage timer, which begins only
              when the agent scans the bags into a compartment.
            </p>
          </>
        )}
      </Modal>
