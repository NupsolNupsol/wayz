...
export function PlatformOverview() {
  const report = useQuery({
    ...
    queryFn: () => platformApi.report(),
  });
  const assistant = useQuery({
    queryKey: ['platform', 'assistant'],
    queryFn: () => platformApi.assistant(),
  });
  ...
  const since = new Date(report.data.from).toLocaleDateString(undefined, {
    day: 'numeric',
    month: 'short',
  });
  ...
  return (
    <div data-testid="platform-overview">
      <PageHeader
        title="Platform"
        subtitle={`Every organisation on this deployment · since ${since}`}
      />
        ...
        <DataTable<OrganisationFigures>
          ...
          columns={[
            ...
            },
            {
              ...
              header: 'Activities',
              align: 'right',
              sortValue: (o) => o.activities.length,
              render: (o) => o.activities.length,
            },
            {
              key: 'staff',
              header: 'People',
            ...
            },
            {
              ...
              render: (a) => (
                <span className={a.organisations === 0 ? 'text-muted' : ''}>
                  {a.organisations === 0
                    ? 'nobody'
                    : `${a.organisations} of ${totals.organisations}`}
                </span>
              ),
            },
            {
