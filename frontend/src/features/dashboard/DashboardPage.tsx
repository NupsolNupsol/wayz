...
const shortEngine = (kind: EngineKind): string =>
  i18n.t(`common:engineShort.${kind}`, { defaultValue: kind });
...
export function DashboardPage() {
  ...
  const actions = [
    ...
    ...(sells && engines.includes('LAGOON')
      ...
      : []),
    ...(sails
      ? [{ icon: 'Map', label: t('voyage.title'), to: '/lagoon/voyage', testId: 'qa-voyage' }]
      : []),
    ...(can(me?.role, 'customer.manage')
      ? [
          {
            icon: 'Users',
            label: t('common:crumb.customers'),
            to: '/customers',
            testId: 'qa-customers',
          },
        ]
  ...
  const active = bookings
    ...
    .sort(
      (a, b) =>
        new Date(a.session.expectedEndAt ?? 0).getTime() -
        new Date(b.session.expectedEndAt ?? 0).getTime()
    )
    .slice(0, 5);
  ...
  const engineBars = (stats?.byEngine ?? []).map((e) => ({
    label: shortEngine(e.engineKind),
    value: e.count,
  }));
  const engineDonut = (stats?.byEngine ?? [])
    .filter((e) => e.count > 0)
    .map((e, i) => ({
      label: shortEngine(e.engineKind),
      value: e.count,
    ...
    <div data-testid="dashboard">
          ...
          <>
            {sells && (
              <Button
                ...
                data-testid="dash-shift"
              >
                <Clock size={16} /> {t('dashboard.shift')}{' '}
                {shift ? (
                  <StatusBadge status={shift.status} group="shift" />
                ) : (
            ...
            {sells && (
              <Button onClick={() => navigate('/pos')} data-testid="dash-new-transaction">
                <CirclePlus size={16} /> {t('dashboard.newTransaction')}
              </Button>
            )}
          </>
        }
      ...
      {isLoading && !isError ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
            <StatCard
              label={t('dashboard.stat.todaysTransactions')}
            ...
            />
            <StatCard
              label={t('dashboard.stat.todaysRevenue')}
            ...
            />
            <StatCard
              label={t('dashboard.stat.activeOperations')}
            ...
            />
            <StatCard
              label={t('dashboard.stat.bagsStored')}
              value={stats.storedBags}
              icon={<Boxes size={22} />}
              tone="neutral"
              testId="stat-bags"
              onClick={() => navigate('/operations')}
            />
            <StatCard
              label={t('dashboard.stat.dueSoon')}
              value={stats.dueSoon}
              icon={<TimerIcon size={22} />}
              tone="warning"
              sublabel={t('dashboard.stat.within45')}
              testId="stat-duesoon"
              onClick={() => navigate('/operations')}
            />
            <StatCard
            ...
            />
            <StatCard
              label={t('dashboard.stat.pendingRetrievals')}
              value={stats.pendingRetrievals}
              icon={<PackageCheck size={22} />}
              tone="warning"
              testId="stat-retrievals"
              onClick={() => navigate('/operations')}
            />
            <StatCard
              label={t('dashboard.stat.openIncidents')}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
            <Card>
              <SectionTitle className="mb-3 flex items-center gap-2">
                <BarChart3 size={18} /> {t('dashboard.bookingsByEngine')}
              </SectionTitle>
              <BarChart data={engineBars} />
            </Card>
            <Card>
              <SectionTitle className="mb-3 flex items-center gap-2">
                <PieChart size={18} /> {t('dashboard.engineMix')}
              </SectionTitle>
              {engineDonut.length ? (
                <DonutChart data={engineDonut} />
              ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="flex flex-col gap-2">
              {active.map((b) => (
                <button
                  ...
                  className="lf-card p-3 flex items-center gap-3 text-start lf-card-hover"
                  data-testid={`active-${b.ref}`}
                >
                  <div className="w-10 h-10 rounded-xl bg-brand/10 flex items-center justify-center text-brand shrink-0">
                    <Icon name={ENGINE_META[b.engineKind].icon} size={18} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-sm text-navy dark:text-dk-texthi">
                        {b.ref}
                      </span>
        ...
        <Card>
          ...
          <SectionTitle className={clsx('mb-3', actions.length > 0 && 'mt-5')}>
            {t('dashboard.recent')}
          </SectionTitle>
          <div className="flex flex-col divide-y divide-line">
            {recent.map((b) => (
              <button
                key={b.id}
                onClick={() => navigate(`/bookings/${b.id}`)}
                className="flex items-center gap-2 py-2 text-start text-sm"
              >
                <RefLink to={`/bookings/${b.id}`} className="w-16 shrink-0">
                  {b.ref}
                </RefLink>
...
function QuickAction({
  ...
  return (
    <button
      onClick={onClick}
      data-testid={testId}
      className="flex flex-col items-center gap-2 p-3 rounded-xl2 border border-line hover:border-brand hover:bg-brand/5 transition-colors"
    >
