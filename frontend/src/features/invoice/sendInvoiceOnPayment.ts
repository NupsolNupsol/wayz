...
export async function sendInvoiceOnPayment(
  bookingId: string,
  ...
  try {
    ...
    if (result.sent && !result.asText) {
      toast(
        'success',
        i18n.t('bookings:invoice.whatsappSent'),
        i18n.t('bookings:invoice.whatsappSentDetail', { phone: invoice.customer.phone })
      );
    } else if (result.sent) {
    ...
    toast(
      'warning',
      i18n.t('bookings:invoice.whatsappNotSent'),
      i18n.t('bookings:invoice.whatsappRetry')
    );
  }
}
