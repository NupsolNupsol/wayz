...
export function TrackingPage() {
  ...
  return (
    <div
      className="min-h-screen bg-canvas dark:bg-dk-bg px-4 py-6 sm:py-10"
      ...
      <div className="mx-auto w-full max-w-md">
        ...
        {isError && (
          <div className="lf-card p-6 text-center" data-testid="tracking-not-found">
            ...
            <h1 className="text-lg font-bold text-navy dark:text-dk-texthi">
              {t('tracking.notFound')}
            </h1>
  ...
  return (
    <div className="flex flex-col gap-4">
      <header className="text-center">
        ...
        <h1
          className="mt-1 text-2xl font-bold text-navy dark:text-dk-texthi"
          data-testid="tracking-ref"
        >
          {data.ref}
        </h1>
      ...
      <div className="lf-card p-4">
        ...
        <ul className="flex flex-col gap-2" data-testid="tracking-bags">
          {data.bags.map((b) => (
            <li
              key={b.index}
              className="flex items-center justify-between rounded-lg bg-canvas px-3 py-2 text-sm dark:bg-dk-elevated"
            >
              <span className="text-navy dark:text-dk-text">
                {t('tracking.item', { index: b.index, description: b.description })}
              </span>
...
function CountdownCard({
  ...
  if (finished) {
    return (
      <section
        className="lf-card border-success/50 bg-emerald-50 p-6 text-center dark:bg-emerald-900/20"
        data-testid="tracking-finished"
      >
        ...
        {o.penaltyAmount > 0 && (
          <p
            className="mt-2 text-sm text-navy dark:text-dk-text"
            data-testid="tracking-final-penalty"
          >
            {t('tracking.finalPenalty', {
              amount: o.penaltyAmount.toFixed(2),
              currency,
              hours: o.chargeableHours,
            })}
          </p>
        )}
      </section>
  ...
  if (o.isOvertime) {
    return (
      <section
        className="lf-card border-danger-strong/50 bg-red-50 p-6 text-center dark:bg-red-900/20"
        data-testid="tracking-overtime"
      >
        ...
        <p className="mt-1 text-sm text-muted">
          {t('tracking.pastBy', { duration: humanizeRemaining(o.overdueMs) })}
        </p>
        <div className="mt-4 rounded-xl bg-white/70 px-4 py-3 dark:bg-black/20">
          <p className="text-xs font-semibold uppercase tracking-wide text-muted">
            {t('tracking.penaltySoFar')}
          </p>
          <p
            className="mt-1 text-3xl font-bold tabular-nums text-danger-strong"
            data-testid="tracking-penalty"
          >
  ...
  if (o.withinGrace) {
    return (
      <section
        className="lf-card border-amber-400/60 bg-amber-50 p-6 text-center dark:bg-amber-900/20"
        data-testid="tracking-grace"
      >
        ...
        <p
          className="mt-3 text-4xl font-bold tabular-nums text-amber-700 dark:text-amber-300"
  ...
  return (
    <section
      className={clsx(
        'lf-card p-6 text-center',
        soon ? 'border-amber-400/60 bg-amber-50 dark:bg-amber-900/20' : ''
      )}
      data-testid="tracking-running"
    >
      <p className="text-xs font-semibold uppercase tracking-wide text-muted">
        {t('tracking.timeRemaining')}
      </p>
      <p
        className={clsx(
          'mt-2 text-5xl font-bold tabular-nums',
          soon ? 'text-amber-700 dark:text-amber-300' : 'text-success'
        )}
        data-testid="tracking-countdown"
      >
      ...
      {soon && (
        <p
          className="mt-3 text-sm font-medium text-amber-700 dark:text-amber-300"
          data-testid="tracking-ending-soon"
        >
          {t('tracking.nearlyUp')}
        </p>
      )}
