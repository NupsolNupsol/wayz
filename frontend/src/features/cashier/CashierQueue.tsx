...
import { useCashierQueue, useCashierOverview, useOpenShift, usePay } from '@/hooks'
import { ApiError } from '@/api/client'
import { toast } from '@/state/toastStore'
import { ENGINE_META } from '@/config/engineMeta'
import { money, waitedFor, STALE_QUEUE_MS } from './cashierFormat'
import type { QueuedPayment } from '@/api/cashier.api'
...
export function CashierQueue() {
  const { t } = useTranslation('cashier')
  const { data: rows, isLoading } = useCashierQueue()
  const { data: overview } = useCashierOverview()
  const pay = usePay()
  ...
  return (
    <div data-testid="cashier-queue">
      <PageHeader
        ...
        crumbs={[{ label: t('common:crumb.cashier') }, { label: t('queue.crumb') }]}
        helpId="cashier-queue"
      />
