...
import { useCashierTransactions, useRefundPayment } from '@/hooks'
import { useAuthStore } from '@/store/auth'
import { can } from '@/permissions/permissions'
import { ApiError } from '@/api/client'
import { toast } from '@/state/toastStore'
import { money } from './cashierFormat'
import type { CashierTransaction } from '@/api/cashier.api'

export function CashierTransactions() {
  const { t } = useTranslation('cashier')
  const statusLabel = useStatusLabel()
  const { data: rows = [], isLoading } = useCashierTransactions()
  const refund = useRefundPayment()
  const role = useAuthStore((s) => s.me?.role)
  const mayRefund = can(role, 'till.refund')

  const [refunding, setRefunding] = useState<CashierTransaction | null>(null)
  const [amount, setAmount] = useState('')
  const [reason, setReason] = useState('')

  const openRefund = (row: CashierTransaction) => {
    setRefunding(row)
  ...
  if (isLoading) {
    return (
      <div data-testid="cashier-transactions">
  ...
  const sum = (list: CashierTransaction[]) => list.reduce((t, r) => t + r.amount, 0)
  ...
  return (
    <div data-testid="cashier-transactions">
      <PageHeader
        ...
        crumbs={[{ label: t('common:crumb.cashier') }, { label: t('common:crumb.transactions') }]}
        helpId="cashier-transactions"
      />
      ...
      <DataTable
        testId="cashier-tx-table"
        rows={rows}
        keyOf={(r: CashierTransaction) => r._id}
        ...
        columns={[
          {
            ...
            sortValue: (r: CashierTransaction) => r._id,
            filter: { kind: 'text', value: (r: CashierTransaction) => `${r._id} ${r.bookingRef} ${r.customerName} ${r.receiptRef ?? ''}` },
            render: (r: CashierTransaction) => (
              <div>
          ...
          },
          {
            ...
            filter: {
              ...
              value: (r: CashierTransaction) => r.method,
            },
            render: (r: CashierTransaction) => (
              <Badge tone="neutral">
          ...
          },
          {
            ...
            filter: {
              ...
              value: (r: CashierTransaction) => r.kind,
            },
            render: (r: CashierTransaction) => (
          ...
          },
          {
            ...
            align: 'right',
            sortValue: (r: CashierTransaction) => r.amount,
            render: (r: CashierTransaction) => (
          ...
          },
          {
            ...
            filter: {
              ...
              value: (r: CashierTransaction) => r.status,
            },
            render: (r: CashierTransaction) => (
          ...
          },
          {
            ...
            filter: { kind: 'text', value: (r: CashierTransaction) => r.takenByName },
            sortValue: (r: CashierTransaction) => r.takenByName,
            render: (r: CashierTransaction) => (
          ...
          },
          {
            ...
            align: 'right',
            sortValue: (r: CashierTransaction) => r.createdAt,
            render: (r: CashierTransaction) => (
