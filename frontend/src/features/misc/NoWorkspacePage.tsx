  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" data-testid="no-workspace">
      <Card className="max-w-lg w-full p-8 text-center">
        ...
        <h1 className="text-xl font-bold text-navy dark:text-dk-texthi">
          {t('noWorkspace.title')}
        </h1>
        <p className="text-sm text-muted mt-2">
          You are signed in to {APP.name}
          {me?.fullName ? ` as ${me.fullName}` : ''}
          {role ? ` with the ${readable} role` : ''}, and that role does not have any screens in
          this release.
        </p>
        ...
        <div className="lf-card p-3 mt-5 flex items-start gap-3 text-start">
          ...
          <p className="text-sm text-muted">
            {t('noWorkspace.message')}
            <strong className="text-navy dark:text-dk-texthi">{t('noWorkspace.team')}</strong>. Your
            account is fine — it simply has nowhere to land.
          </p>
        </div>
