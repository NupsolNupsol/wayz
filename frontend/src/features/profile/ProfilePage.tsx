      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <Card className="lg:col-span-2">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="w-16 h-16 rounded-2xl bg-brand text-brand-fg flex items-center justify-center text-xl font-bold">
              {me.fullName
                .split(' ')
                .map((p) => p[0])
                .join('')}
            ...
            <div>
              ...
              <Badge tone="info" className="mt-1">
                {t(`common:role.${me.role}`, { defaultValue: me.role })}
              </Badge>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <Info
              icon={<IdCard size={16} />}
              label={t('profile.engines')}
              value={t('profile.enginesEnabled', { count: me.tenant?.enabledEngines.length ?? 0 })}
            />
          </div>
        </Card>
...
function Info({
  ...
  return (
    <div className="lf-card p-3 bg-canvas dark:bg-dk-elevated">
      ...
      <p className="font-semibold text-navy dark:text-dk-text text-sm" data-testid={testId}>
        {value}
      </p>
    </div>
  );
