    ...
    return (
      <div>
        <PageHeader
          ...
          title="Customer"
          crumbs={[
            { label: t('common:crumb.customers'), to: '/customers' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <Card>
          ...
          <div className="flex items-center gap-2 text-sm mb-2">
            <Phone size={15} className="text-muted" /> {customer.phone}
          </div>
          {customer.email && (
            <div className="flex items-center gap-2 text-sm mb-2">
              <Mail size={15} className="text-muted" /> {customer.email}
            </div>
          )}
          ...
          {customer.nationalId && (
            <div
              ...
              data-testid="customer-detail-national-id"
            >
          ...
          <div className="flex items-center gap-2 text-sm">
            <Calendar size={15} className="text-muted" /> Added{' '}
            {formatDateTime(new Date(customer.createdAt).getTime())}
          </div>
        </Card>
              ...
              },
              {
                ...
                render: (b) => (
                  <span className="text-muted">
                    {formatDateTime(new Date(b.createdAt).getTime())}
                  </span>
                ),
              },
            ]}
