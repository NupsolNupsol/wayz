  ...
  return (
    <div data-testid="customers-page">
      <PageHeader
        ...
        subtitle={t('common:table.records', { count: rows.length })}
        crumbs={[
          { label: t('common:crumb.home'), to: '/dashboard' },
          { label: t('common:crumb.customers') },
        ]}
        actions={
          <Button onClick={() => setOpen(true)} data-testid="customers-new">
            <UserPlus size={16} />
          ...
          columns={[
            {
              ...
              render: (c) => (
                <span className="font-semibold text-navy dark:text-dk-text">{c.name}</span>
              ),
            },
            {
              key: 'phone',
              header: t('common:column.phone'),
              filter: { kind: 'text', value: (c) => c.phone },
              render: (c) => c.phone,
            },
            {
              key: 'nationalId',
              header: t('ui:customer.nationalId'),
              filter: { kind: 'text', value: (c) => c.nationalId ?? '' },
              render: (c) => (
                <span className="text-muted font-mono text-xs">{c.nationalId || '—'}</span>
              ),
            },
            {
              key: 'email',
              header: t('common:column.email'),
              filter: { kind: 'text', value: (c) => c.email ?? '' },
              render: (c) => <span className="text-muted">{c.email || '—'}</span>,
            },
            {
              key: 'created',
              header: t('common:column.added'),
              align: 'right',
              sortValue: (c) => new Date(c.createdAt).getTime(),
              render: (c) => (
                <span className="text-muted">
                  {formatDateTime(new Date(c.createdAt).getTime())}
                </span>
              ),
            },
          ]}
      ...
      >
        ...
        >
          <PhoneInput
            ...
            testId="cust-modal-phone"
          />
        </Field>
        <Field
          label={t('ui:customer.nationalId')}
          required
          hint={t('ui:customer.nationalIdHint')}
          error={
        ...
        >
          <input
            ...
            onBlur={() => setTouched(true)}
            data-testid="cust-modal-national-id"
          />
        </Field>
        <Field
