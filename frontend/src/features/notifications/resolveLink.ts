...
export function resolveNotificationLink(
  ...
  if (link === '/refund-requests')
    return can(role, 'refund.request') || can(role, 'refund.approve') ? link : null;
  if (link === '/manual-sales')
    return can(role, 'sales.enterManual') || can(role, 'sales.approveManual') ? link : null;
  ...
  if (link.startsWith('/accounting'))
    return isAccountantRole(role) || can(role, 'tenant.administer') ? link : null;
