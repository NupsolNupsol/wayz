  ...
  return (
    <div data-testid="season-detail">
      <PageHeader
        ...
        crumbs={[
          { label: t('common:crumb.hr') },
          { label: t('common:crumb.seasons'), to: '/hr/seasons' },
          { label: season.name },
        ]}
        helpId="hr-seasons"
        actions={
          <>
            <Button
              ...
              data-testid="season-charge"
            >
              <Users size={16} />{' '}
              {missing.length
                ? t('season.chargeEmployees', { count: missing.length })
                : t('season.everyoneCharged')}
            </Button>
          </>
        }
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-5">
        <StatCard
          ...
          icon={<Wallet size={18} />}
          tone="info"
          sublabel={t('season.entriesWithVat', {
            count: season.expenseCount,
            vat: money(season.expenseVat),
          })}
        ...
        <StatCard
          ...
          sublabel={
            missing.length
              ? t('season.stillToCharge', { count: missing.length })
              : t('season.allOnSeason')
          }
      ...
      <DataTable
        ...
        columns={[
            ...
            render: (e: SeasonEmployee) => (
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  {!e.active && (
                    <p className="text-[11px] text-muted">{t('common:state.suspended')}</p>
                  )}
                </div>
              </div>
            ),
          },
          {
            ...
            render: (e: SeasonEmployee) => (
              <Badge tone="neutral">
                {t(`common:role.${e.role.toUpperCase()}`, { defaultValue: e.role })}
              </Badge>
            ),
          },
          {
            ...
            render: (e: SeasonEmployee) =>
              e.charged ? (
                <span className="tabular-nums">{e.monthly.toFixed(2)}</span>
              ) : (
        ...
        columns={[
          {
            ...
            filter: {
              kind: 'text',
              value: (c: Expense) => `${c.description} ${c.supplier} ${c.reference}`,
            },
            render: (c: Expense) => (
              <div className="max-w-[300px]">
                ...
                <p className="text-[11px] text-muted">
                  {c.supplier || '—'} · {c.reference || t('common:label.noReference')}
                </p>
              </div>
            ),
          },
          {
            ...
            filter: {
              kind: 'select',
              options: season.byCategory.map((c) => ({
          ...
          <Card className="p-4" data-testid="season-by-category">
            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-1">
              {season.byCategory.map((c) => (
                <div
                  key={c.category}
                  className="flex items-baseline justify-between gap-3 py-1 border-b border-line/60 dark:border-dk-border/60"
                >
                  <span className="text-sm text-muted truncate">{categoryLabel(c.category)}</span>
                  <span
                    className={clsx(
                      'tabular-nums text-sm font-semibold shrink-0',
                      c.category === 'PAYROLL' && 'text-brand'
                    )}
                  >
