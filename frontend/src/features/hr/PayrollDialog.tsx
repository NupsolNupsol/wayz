...
export function PayrollDialog({
  ...
  const [costs, setCosts] = useState<Record<string, string>>(
    Object.fromEntries(
      Object.entries(SUGGESTED_MONTHLY).map(([role, value]) => [role, String(value)])
    )
  );
  ...
  const total = byRole.reduce(
    (sum, [role, people]) => sum + monthlyOf(role) * months * people.length,
    0
  );
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button
            onClick={submit}
            loading={chargePayroll.isPending}
            disabled={!priced}
            data-testid="hr-payroll-submit"
          >
            {priced
              ? t('payroll.chargeCount', { count: priced, total: money(total) })
              : t('payroll.nothingToCharge')}
          </Button>
        </>
      }
    >
      {alreadyCharged > 0 && (
        <div className="flex items-start gap-2 mb-4 text-sm" data-testid="payroll-already">
          ...
          <p className="text-muted">
            <strong className="text-navy dark:text-dk-text">{alreadyCharged}</strong> already
            charged to this season — they are skipped, so running this again never doubles the cost.
          </p>
        </div>
      )}
        ...
        <>
          <div className="flex items-start gap-2 mb-4 text-sm" data-testid="payroll-pending">
            ...
            <p className="text-muted">
              ...
              {pending.length === 1 ? '' : 's'} will be charged. Set the monthly cost per role — it
              is multiplied by the {months} months of the season.
            </p>
          </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <div className="text-end w-32">
                    <p className="text-[11px] uppercase tracking-wide text-muted font-semibold">
                      {t('payroll.seasonTotal')}
                    </p>
                    <p className="tabular-nums font-semibold text-navy dark:text-dk-texthi">
                      {(monthlyOf(role) * months * people.length).toLocaleString('en-US', {
          ...
          <div className="flex items-center justify-between mt-4 pt-3 border-t-2 border-line dark:border-dk-border">
            <span className="font-semibold text-navy dark:text-dk-texthi">
              {t('payroll.addedToCostBase')}
            </span>
            <span className="text-xl font-bold tabular-nums" data-testid="payroll-total">
              {money(total)}
            </span>
          </div>
        </>
