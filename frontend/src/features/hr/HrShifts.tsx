...
export function HrShifts() {
  ...
  const save = useMutation({
    mutationFn: () =>
      hrApi.setShiftWindow({
        startsAt: startsAt || window.data!.startsAt,
        endsAt: endsAt || window.data!.endsAt,
      }),
    onSuccess: (w) => {
      ...
      toast(
        'success',
        t('shifts.windowSaved'),
        t('shifts.windowSavedDetail', { from: w.startsAt, to: w.endsAt })
      );
    },
    onError: (e) =>
      toast(
        'danger',
        t('shifts.windowFailed'),
  ...
  return (
    <div data-testid="hr-shifts">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <StatCard
          label={t('shifts.window')}
          value={w ? `${w.startsAt} → ${w.endsAt}` : '—'}
          icon={<Clock size={18} />}
          tone="info"
          testId="hr-shift-window"
        />
        <StatCard
          label={t('shifts.length')}
          value={w ? `${Math.round((w.lengthMin / 60) * 10) / 10} h` : '—'}
          tone="neutral"
          testId="hr-shift-length"
        />
        <StatCard
          label={t('shifts.peopleCounted')}
          value={hours.data?.rows.length ?? 0}
          icon={<Users size={18} />}
          tone="neutral"
          testId="hr-hours-people"
        />
        <StatCard
          label={t('shifts.hoursWorked')}
          value={`${hours.data?.totalHours ?? 0} h`}
          tone="success"
          testId="hr-hours-total"
        />
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-4 items-end">
          <Field label={t('shifts.startsAt')}>
            <input
              type="time"
              className="lf-input"
              value={startsAt || w?.startsAt || ''}
              onChange={(e) => setStartsAt(e.target.value)}
              data-testid="hr-shift-start"
            />
          </Field>
          <Field label={t('shifts.endsAt')}>
            <input
              type="time"
              className="lf-input"
              value={endsAt || w?.endsAt || ''}
              onChange={(e) => setEndsAt(e.target.value)}
              data-testid="hr-shift-end"
            />
          </Field>
          <div className="mb-4">
            <Button
      ...
      <Card className="mb-5">
        <SectionTitle className="mb-3 flex items-center gap-2">
          <Users size={18} /> {t('shifts.hoursTitle')}
        </SectionTitle>
        {hours.isLoading ? (
          <Spinner />
        ) : (
          <DataTable
            ...
            columns={[
              ...
              },
              {
                ...
                filter: { kind: 'text', value: (r) => r.role ?? '' },
                render: (r) => <span className="text-muted text-xs">{r.role ?? '—'}</span>,
              },
              {
                key: 'shifts',
                header: t('shifts.shiftsWorked'),
                align: 'right',
                sortValue: (r) => r.shifts,
                render: (r) => <span className="tabular-nums">{r.shifts}</span>,
              },
              {
                key: 'hours',
                ...
                align: 'right',
                sortValue: (r) => r.hours,
                render: (r) => <strong className="tabular-nums">{r.hours}</strong>,
              },
              {
                key: 'expected',
                ...
                render: (r) => <span className="tabular-nums text-muted">{r.expectedHours}</span>,
              },
              {
                key: 'open',
                header: t('shifts.stillOpen'),
                align: 'right',
                render: (r) =>
                  r.stillOpen ? (
                    <span className="text-amber-600 font-semibold">{r.stillOpen}</span>
                  ) : (
              ...
              {
                key: 'seen',
                header: t('shifts.lastSeen'),
                sortValue: (r) => (r.lastSeen ? new Date(r.lastSeen).getTime() : 0),
                render: (r) => (
                  <span className="text-xs text-muted tabular-nums">
                    {r.lastSeen ? formatDateTime(new Date(r.lastSeen).getTime()) : '—'}
                  </span>
                ),
              },
            ]}
      ...
      <Card>
        <SectionTitle className="mb-3 flex items-center gap-2">
          <ScrollText size={18} /> {t('shifts.auditTitle')}
        </SectionTitle>
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                key: 'who',
                header: t('common:column.agent'),
                filter: { kind: 'text', value: (r) => r.actorName },
              ...
              },
              {
                ...
                filter: {
                  kind: 'select',
                  options: (audit.data?.actions ?? []).map((a) => ({
                    label: a.replaceAll('_', ' ').toLowerCase(),
                    value: a,
                  })),
                  value: (r) => r.action,
                },
              ...
              },
              {
                ...
                render: (r) => (
                  <span className="text-xs text-muted">{r.detail ?? r.reason ?? '—'}</span>
                ),
              },
            ]}
          />
