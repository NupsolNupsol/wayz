    ...
    resolveMut.mutate(
      { id: shift._id, note: note.trim() },
      {
        onSuccess: () => {
          ...
          setNote('');
          toast('success', t('shifts.resolved'));
        },
        onError: fail,
      }
    );
    ...
    forceMut.mutate(
      { id: shift._id, countedCash: counted, reason: note.trim() },
      {
        onSuccess: (s) => {
          ...
          setNote('');
          toast(
            s.variance ? 'warning' : 'success',
            t('shifts.forceClosed'),
            t('shifts.forceClosedDetail', { amount: money(s.variance ?? 0) })
          );
        },
        onError: fail,
      }
    ...
    <div data-testid="mgr-shift-detail">
      <PageHeader
        ...
        crumbs={[
          ...
          { label: t('common:crumb.shifts'), to: '/manager/shifts' },
          { label: shift.agentName },
        ]}
        actions={
          mayAct ? (
            <>
              {shift.status === 'RECONCILING' && (
                <Button
                  ...
                  data-testid="shift-resolve-open"
                >
              ...
              {shift.status !== 'CLOSED' && (
                <Button
                  ...
                  onClick={() => {
                    setNote('');
                    setCounted(shift.expectedCash);
                    setForceOpen(true);
                  }}
                  data-testid="shift-force-open"
                >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <StatCard
          ...
          value={<StatusBadge status={shift.status} />}
          tone="neutral"
          testId="shift-detail-status"
        />
        <StatCard
          label={t('agent:shift.openingFloat')}
          value={money(shift.openingFloat ?? 0)}
          tone="neutral"
          testId="shift-detail-float"
        />
        <StatCard
          label={t('common:column.expected')}
          value={money(shift.expectedCash)}
          icon={<Wallet size={18} />}
          tone="info"
          testId="shift-detail-expected"
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                ...
                render: (r) => (
                  <span className="font-mono text-xs">{r.bookingRef || r.orderId}</span>
                ),
              },
              {
                key: 'method',
                header: t('common:column.method'),
                render: (r) => `${r.method}${r.cardScheme ? ` · ${r.cardScheme}` : ''}`,
              },
              {
                key: 'kind',
                header: t('common:column.kind'),
                render: (r) => <span className="text-muted text-xs">{r.kind}</span>,
              },
              {
                key: 'amount',
        ...
        <Card>
            ...
            <dl className="text-sm flex flex-col gap-2" data-testid="shift-drawer-breakdown">
              {(
                ...
                <div key={key} className="flex items-baseline justify-between gap-3">
                  ...
                  <dd className="tabular-nums font-medium" data-testid={`shift-drawer-${key}`}>
                    {money(value)}
                  </dd>
                </div>
              ))}
          ...
          {shift.closedByName && (
            <p
              className="text-xs text-muted mt-4 pt-3 border-t border-line"
              data-testid="shift-closed-by"
            >
        ...
        footer={
          <>
            ...
            <Button
              ...
              data-testid="shift-resolve-submit"
            >
              {t('shifts.resolve')}
            </Button>
          </>
      ...
      >
        <Field label={t('shifts.note')} required hint={t('shifts.noteHint')}>
          <input
            ...
            data-testid="shift-resolve-note"
          />
        </Field>
      </Modal>
        ...
        footer={
          <>
            ...
            <Button
              ...
              disabled={note.trim().length < 3}
              data-testid="shift-force-submit"
            >
              <Lock size={16} /> {t('shifts.forceClose')}
            </Button>
          </>
      ...
      >
        <Field
          label={t('shifts.counted')}
          required
          hint={t('shifts.countedHint', { amount: money(shift.expectedCash) })}
        >
          <NumberInput
            value={counted}
            onChange={setCounted}
