...
export function ManagerRentals() {
  ...
  return (
    <div data-testid="manager-rentals">
      <PageHeader
        title={t('records.rentals')}
        subtitle={t('records.rentalsSubtitle')}
        crumbs={[{ label: t('common:crumb.manager') }, { label: t('common:crumb.rentals') }]}
      />
          ...
          columns={[
            {
              ...
              sortValue: (r) => r.ref,
              filter: { kind: 'text', value: (r) => r.ref },
              render: (r) => <RefLink to={`/manager/rentals/${r._id}`}>{r.ref}</RefLink>,
            },
            {
              key: 'customer',
              header: t('common:column.customer'),
              filter: { kind: 'text', value: (r) => r.customerName },
              render: (r) => (
                <div>
                  <p>{r.customerName || '—'}</p>
            ...
            },
            {
              ...
              filter: {
                kind: 'select',
                options: Object.keys(ENGINE_META).map((e) => ({
                  label: engineLabel(e as EngineKind),
                  value: e,
                })),
            ...
            },
            {
              ...
              filter: { kind: 'text', value: (r) => r.stationName },
              render: (r) => <span className="text-muted">{r.stationName}</span>,
            },
            {
              key: 'agent',
              header: t('common:column.agent'),
              render: (r) => <span className="text-muted">{r.agentName}</span>,
            },
            {
              key: 'status',
              header: t('common:column.status'),
              filter: { kind: 'text', value: (r) => r.status },
            ...
            },
            {
              ...
              align: 'right',
              sortValue: (r) => r.remainingMs ?? 0,
              render: (r) =>
                r.startedAt ? (
                  <Timer expectedEndAt={r.expectedEndAt} gracePeriodMin={r.gracePeriodMin} />
                ) : (
            ...
            {
              key: 'penalty',
              header: t('common:column.penalty'),
              align: 'right',
              sortValue: (r) => r.penaltyAmount,
              render: (r) =>
                r.penaltyAmount > 0 ? (
                  <strong className="text-danger-strong">{money(r.penaltyAmount)}</strong>
                ) : (
                  <span className="text-muted">—</span>
                ),
            },
...
export function ManagerRentalDetail() {
  ...
  if (!data)
    return (
      <Card>
        <EmptyState title={t('records.rentalNotFound')} />
      </Card>
    );
  ...
  return (
    <div data-testid="manager-rental-detail">
      <PageHeader
        ...
        crumbs={[
          { label: t('common:crumb.manager') },
          { label: t('common:crumb.rentals'), to: '/manager/rentals' },
          { label: b.ref },
        ]}
      />
        ...
        <div className="lg:col-span-2 flex flex-col gap-5">
          <Card>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              ...
              <Meta
                label={t('records.penalty')}
                value={b.overtime.penaltyAmount > 0 ? money(b.overtime.penaltyAmount) : '—'}
              />
            </div>
          </Card>
          ...
          {data.order && (
            <Card>
              <SectionTitle className="mb-3 flex items-center gap-2">
                <Receipt size={18} /> Order {data.order.ref}
              </SectionTitle>
              ...
              <Badge tone={data.order.status === 'PAID' ? 'success' : 'warning'} className="mt-2">
                {data.order.status}
              </Badge>
            </Card>
          )}
            ...
            <Card>
              ...
              <ul className="flex flex-col gap-1.5">
                {b.bags.map((bag) => (
                  <li
                    key={bag.index}
                    className="flex justify-between rounded-lg bg-canvas dark:bg-dk-elevated px-3 py-2 text-sm"
                  >
                    <span>
                      Item {bag.index} · {bag.description}
                    </span>
        ...
        <div className="flex flex-col gap-5">
          <Card>
            ...
            {data.payments.length === 0 ? (
              <p className="text-sm text-muted">{t('records.noneRecorded')}</p>
            ) : (
          ...
          <Card>
            ...
            {!b.custody?.length ? (
              <p className="text-sm text-muted">{t('records.noEvents')}</p>
            ) : (
...
export function ManagerCustomers() {
  ...
  return (
    <div data-testid="manager-customers">
      <PageHeader
        title={t('records.customers')}
        subtitle={t('records.customersSubtitle')}
        crumbs={[{ label: t('common:crumb.manager') }, { label: t('common:crumb.customers') }]}
      />
      {isLoading ? (
        <Spinner />
      ) : (
          ...
          columns={[
            {
              ...
              sortValue: (r) => r.name,
              filter: { kind: 'text', value: (r) => `${r.name} ${r.phone}` },
              render: (r) => <span className="font-semibold">{r.name}</span>,
            },
            {
              key: 'phone',
              header: t('common:column.phone'),
            ...
            {
              key: 'completed',
              header: t('common:column.completed'),
              align: 'right',
              render: (r) => <span className="tabular-nums text-muted">{r.completed}</span>,
            },
            {
              key: 'last',
              header: t('common:column.lastseen'),
              align: 'right',
              sortValue: (r) => (r.lastBookingAt ? new Date(r.lastBookingAt).getTime() : 0),
              render: (r) => (
                <span className="text-muted text-xs">
                  {r.lastBookingAt ? formatDateTime(new Date(r.lastBookingAt).getTime()) : '—'}
                </span>
              ),
            },
          ]}
...
export function ManagerCustomerDetail() {
  ...
  if (!data)
    return (
      <Card>
        <EmptyState title={t('records.customerNotFound')} />
      </Card>
    );
  ...
  return (
    <div data-testid="manager-customer-detail">
      <PageHeader
        ...
        crumbs={[
          { label: t('common:crumb.manager') },
          { label: t('common:crumb.customers'), to: '/manager/customers' },
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        <Card>
          <p className="text-xs uppercase tracking-wide text-muted">{t('records.lifetimeValue')}</p>
          <p className="text-2xl font-bold text-success mt-1">{money(data.lifetimeValue)}</p>
        </Card>
        <Card>
          <p className="text-xs uppercase tracking-wide text-muted">{t('records.bookingsCount')}</p>
          <p className="text-2xl font-bold mt-1">{data.bookings.length}</p>
        </Card>
        <Card>
          <p className="text-xs uppercase tracking-wide text-muted">{t('records.since')}</p>
          <p className="text-sm font-semibold mt-2">
            {formatDateTime(new Date(data.customer.createdAt).getTime())}
          </p>
        </Card>
        <Card>
          <p className="text-xs uppercase tracking-wide text-muted">{t('records.contact')}</p>
          <p className="text-sm font-semibold mt-2 flex items-center gap-1">
            <User size={14} /> {data.customer.phone}
          </p>
        </Card>
      </div>
        ...
        <DataTable
          ...
          columns={[
            ...
            },
            {
              ...
              render: (r) => <span className="text-muted">{r.productName}</span>,
            },
            {
              key: 'status',
              header: t('common:column.status'),
            ...
            },
            {
              ...
              header: t('common:column.penalty'),
              align: 'right',
              render: (r) => (r.penaltyAmount > 0 ? money(r.penaltyAmount) : '—'),
            },
            {
              key: 'when',
...
export function ManagerPayments() {
  ...
  return (
    <div data-testid="manager-payments">
      <PageHeader
        title={t('records.payments')}
        subtitle={t('records.paymentsSubtitle')}
        crumbs={[{ label: t('common:crumb.manager') }, { label: t('common:crumb.payments') }]}
      />

      {refunds.length > 0 && (
        <Card className="mb-5 border-amber-400/60 bg-amber-50 dark:bg-amber-900/20">
          <p className="text-sm font-semibold text-amber-700 dark:text-amber-300">
            {t('records.refundsRecorded', { count: refunds.length })}
          </p>
        </Card>
      )}
      ...
      {isLoading ? (
        ...
        <DataTable
          ...
          columns={[
            {
              key: 'id',
              header: t('common:column.payment'),
              sortValue: (r) => r._id,
              filter: { kind: 'text', value: (r) => r._id },
              render: (r) => <RefText>{r._id}</RefText>,
            },
            {
              key: 'booking',
            ...
            },
            {
              ...
              filter: {
                kind: 'select',
                options: [...new Set(data.map((p) => p.method))].map((m) => ({
                  label: m,
                  value: m,
                })),
                value: (r) => r.method,
              },
              render: (r) => <Badge tone="neutral">{statusLabel(r.method, 'method')}</Badge>,
            },
            {
              key: 'kind',
              ...
              filter: {
                kind: 'select',
                options: [...new Set(data.map((p) => p.kind))].map((k) => ({
                  label: statusLabel(k, 'paymentKind'),
                  value: k,
                })),
                value: (r) => r.kind,
              },
            ...
            },
            {
              ...
              render: (r) => <strong className="tabular-nums">{money(r.amount)}</strong>,
            },
            {
              key: 'when',
              header: t('common:column.date'),
          ...
          columns={[
            {
              ...
              render: (r) => (
                <span className="text-muted text-xs">
                  {formatDateTime(new Date(r.at).getTime())}
                </span>
              ),
            },
            {
              ...
              filter: { kind: 'text', value: (r) => r.action },
              render: (r) => (
                <RefText>
            ...
            {
              key: 'entity',
              header: t('common:column.entity'),
              filter: {
                kind: 'select',
                options: [...new Set(data.map((r) => r.entity))].map((e) => ({
            ...
            },
            {
              key: 'reference',
              header: t('common:column.reference'),
              filter: { kind: 'text', value: (r) => r.entityId },
              render: (r) => <RefText>{r.entityId}</RefText>,
            },
            {
              key: 'actor',
              header: t('common:column.by'),
              filter: { kind: 'text', value: (r) => r.actorId },
              render: (r) => <span className="text-muted">{r.actorId}</span>,
            },
            {
              key: 'detail',
              header: t('common:column.detail'),
              render: (r) => <span className="text-muted line-clamp-1">{r.detail ?? '—'}</span>,
            },
            {
              key: 'reason',
              ...
              render: (r) =>
                r.reason ? (
                  <span className="text-amber-700 dark:text-amber-300 line-clamp-1">
                    {r.reason}
                  </span>
                ) : (
                  <span className="text-muted">—</span>
