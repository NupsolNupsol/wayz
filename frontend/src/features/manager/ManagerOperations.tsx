...
import {
  useManagerIncidents,
  useManagerLiveSessions,
  useManagerShifts,
  useManagerUpdateIncident,
} from '@/hooks';
...
export function ManagerLive() {
  ...
  return (
    <div data-testid="manager-live">
      <PageHeader
        title={t('operations.liveSessions')}
        subtitle={t('operations.liveSubtitle')}
        crumbs={[{ label: t('common:crumb.manager') }, { label: t('common:crumb.livesessions') }]}
      />
      {isLoading ? (
        <Spinner />
      ) : (
        <DataTable
          ...
          keyOf={(r) => r._id}
          empty={{
            title: t('operations.nothingRunning'),
            message: t('operations.nothingRunningHint'),
          }}
          columns={[
            {
              key: 'ref',
...
export function ManagerIncidents() {
  ...
  const advance = (id: string, status: string) => {
    ...
    update.mutate(
      { id, status: next },
      { onSuccess: () => toast('info', t('operations.incidentUpdated'), next) }
    );
  };
  ...
  return (
    <div data-testid="manager-incidents">
      <PageHeader
        title={t('operations.incidents')}
        subtitle={t('operations.incidentsSubtitle')}
        crumbs={[{ label: t('common:crumb.manager') }, { label: t('common:crumb.incidents') }]}
      />
      {isLoading ? (
        <Spinner />
      ) : (
          ...
          columns={[
            {
              ...
              sortValue: (r) => r.ref,
              filter: { kind: 'text', value: (r) => r.ref },
              render: (r) => <RefText>{r.ref}</RefText>,
            },
            {
              key: 'type',
              header: t('common:column.type'),
              filter: { kind: 'text', value: (r) => r.type },
              render: (r) => r.type.replaceAll('_', ' '),
            },
            {
              key: 'service',
              header: t('common:column.service'),
              render: (r) => (r.engineKind ? engineLabel(r.engineKind) : '—'),
            },
            {
              key: 'station',
              header: t('common:column.station'),
            ...
            {
              key: 'desc',
              header: t('common:column.description'),
            ...
            {
              key: 'date',
              header: t('common:column.reported'),
              align: 'right',
              ...
              render: (r) =>
                NEXT[r.status] ? (
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      advance(r._id, r.status);
                    }}
                  >
                    → {NEXT[r.status]?.replaceAll('_', ' ')}
                  </Button>
                ) : null,
            },
          ]}
...
export function ManagerShifts() {
  ...
  return (
    <div data-testid="manager-shifts">
      <PageHeader
        title={t('operations.shifts')}
        subtitle={t('operations.shiftsSubtitle')}
        crumbs={[{ label: t('common:crumb.manager') }, { label: t('common:crumb.shifts') }]}
      />

      {reconciling.length > 0 && (
        <Card
          className="mb-5 border-amber-400/60 bg-amber-50 dark:bg-amber-900/20"
          data-testid="mgr-shifts-reconciling"
        >
      ...
      {stillOpen.length > 0 && (
        <Card className="mb-5" data-testid="mgr-shifts-open">
          <p className="text-sm font-semibold text-navy dark:text-dk-texthi">
            {t('operations.stillOpen', { count: stillOpen.length })}
          </p>
          ...
          columns={[
            {
              ...
              filter: { kind: 'text', value: (r) => r.agentName },
            ...
            {
              key: 'opened',
              header: t('common:column.opened'),
              sortValue: (r) => new Date(r.openedAt).getTime(),
              render: (r) => formatDateTime(new Date(r.openedAt).getTime()),
            },
            {
              key: 'expected',
              header: t('common:column.expected'),
              align: 'right',
              sortValue: (r) => r.expectedCash,
              render: (r) => money(r.expectedCash),
            },
            {
              key: 'counted',
              header: t('common:column.counted'),
              align: 'right',
              render: (r) =>
                r.countedCash != null ? (
                  money(r.countedCash)
            ...
            {
              key: 'variance',
              header: t('common:column.variance'),
              align: 'right',
              ...
              render: (r) =>
                r.variance == null ? (
                  ...
                  <strong className="text-danger-strong">{money(r.variance)}</strong>
                ),
            },
            {
              key: 'status',
              ...
              filter: {
                kind: 'select',
                options: ['OPEN', 'RECONCILING', 'CLOSED'].map((s) => ({ label: s, value: s })),
                value: (r) => r.status,
              },
              render: (r) => <StatusBadge status={r.status} />,
            },
