  ...
  return (
    <div data-testid="captain-history">
      <PageHeader
        ...
        crumbs={[
          { label: t('trips.captainTitle'), to: '/lagoon/captain' },
      ...
      {isLoading ? (
        ...
        <Card className="p-6">
          <EmptyState
            icon={<Anchor size={26} />}
            title={t('trips.noneFinished')}
            message={t('trips.noneFinishedHint')}
          />
        </Card>
      ) : (
            ...
            <Card key={trip._id} className="p-4" data-testid={`captain-done-${trip._id}`}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-muted flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                    ...
                    <span className="flex items-center gap-1.5">
                      <Ship size={14} />{' '}
                      {formatDateTime(trip.startedAt ? new Date(trip.startedAt).getTime() : null)}
                    </span>
                  </p>
                </div>
              ...
              {trip.stops.length > 0 && (
                <ol
                  ...
                  data-testid={`captain-done-stops-${trip._id}`}
                >
