...
export function IncidentsPage() {
  ...
  const submit = () => {
    createMut.mutate(
      ...
      {
        onSuccess: () => {
          toast('warning', t('incidents.logged'));
          setOpen(false);
          setDesc('');
        },
      }
  ...
  const advance = (id: string, status: string) => {
    ...
    if (next)
      updateMut.mutate(
        { id, status: next },
        { onSuccess: () => toast('info', t('incidents.updated'), next) }
      );
  };
    ...
    <div data-testid="incidents-page">
      <PageHeader
        ...
        crumbs={[
          ...
          { label: t('common:crumb.incidents') },
        ]}
        helpId="incidents"
        actions={
          <Button variant="danger" onClick={openModal} data-testid="incident-new">
      ...
      {isLoading ? (
        ...
        <DataTable
          ...
          keyOf={(i) => i._id}
          empty={{
            title: t('manager:operations.noIncidents'),
            message: t('manager:operations.noIncidentsHint'),
          }}
          columns={[
            {
              key: 'ref',
              ...
              sortValue: (i) => i.ref,
              filter: { kind: 'text', value: (i) => i.ref },
              render: (i) => <RefText>{i.ref}</RefText>,
            },
            {
              key: 'type',
              ...
              filter: {
                kind: 'select',
                options: allTypes.map((t) => ({ label: labelOf(t), value: t })),
                value: (i) => i.type,
              },
              render: (i) => labelOf(i.type),
            },
            {
              key: 'engine',
              ...
              filter: {
                ...
                value: (i) => i.engineKind ?? '',
              },
              render: (i) => (
                <span className="text-muted">{i.engineKind ? engineLabel(i.engineKind) : '—'}</span>
              ),
            },
            {
              key: 'desc',
              header: t('common:column.description'),
              filter: { kind: 'text', value: (i) => i.description },
              render: (i) => <span className="text-muted line-clamp-1">{i.description}</span>,
            },
            {
              key: 'booking',
            ...
            },
            {
              ...
              render: (i) => (
                <span className="text-muted">
                  {formatDateTime(new Date(i.createdAt).getTime())}
                </span>
              ),
            },
            {
              ...
              render: (i) =>
                NEXT[i.status] ? (
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      advance(i._id, i.status);
                    }}
                  >
                    → {NEXT[i.status]?.replaceAll('_', ' ')}
                  </Button>
                ) : null,
            },
          ]}
      ...
      <Modal
        ...
        title={t('incidents.log')}
        testId="incident-log-modal"
        footer={
          <>
            ...
            <Button
              ...
              disabled={!desc.trim()}
              data-testid="incident-log-submit"
            >
              Log
            </Button>
          </>
      ...
      >
        ...
        <Field label={t('common:field.type')}>
          <Select
            ...
            options={engineTypes.map((t) => ({ label: labelOf(t), value: t }))}
            testId="incident-type-select"
          />
        </Field>
