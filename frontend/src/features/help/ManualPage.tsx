      ...
      .filter((s) =>
        !term
          ...
          : [
              s.title,
              s.summary,
              ...s.steps.flatMap((x) => [x.title, x.detail]),
              ...s.rules,
              ...s.tips,
            ]
              .join(' ')
    ...
    <div data-testid="manual-page">
      <PageHeader
        ...
        crumbs={[
          ...
          { label: t('common:crumb.help') },
          { label: t('common:crumb.usermanual') },
        ]}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-5 items-start">
        <Card className="order-first lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] flex flex-col !p-4">
          ...
          <nav
            className="flex-1 overflow-y-auto scroll-thin -mx-1 px-1 mt-3"
          ...
          >
              ...
              <div key={g.id} className={clsx(gi > 0 && 'mt-4 pt-4 border-t border-line')}>
                ...
                {g.sections.map((s) => {
                  ...
                  return (
                    ...
                    >
                      {isActive && (
                        <span className="absolute start-0 top-2 bottom-2 w-[3px] rounded-e bg-brand" />
                      )}
                      {s.title}
                    </a>
                  );
            ...
            {groups.length === 0 && (
              <p className="text-sm text-muted px-2">{t('page.noMatch', { query })}</p>
            )}
          </nav>
        </Card>
  ...
  return (
    <Card
      ...
      className={clsx(
        ...
        highlighted && 'ring-2 ring-brand shadow-pop'
      )}
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        ...
        {section.route && (
          <Link
            to={section.route}
            className="lf-btn-secondary !h-8 !px-2.5 text-xs shrink-0 no-underline"
          >
        ...
        <div className="mt-4 rounded-xl border border-amber-300/70 bg-amber-50 dark:bg-amber-900/20 p-3">
          ...
          <ul className="mt-1.5 flex flex-col gap-1">
            {section.rules.map((r, i) => (
              <li key={i} className="text-sm text-navy dark:text-dk-text">
                • {r}
              </li>
            ))}
          </ul>
