...
const defaultBag = (i: number): BagRow => ({
  description: `Bag ${i}`,
  category: 'SOFT',
  w: 30,
  h: 25,
...
export function ShopDropPage() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      draftBooking &&
      draftOrder &&
      !isUnfinishedSale(draftBooking, draftOrder)
    ) {
      writeDraft(DRAFT_KEY, null);
      setRestoring(false);
      return;
  ...
  const reservedUnit = booking?.reservation
    ? units.find((u) => u._id === booking.reservation!.assetUnitId)
    : undefined;

  const patchBag = (i: number, patch: Partial<BagRow>) =>
    setBags((prev) => prev.map((x, idx) => (idx === i ? { ...x, ...patch } : x)));
  ...
  const compartments = useMemo(
    () =>
      (units ?? []).filter(
        (u) => u.assetKind === 'COMPARTMENT' || u.engineKind === 'SHOP_AND_DROP'
      ),
    [units]
  );
  const freeCompartments = useMemo(
    () => compartments.filter((u) => u.status === 'AVAILABLE'),
    [compartments]
  );
  ...
  const sizeOptions = useMemo(
    () =>
      [...new Set(compartments.map((u) => u.assetTypeName))]
        .sort()
  ...
  const statusOptions = useMemo(
    () =>
      [...new Set(compartments.map((u) => u.status))]
        .sort()
  ...
  const chosenUnit = useMemo(
    () => compartments.find((u) => u._id === unitId) ?? null,
    [compartments, unitId]
  );
  ...
  const compartmentColumns: Column<AssetUnit>[] = [
    {
      ...
      render: (u) => (
        <div className="flex items-center gap-2">
          ...
          />
          <span className="font-mono font-semibold text-navy dark:text-dk-texthi">
            {u.identifier}
          </span>
        </div>
      ),
      ...
      render: (u) => (
        <span
          className={clsx(
            ...
            (u.maxBags ?? 0) < bags.length && 'text-warn font-semibold'
          )}
        >
    ...
    },
    {
      ...
      render: (u) => (
        <span className="text-sm tabular-nums">{u.price == null ? '—' : money(u.price)}</span>
      ),
    },
    {
      key: 'status',
  ...
  const holdAndDraft = async () => {
    ...
    try {
      ...
      toast(
        'success',
        t('shopdrop.capacityHeld'),
        `${res.booking.packingPlan?.numberOfCompartmentsRequired ?? 1} compartment(s) held before payment.`
      );
      setStep(3);
    } catch (e) {
  ...
  const pay = async (splits: PaymentSplit[]) => {
    ...
    try {
      ...
      toast('danger', t('shopdrop.paymentFailed'), e instanceof ApiError ? e.message : '');
    }
  };
  ...
  const reserve = async () => {
    ...
    try {
      ...
      toast(
        'success',
        t('shopdrop.unitReserved'),
        `${units.find((u) => u._id === b.reservation?.assetUnitId)?.identifier ?? ''} locked (Agent Lease).`
      );
    } catch (e) {
      toast('danger', t('shopdrop.reserveFailed'), e instanceof ApiError ? e.message : '');
    }
  };
  ...
  const requestStorageRun = async () => {
    ...
    try {
      ...
      toast(
        'success',
        t('shopdrop.storageRunRaised'),
        t('shopdrop.storageRunBlurb', { gate: reservedUnit?.gateName ?? '' })
      );
      setStep(5);
    } catch (e) {
      toast(
        'danger',
        t('shopdrop.storageRunFailed'),
  ...
  return (
    <div data-testid="shopdrop-page">
      <PageHeader
        helpId="shop-drop"
        title={t('common:engine.SHOP_AND_DROP')}
        subtitle={t('shopdrop.subtitle')}
        crumbs={[
          { label: t('common:crumb.home'), to: '/dashboard' },
          { label: t('common:crumb.shopdrop') },
        ]}
      />
      <div className="mb-5">
      ...
      {resumed && step < 4 && (
        <Card
          ...
          data-testid="sd-resumed"
        >
          <p className="text-sm text-navy dark:text-dk-text">{t('shopdrop.resumed')}</p>
          <Button variant="ghost" onClick={reset} data-testid="sd-resumed-discard">
            {t('shopdrop.startFresh')}
          </Button>
        </Card>
      )}
      ...
      {step === 0 && !restoring && (
        <Card data-testid="shopdrop-wizard">
          ...
          <div className="flex justify-end mt-4">
            <Button onClick={() => setStep(1)} disabled={!customer} data-testid="sd-next-customer">
              {t('common:action.continue')} <ArrowRight size={15} />
            </Button>
          </div>
        </Card>
      )}
      ...
      {step === 1 && (
        <Card>
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => setBags((b) => (b.length > 1 ? b.slice(0, -1) : b))}
                className="lf-btn-secondary !h-9 !px-3"
                data-testid="sd-bag-minus"
              >
                <Minus size={15} />
              </button>
              <span className="font-bold text-lg w-8 text-center" data-testid="sd-bag-count">
                {bags.length}
              </span>
              <button
                onClick={() => setBags((b) => [...b, defaultBag(b.length + 1)])}
                className="lf-btn-secondary !h-9 !px-3"
                data-testid="sd-bag-plus"
              >
                <Plus size={15} />
              </button>
            </div>
          ...
          <div className="hidden md:grid grid-cols-[1.4fr_1fr_1.6fr_0.8fr] gap-2 px-1 mb-1 text-[11px] font-bold uppercase tracking-wide text-muted">
            ...
            <span>{t('shopdrop.sizeWhd')}</span>
            <span>{t('shopdrop.weightKg')}</span>
          </div>
          <div className="flex flex-col gap-2">
            {bags.map((b, i) => (
              <div
                key={i}
                className="grid grid-cols-2 md:grid-cols-[1.4fr_1fr_1.6fr_0.8fr] gap-2 items-center"
                data-testid={`sd-bag-row-${i + 1}`}
              >
                <input
                  className="lf-input !h-10"
                  value={b.description}
                  onChange={(e) => patchBag(i, { description: e.target.value })}
                  data-testid={`sd-bag-desc-${i + 1}`}
                  placeholder={t('shopdrop.bagPlaceholder', { index: i + 1 })}
                />
                <Select
                  value={b.category}
                  onChange={(v) => patchBag(i, { category: v as BagCategory })}
                  size="sm"
                  testId={`sd-bag-type-${i + 1}`}
                  options={[
                    { label: t('common:label.soft'), value: 'SOFT' },
                    { label: t('common:label.hard'), value: 'HARD' },
                    { label: t('common:label.oversize'), value: 'OVERSIZE' },
                    { label: t('common:label.fragile'), value: 'FRAGILE' },
                  ]}
                />
                <div className="flex items-center gap-1">
                  {(['w', 'h', 'd'] as const).map((dim) => (
                    <NumberInput
                      ...
                      value={b[dim]}
                      onChange={(v) => patchBag(i, { [dim]: v } as Partial<BagRow>)}
                      testId={`sd-bag-${dim}-${i + 1}`}
                      ariaLabel={t('shopdrop.bagDimension', { index: i + 1, dimension: dim })}
                    />
                  ))}
                </div>
                <NumberInput
                  ...
                  className="!h-10"
                  value={b.weight}
                  onChange={(v) => patchBag(i, { weight: v })}
                  testId={`sd-bag-weight-${i + 1}`}
                />
              </div>
            ))}
          </div>
          <div className="flex justify-between mt-4">
            <Button variant="ghost" onClick={() => goToStep(0)} data-testid="sd-back-customer">
              <ArrowLeft size={15} />
              {t('shopdrop.changeCustomer')}
            </Button>
            <Button onClick={goToPlan} data-testid="sd-next-bags">
              {t('shopdrop.suggestCompartment')}
              <ArrowRight size={15} />
            </Button>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <Card className="lg:col-span-2">
            ...
            <p className="text-sm text-muted mb-3">
              {t('shopdrop.pickCompartmentBlurb', { count: bags.length })}
            </p>
            ...
            <DataTable<AssetUnit>
              ...
              empty={{
                title: t('shopdrop.noCompartments'),
                message: t('shopdrop.noCompartmentsHint'),
              }}
            ...
            <div className="mt-4">
              <Field label={t('shopdrop.storageDuration')}>
                <Counter
                  ...
                  testId="sd-duration"
                  ariaLabel={t('shopdrop.storageDuration')}
                />
              </Field>
            </div>
            <div className="flex justify-between mt-2">
              <Button variant="ghost" onClick={() => goToStep(1)} data-testid="sd-back-bags">
                <ArrowLeft size={15} />
                {t('shopdrop.changeBags')}
              </Button>
              <Button
                onClick={holdAndDraft}
                loading={createMut.isPending}
                disabled={!online || !unitId}
                data-testid="sd-hold"
              >
                {t('shopdrop.checkAvailability')}
                <ArrowRight size={15} />
              </Button>
            </div>
          </Card>
          <Card>
            <SectionTitle className="mb-3 flex items-center gap-2">
              <Boxes size={18} /> {t('shopdrop.chosenTitle')}
            </SectionTitle>
            {chosenUnit ? (
              <div className="lf-card p-3 bg-canvas dark:bg-dk-elevated" data-testid="sd-chosen">
                <p className="text-sm">{t('shopdrop.chosen')}</p>
                <p
                  className="font-mono text-2xl font-bold text-navy dark:text-dk-texthi mt-1"
                  data-testid="sd-chosen-unit"
                >
                ...
                {chosenUnit.maxBags != null && (
                  <p
                    className={clsx(
                      'text-xs mt-2',
                      chosenUnit.maxBags < bags.length ? 'text-warn font-semibold' : 'text-muted'
                    )}
                  >
                ...
                <p className="text-sm font-semibold mt-2">
                  {chosenUnit.price == null ? '—' : money(chosenUnit.price)}
                </p>
              </div>
            ) : (
      ...
      {step === 3 && booking && order && customer && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <Card>
            ...
            <PaymentPanel
              total={order.total}
              discountOff={quoteDiscount}
              onConfirm={pay}
              confirming={payMut.isPending}
              disabled={!online || !phoneVerified}
            />
            {!phoneVerified && (
              <p className="text-xs text-amber-600 mt-2">{t('shopdrop.verifyPhoneFirst')}</p>
            )}
          </Card>
          <Card>
            <SectionTitle className="mb-3 flex items-center gap-2">
              <Boxes size={18} />
              {t('shopdrop.packingPlanQuote')}
            </SectionTitle>
            <div className="lf-card p-3 bg-canvas dark:bg-dk-elevated mb-3">
              <p className="text-sm">
                <strong data-testid="sd-compartments">
                  {booking.packingPlan?.numberOfCompartmentsRequired ?? 1}
                </strong>{' '}
                compartment(s) for {booking.bags.length} bag(s)
              </p>
                ...
                return (
                  <div
                    ...
                    className={clsx(
                      ...
                      off ? 'text-success font-medium' : 'text-muted'
                    )}
      ...
      {step === 4 && booking && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <Card>
            ...
            {!reservedUnit ? (
              <Button onClick={reserve} loading={reserveMut.isPending} data-testid="sd-reserve">
                {t('shopdrop.reserveRecommended')}
              </Button>
            ) : (
              <div className="lf-card p-3 flex items-center gap-2" data-testid="sd-reserved-unit">
                <PackageCheck className="text-success" size={18} />
                <div>
                  <p className="font-semibold">{reservedUnit.identifier}</p>
            ...
            {reservedUnit && (
              <>
                <SectionTitle className="mb-3 mt-6">2 · Labels</SectionTitle>
                <Button
                  variant="secondary"
                  onClick={() => setLabelsOpen(true)}
                  data-testid="sd-labels"
                >
                  <Printer size={15} /> Print bag labels ({booking.bags.length})
                </Button>
              </>
            )}
          </Card>
                  ...
                  <ul className="text-sm" data-testid="sd-run-bags">
                    {booking.bags.map((b) => (
                      <li key={b.index} className="flex justify-between py-0.5">
                        <span>
                          {t('shopdrop.bagLine', { index: b.index, description: b.description })}
                        </span>
      ...
      {step === 5 && booking && (
        <Card className="text-center py-8" data-testid="sd-done">
          ...
          <h2 className="text-xl font-bold text-navy dark:text-dk-texthi">
            {t('shopdrop.storageActive')}
          </h2>
          <p className="text-muted mt-1">
            {booking.ref} · {reservedUnit?.identifier}
          </p>
          <div className="mt-3 flex items-center justify-center gap-2">
            <span className="text-muted text-sm">{t('shopdrop.remaining')}</span>{' '}
            <Timer
          ...
          {deliveryId ? (
            <div
              className="mt-5 lf-card p-3 mx-auto max-w-sm border-brand/40 bg-brand/5"
              data-testid="sd-delivery-created"
            >
            ...
            <div
              className="mt-5 lf-card p-4 mx-auto max-w-md text-start"
              data-testid="sd-delivery-offer"
            >
              <p className="text-sm font-semibold text-navy dark:text-dk-texthi flex items-center gap-2">
                ...
                {t('shopdrop.wantBagsBrought')}
              </p>
              <p className="text-xs text-muted mt-1 mb-3">{t('shopdrop.askNow')}</p>
              <Button
                variant="secondary"
                onClick={() => setDeliveryOpen(true)}
                data-testid="sd-request-delivery"
              >
                {t('shopdrop.arrangeDelivery')}
              </Button>
            </div>
          )}
          ...
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            {order && (
              <Button
                ...
                data-testid="sd-invoice-slip"
              >
                <ReceiptText size={15} />
                {t('bookings:invoice.title')}
              </Button>
            )}
            <Button
              variant="secondary"
              onClick={() => navigate(`/bookings/${booking.id}`)}
              data-testid="sd-view-booking"
            >
              {t('shopdrop.viewBooking')}
            </Button>
            <Button onClick={reset} data-testid="sd-new-another">
              {t('shopdrop.newTransaction')}
            </Button>
          </div>
          <p className="mt-4 text-xs text-muted">
            {t('shopdrop.trackingLink')}{' '}
            <span className="font-mono">{trackingUrl(booking.trackingToken)}</span>
          </p>
        </Card>
      )}
      ...
      <Modal
        ...
        title={t('shopdrop.bagLabels')}
        subtitle={t('shopdrop.oneBarcode')}
        size="md"
        footer={
          <>
