  ...
  return (
    <div data-testid="admin-overview">
      <PageHeader
        ...
        actions={
          <Button
            ...
            data-testid="admin-go-company"
          >
            <Building2 size={16} />
            {t('overview.companyBranding')}
          </Button>
        }
      />
      ...
      {(o.overdue > 0 || o.openIncidents > 0 || o.reconciling > 0) && (
        <Card
          className="mb-6 p-4 border-amber-300 bg-amber-50 dark:bg-amber-900/20"
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm">
              {o.overdue > 0 && (
                <button
                  className="text-navy dark:text-dk-text hover:underline"
              ...
              {o.reconciling > 0 && (
                ...
                >
                  <strong>{o.reconciling}</strong> till{o.reconciling === 1 ? '' : 's'} awaiting
                  sign-off
                </button>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start mb-6">
        <Card className="p-4" data-testid="admin-people-breakdown">
          <SectionTitle className="mb-3 flex items-center gap-2">
            <Users size={16} />
            {t('overview.whoWorksHere')}
          </SectionTitle>
          {(Object.keys(ROLE_LABEL) as Role[]).map((role) => (
            <div
              key={role}
              className="flex items-center justify-between py-1.5 border-b border-line dark:border-dk-border last:border-0"
            >
              <span className="text-sm text-navy dark:text-dk-text">
                {t(`common:role.${role}`, { defaultValue: ROLE_LABEL[role] })}
              </span>
          ...
          <Button
            ...
            className="mt-4"
            onClick={() => navigate('/manager/team')}
            data-testid="admin-go-people"
          >
            {t('overview.everyEmployee')}
          </Button>
        </Card>
        ...
        <Card className="p-4" data-testid="admin-money">
          ...
          <p className="text-[11px] text-muted mt-3">
            VAT {(data.tenant.vatRate * 100).toFixed(0)}% · {data.tenant.currency}
          </p>
        </Card>
      </div>
      ...
      <DataTable
        ...
        columns={[
          ...
          {
            key: 'stations',
            header: t('common:column.stations'),
            align: 'right',
            sortValue: (r: SiteRow) => r.stations,
            render: (r: SiteRow) => <span className="tabular-nums">{r.stations}</span>,
          },
          {
            key: 'kiosks',
            header: t('common:column.kiosks'),
            align: 'right',
            sortValue: (r: SiteRow) => r.kiosks,
            render: (r: SiteRow) => <span className="tabular-nums">{r.kiosks}</span>,
          },
          {
            key: 'units',
          ...
          {
            key: 'staff',
            header: t('common:column.people'),
            align: 'right',
            sortValue: (r: SiteRow) => r.staff,
            render: (r: SiteRow) => <span className="tabular-nums">{r.staff}</span>,
          },
          {
            key: 'live',
            header: t('common:column.live'),
            align: 'right',
            sortValue: (r: SiteRow) => r.live,
            render: (r: SiteRow) => <span className="tabular-nums">{r.live}</span>,
          },
          {
            key: 'revenue',
          ...
          },
          {
            ...
            render: (r: SiteRow) => (
              <Badge tone={r.active ? 'success' : 'neutral'}>
                {r.active ? t('common:state.active') : t('common:state.closed')}
              </Badge>
            ),
          },
        ]}
