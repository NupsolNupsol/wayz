            ...
            <button
              ...
              className={clsx(
                ...
                siteId === site._id
                  ...
                  : 'bg-surface border border-line text-muted hover:text-brand'
              )}
            >
              {site.name}
            </button>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_300px] gap-5 items-start">
        ...
        <div className="flex flex-col gap-4">
          <Card className="p-4">
            ...
            {tray.length === 0 ? (
              <p className="text-sm text-muted" data-testid="map-tray-empty">
                {t('map.allPlaced')}
              </p>
            ) : (
                ...
                {tray.map((point) => (
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      <span className="block text-sm font-semibold text-navy dark:text-dk-texthi truncate">
                        {point.name}
                      </span>
          ...
          <Card className="p-4">
            <SectionTitle className="mb-3">
              {t('map.placed', { count: placed.length })}
            </SectionTitle>
            {placed.length === 0 ? (
                ...
                {placed.map((point) => (
                  <div
                    ...
                    className={clsx(
                      ...
                      selected === point._id
                        ? 'bg-brand/10'
                        : 'hover:bg-canvas dark:hover:bg-dk-elevated'
                    )}
                  >
                    ...
                    <span className="text-[11px] text-muted tabular-nums">
                      {Math.round((point.mapX as number) * 100)},
                      {Math.round((point.mapY as number) * 100)}
                    </span>
                    <button
                      type="button"
