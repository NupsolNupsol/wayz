...
export const DELIVERY_META: Record<DeliveryStatus, DeliveryStatusMeta> = {
  REQUESTED: {
    labelKey: 'delivery:state.REQUESTED.label',
    hintKey: 'delivery:state.REQUESTED.hint',
    tone: 'warning',
    icon: 'Bell',
    step: 0,
  },
  ASSIGNED: {
    labelKey: 'delivery:state.ASSIGNED.label',
    hintKey: 'delivery:state.ASSIGNED.hint',
    tone: 'info',
    icon: 'MapPin',
    step: 1,
  },
  RELEASE_REQUESTED: {
    labelKey: 'delivery:state.RELEASE_REQUESTED.label',
    hintKey: 'delivery:state.RELEASE_REQUESTED.hint',
    tone: 'warning',
    icon: 'Clock',
    step: 2,
  },
  RELEASE_APPROVED: {
    labelKey: 'delivery:state.RELEASE_APPROVED.label',
    hintKey: 'delivery:state.RELEASE_APPROVED.hint',
    tone: 'info',
    icon: 'PackageOpen',
    step: 3,
  },
  PICKED_UP: {
    labelKey: 'delivery:state.PICKED_UP.label',
    hintKey: 'delivery:state.PICKED_UP.hint',
    tone: 'info',
    icon: 'Truck',
    step: 4,
  },
  DELIVERED: {
    labelKey: 'delivery:state.DELIVERED.label',
    hintKey: 'delivery:state.DELIVERED.hint',
    tone: 'success',
    icon: 'PackageCheck',
    step: 5,
  },
  CANCELLED: {
    labelKey: 'delivery:state.CANCELLED.label',
    hintKey: 'delivery:state.CANCELLED.hint',
    tone: 'neutral',
    icon: 'Circle',
    step: -1,
  },
  FAILED: {
    labelKey: 'delivery:state.FAILED.label',
    hintKey: 'delivery:state.FAILED.hint',
    tone: 'danger',
    icon: 'TriangleAlert',
    step: -1,
  },
...
export const meta = (status: DeliveryStatus): DeliveryStatusMeta =>
  DELIVERY_META[status] ?? DELIVERY_META.REQUESTED;
...
export function relativeTime(iso: string | null | undefined, now = Date.now()): string {
  ...
  if (mins < 60)
    return diff < 0
      ? i18n.t('common:time.minsAgo', { count: mins })
      : i18n.t('common:time.inMins', { count: mins });
  const hours = Math.round(mins / 60);
  if (hours < 24)
    return diff < 0
      ? i18n.t('common:time.hoursAgo', { count: hours })
      : i18n.t('common:time.inHours', { count: hours });
