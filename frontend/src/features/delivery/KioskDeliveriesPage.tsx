    ...
    run.mutate(
      {
        ...
        payload: { confirmCourierId: data.courier._id, handedOver: true },
      },
      {
        onSuccess: () => {
          toast(
            'success',
            t('kiosk.released'),
            `${data.courier?.fullName} can now collect the bags.`
          );
          close();
        },
        onError: (e) =>
  ...
  return (
    ...
    >
      {isLoading || !data ? (
        ...
        <>
          <div
            className="lf-card p-4 mb-4 border-brand/40 bg-brand/5"
            data-testid="delivery-approve-courier"
          >
            ...
            {data.courier ? (
              <>
                <p className="text-xl font-bold text-navy dark:text-dk-texthi">
          ...
          <div className="lf-card p-3 mb-4 flex items-start gap-3 border-amber-300 bg-amber-50 dark:bg-amber-900/20">
            ...
            <p className="text-xs text-navy dark:text-dk-text">
              Approving hands a stranger someone else’s property. If the person at your desk is not
              the courier named above, close this and tell your supervisor.
            </p>
          </div>
          ...
          <p className="text-sm text-muted mb-3">
            {t('kiosk.handOverHint', { unit: data.delivery.assetUnitIdentifier ?? '' })}
          </p>

          <div className="text-sm text-muted">
            <p className="font-semibold text-navy dark:text-dk-texthi">
              {data.delivery.customerName}
            </p>
            <p>
              {data.bags.length} bag(s) · booking {data.delivery.bookingId}
            </p>
  ...
  return (
    <div data-testid="kiosk-deliveries">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-5">
        <StatCard
          label={t('kiosk.atYourDesk')}
          value={waiting.length}
          icon={<BellRing size={18} />}
          tone={waiting.length ? 'warning' : 'neutral'}
          testId="kiosk-stat-waiting"
        />
        <StatCard
          label={t('kiosk.openJobs')}
          value={open.length}
          icon={<Truck size={18} />}
          tone="info"
          testId="kiosk-stat-open"
        />
        <StatCard
          label={t('kiosk.inTransit')}
          value={inTransit.length}
          icon={<MapPin size={18} />}
          tone="info"
          testId="kiosk-stat-transit"
        />
      </div>
      ...
      {waiting.length > 0 && (
        <>
          <SectionTitle className="mb-2 flex items-center gap-2">
            ...
            {t('kiosk.waitingAtDesk')}
          </SectionTitle>
          <div className="flex flex-col gap-3 mb-6" data-testid="kiosk-waiting">
            {waiting.map((d) => (
              <Card
                key={d._id}
                className="p-4 border-amber-300 bg-amber-50/60 dark:bg-amber-900/20"
                data-testid={`kiosk-waiting-${d._id}`}
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {(d.stops?.length ?? 0) > 1 && (
                      <p
                        ...
                        data-testid={`kiosk-stop-note-${d._id}`}
                      >
                        {t('kiosk.stopOf', {
                  ...
                  <Button
                    onClick={() => setApproving(d._id)}
                    data-testid={`kiosk-approve-${d._id}`}
                  >
                    <UserCheck size={16} />
                    {t('kiosk.checkCourier')}
                  </Button>
                </div>
              </Card>
            ))}
          ...
          columns={[
            ...
            },
            {
              ...
              filter: {
                kind: 'text',
                value: (r: Delivery) =>
                  `${r.activeStop?.kioskName ?? ''} ${r.assetUnitIdentifier ?? ''}`,
              },
              render: (r: Delivery) => (
              ...
              render: (r: Delivery) => (
                <div className="text-xs">
                  ...
                  {r.verificationMethod && (
                    <p className="text-muted">
                      verified · {r.verificationMethod.replaceAll('_', ' ').toLowerCase()}
                    </p>
                  )}
                </div>
              ),
            },
            {
              ...
              filter: {
                ...
                options: [
                  ...
                  {
                    label: t('delivery:state.RELEASE_REQUESTED.label'),
            ...
            },
            {
              ...
              render: (r: Delivery) => (
                <span className="text-xs text-muted">{relativeTime(r.requestedAt)}</span>
              ),
            },
            {
              key: 'actions',
