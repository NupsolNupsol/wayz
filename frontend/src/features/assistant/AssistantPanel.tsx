        ...
        <header className="flex items-center gap-3 px-4 py-3.5 border-b border-line dark:border-dk-border shrink-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-muted truncate">
              {context?.screenTitle
                ? t('onScreen', { screen: context.screenTitle })
                : t('subtitle')}
            </p>
          </div>
          <button
      ...
      <div className="max-w-[92%] w-full space-y-2.5">
        ...
        {turn.answer && !turn.failed && (
          <>
            ...
            {!turn.answer.grounded && <p className="text-[11px] text-muted">{t('notGrounded')}</p>}
          </>
        )}
      </div>
    </div>
...
function SpeakButton({ text, interactionId }: { text: string; interactionId: string }) {
  ...
  return (
    ...
    >
      {state === 'loading' ? <Loader2 size={12} className="animate-spin" /> : <Volume2 size={12} />}
      {state === 'failed'
...
function FeedbackButtons({ interactionId }: { interactionId: string }) {
  ...
  if (sent !== null) {
    return (
      <span className="lf-chip bg-emerald-50 text-success dark:bg-emerald-900/30 dark:text-emerald-300">
        {t('feedback.thanks')}
      </span>
    );
  }
