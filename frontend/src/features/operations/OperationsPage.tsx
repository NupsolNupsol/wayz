...
const endMs = (b: Booking) =>
  b.session.expectedEndAt ? new Date(b.session.expectedEndAt).getTime() : Number.MAX_SAFE_INTEGER;
const startedMs = (b: Booking) =>
  b.session.startedAt ? new Date(b.session.startedAt).getTime() : new Date(b.createdAt).getTime();
  ...
  const columns: Column<Booking>[] = [
    {
      ...
      sortValue: (b) => b.ref,
      filter: { kind: 'text', value: (b) => b.ref },
      render: (b) => <RefLink to={`/bookings/${b.id}`}>{b.ref}</RefLink>,
    },
    {
      key: 'client',
      header: t('common:column.client'),
      sortValue: (b) => b.customerName,
      filter: { kind: 'text', value: (b) => `${b.customerName} ${b.customerPhone}` },
      render: (b) => (
        <div>
          <p className="font-medium text-navy dark:text-dk-text">{b.customerName || '—'}</p>
    ...
    {
      key: 'engine',
      header: t('common:column.engine'),
      filter: { kind: 'select', options: ENGINE_OPTS, value: (b) => b.engineKind },
      render: (b) => (
        <span className="inline-flex items-center gap-2">
          <Icon name={ENGINE_META[b.engineKind].icon} size={16} className="text-brand" />{' '}
          <span className="text-muted">{engineLabel(b.engineKind)}</span>
        </span>
      ),
    },
    {
      ...
      filter: { kind: 'text', value: (b) => localBaked(b) },
      sortValue: (b) => localBaked(b),
      render: (b) => localBaked(b),
    },
    {
      key: 'unit',
      header: t('common:column.unit'),
      sortValue: (b) => unitName.get(b.assetUnitId ?? '') ?? '',
      render: (b) => (
        <span className="font-mono text-xs">{unitName.get(b.assetUnitId ?? '') ?? '—'}</span>
      ),
    },
    {
      key: 'started',
      ...
      filter: {
        kind: 'select',
        options: AGE_BUCKETS.map(({ label, value }) => ({ label, value })),
        value: bucketOf,
      },
      render: (b) => (
        <div>
    ...
    },
    {
      ...
      filter: { kind: 'select', options: STATUS_OPTS, value: (b) => b.status },
    ...
    },
    {
      ...
      render: (b) =>
        (b.amountDue ?? 0) > 0 ? (
          <Badge tone="warning" testId={`operations-due-${b.id}`}>
            {money(b.amountDue ?? 0)}
          </Badge>
        ) : (
    ...
    },
    {
      ...
      render: (b) =>
        b.session.startedAt ? (
          ...
          <span className="text-amber-600 text-xs">{t('operations.awaitingFulfilment')}</span>
        ),
    },
    {
      key: 'action',
    ...
    <div data-testid="operations-page">
      <PageHeader
        ...
        crumbs={[
          ...
          { label: t('common:crumb.operations') },
        ]}
