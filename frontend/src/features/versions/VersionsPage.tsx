...
export function VersionsPage() {
  ...
  const rows = useMemo(() => {
    ...
    return all.filter((v) =>
      [v.number, v.name, v.summary, ...v.areas, ...v.highlights]
        .join(' ')
        .toLowerCase()
        .includes(needle)
    );
    ...
    <div className="min-h-screen bg-canvas dark:bg-dk-bg" data-testid="versions-page">
      ...
      >
        ...
        <div className="relative max-w-[1080px] mx-auto px-5 sm:px-8 py-10 sm:py-14">
          <Link
            to="/login"
            className="inline-flex items-center gap-2.5 text-white no-underline mb-8"
            data-testid="versions-brand"
          >
          ...
          <p className="text-white/80 mt-3 max-w-xl text-sm sm:text-base">
            Every release we cut, in plain language — with the exact steps to test each change
            yourself.
          </p>
          ...
          <div className="lf-card p-10 text-center" data-testid="versions-empty">
            ...
            <p className="text-sm text-muted mt-1">
              {term
                ...
                : 'The first release will appear here the moment it is cut.'}
            </p>
          </div>
        ) : (
              ...
              <table className="w-full text-sm" data-testid="versions-table">
                <thead className="bg-canvas dark:bg-dk-elevated text-muted">
                  <tr className="text-start">
                    <th className="text-start font-semibold px-4 py-3 whitespace-nowrap">
                      Version
                    </th>
                    <th className="text-start font-semibold px-4 py-3">Release</th>
                    <th className="text-start font-semibold px-4 py-3 hidden md:table-cell">
                      What is in it
                    </th>
                    <th className="text-start font-semibold px-4 py-3 whitespace-nowrap">
                      Checked
                  ...
                  {rows.map((v, i) => (
                    ...
                    >
                      <td className="px-4 py-4 align-top whitespace-nowrap">
                        <Link
                          to={`/versions/${v._id}`}
                          className="font-mono font-bold text-brand no-underline hover:underline"
                        >
                          {v.number}
                        </Link>
                      ...
                      <td className="px-4 py-4 align-top">
                        ...
                        <p className="text-xs text-muted mt-1 md:hidden line-clamp-2">
                          {v.summary}
                        </p>
                      </td>
                      ...
                      <td className="px-4 py-4 align-top">
                        <p
                          className="tabular-nums font-semibold text-navy dark:text-dk-texthi"
                          data-testid={`versions-progress-${v._id}`}
                        >
                        ...
                        <div className="h-1.5 w-16 rounded-full bg-line dark:bg-dk-border mt-1.5 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${v.changeCount ? Math.round(((v.checkedCount ?? 0) / v.changeCount) * 100) : 0}%`,
                            }}
                          />
                        </div>
                        {(v.openIssues ?? 0) > 0 && (
                          <p
                            className="text-[11px] font-semibold text-amber-600 dark:text-amber-300 mt-1.5"
                            data-testid={`versions-issues-${v._id}`}
                          >
