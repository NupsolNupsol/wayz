...
export function VersionDetailPage() {
  ...
  if (isLoading) {
    return (
      <div
        className="min-h-screen bg-canvas dark:bg-dk-bg grid place-items-center"
  ...
  if (isError || !data) {
    return (
      <div
        className="min-h-screen bg-canvas dark:bg-dk-bg grid place-items-center px-6"
        data-testid="version-detail"
      >
        <div className="lf-card p-8 text-center max-w-md">
          ...
          <Link
            to="/versions"
            className="lf-btn-primary no-underline inline-flex"
    ...
    <div className="min-h-screen bg-canvas dark:bg-dk-bg" data-testid="version-detail">
      ...
      >
        ...
        <div className="relative max-w-[900px] mx-auto px-5 sm:px-8 py-9 sm:py-12">
          <div className="flex items-center justify-between gap-3 mb-8">
            <Link
              to="/versions"
              className="inline-flex items-center gap-2 text-white/85 hover:text-white no-underline text-sm font-semibold"
              data-testid="version-back"
            >
          ...
          <p className="font-mono text-sm text-white/80 mb-2">
            {data.number} · {dateOf(data.releasedAt)}
          </p>
      ...
      <main className="max-w-[900px] mx-auto px-5 sm:px-8 py-8 sm:py-10 flex flex-col gap-5">
        {data.changes.map((change, i) => (
          <article
            className="lf-card p-5 sm:p-6"
            key={`${change.title}-${i}`}
            data-testid={`version-change-${i}`}
          >
            ...
            <h2 className="text-lg sm:text-xl font-bold text-navy dark:text-dk-texthi">
              {change.title}
            </h2>
            {change.detail && (
              <p className="text-sm text-muted mt-2 leading-relaxed">{change.detail}</p>
            )}
            ...
            {change.expect && (
              <div className="mt-3 flex gap-2.5 rounded-xl2 border border-success/40 bg-emerald-50 dark:bg-emerald-900/20 p-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-navy dark:text-dk-text leading-relaxed">
                    {change.expect}
                  </p>
                </div>
              </div>
        ...
        <p className="text-xs text-muted text-center mt-2">
          <Link to="/versions" className="text-brand no-underline hover:underline">
            Every release
          </Link>
          {' · '}
