    ...
    <div data-testid="reconciliation-page">
      <PageHeader
        ...
        crumbs={[
          ...
          { label: t('common:crumb.settlement') },
          { label: t('common:crumb.reconciliation') },
        ]}
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-5">
          <StatCard
            ...
            sublabel={
              summary.credits.count
                ? t('reconciliation.refundedCount', { count: summary.credits.count })
                : t('reconciliation.noneReversed')
            }
            testId="txn-stat-count"
          />
          <StatCard
            label={t('reconciliation.gross')}
            value={money(summary.totals.grossAmount)}
            icon={<CreditCard size={18} />}
            tone="info"
            testId="txn-stat-gross"
          />
          <StatCard
          ...
          />
          <StatCard
            label={t('reconciliation.netSettled')}
      ...
      {reconciliation && (
        <>
          <SectionTitle className="mb-2 flex items-center gap-2">
            <Scale size={16} />
            {t('reconciliation.terminalAgainst')}
          </SectionTitle>
          <Card className="p-4 mb-6" data-testid="reconciliation-panel">
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-4">
              ...
              <div>
                <p className="text-xs uppercase tracking-wide text-muted font-semibold">
                  {t('reconciliation.amountDiffers')}
                </p>
                <p
                  className="text-2xl font-bold text-amber-500 tabular-nums"
                  data-testid="recon-mismatch"
                >
                  {reconciliation.totals.amountMismatch}
                </p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-wide text-muted font-semibold">
                  {t('reconciliation.cardDiffers')}
                </p>
                <p
                  className="text-2xl font-bold text-amber-500 tabular-nums"
                  data-testid="recon-scheme-mismatch"
                >
              ...
              <div>
                <p className="text-xs uppercase tracking-wide text-muted font-semibold">
                  {t('reconciliation.onlyTerminal')}
                </p>
                <p
                  className="text-2xl font-bold text-danger-strong tabular-nums"
                  data-testid="recon-terminal-only"
                >
              ...
              <div>
                <p className="text-xs uppercase tracking-wide text-muted font-semibold">
                  {t('reconciliation.onlyPlatform')}
                </p>
                <p
                  className="text-2xl font-bold text-danger-strong tabular-nums"
                  data-testid="recon-platform-only"
                >
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-line dark:border-dk-border">
              <div className="flex gap-6 text-sm">
                <span>
                  ...
                  <strong className="tabular-nums" data-testid="recon-terminal-total">
                    {money(reconciliation.totals.terminal)}
                  </strong>
                </span>
                <span>
                  <span className="text-muted">{t('reconciliation.platform')}</span>
                  <strong className="tabular-nums" data-testid="recon-platform-total">
                    {money(reconciliation.totals.platform)}
                  </strong>
                </span>
              </div>
              <Badge tone={reconciliation.totals.balanced ? 'success' : 'warning'}>
                {reconciliation.totals.balanced
                  ? t('reconciliation.balanced')
                  : t('reconciliation.needsAttention')}
              </Badge>
            </div>
          </Card>
          ...
          <DataTable
            ...
            keyOf={(r: ReconciliationRow, i?: number) =>
              `${r.transactionId ?? r.paymentId}-${i ?? 0}`
            }
            pageSize={12}
            initialSort={{ key: 'when', dir: 'desc' }}
            empty={{
              title: t('reconciliation.allMatch'),
              message: t('reconciliation.allMatchHint'),
            }}
            columns={[
              {
                key: 'ref',
                ...
                filter: {
                  kind: 'text',
                  value: (r: ReconciliationRow) => `${r.externalRef} ${r.paymentId ?? ''}`,
                },
                  ...
                  return (
                    <div>
                      {to ? (
                        <RefLink
                          to={to}
                          title={t('reconciliation.openRecord')}
                          testId={`recon-ref-${r.transactionId ?? r.paymentId}`}
                        >
                          {label}
                        </RefLink>
                      ) : (
                      ...
                      <p className="text-[11px] text-muted">
                        {r.transactionId
                          ? t('reconciliation.cardTransaction')
                          : t('reconciliation.platformPayment')}
                      </p>
                    </div>
                  );
                },
              },
              {
                ...
                render: (r: ReconciliationRow) => (
                  <Badge tone={RECON_TONES[r.status]}>{reconLabel(r.status)}</Badge>
                ),
              },
              {
                key: 'scheme',
                  ...
                  r.status === 'SCHEME_MISMATCH' ? (
                    <div className="text-xs">
                      <p className="text-navy dark:text-dk-text">
                        {t('reconciliation.terminalSaid')}{' '}
                        <strong>{r.scheme ? schemeLabel(r.scheme) : '—'}</strong>
                      </p>
                      <p className="text-danger-strong">
                        {t('reconciliation.agentSaid')}{' '}
                        <strong>{r.recordedScheme ? schemeLabel(r.recordedScheme) : '—'}</strong>
                      </p>
                    </div>
                  ) : r.scheme ? (
              ...
              },
              {
                ...
                render: (r: ReconciliationRow) =>
                  r.terminalAmount === null ? (
                    <span className="text-muted">—</span>
                  ) : (
                    <span className="tabular-nums">{r.terminalAmount.toFixed(2)}</span>
                  ),
              },
              {
                key: 'platform',
                ...
                render: (r: ReconciliationRow) =>
                  r.platformAmount === null ? (
                    <span className="text-muted">—</span>
                  ) : (
                    <span className="tabular-nums">{r.platformAmount.toFixed(2)}</span>
                  ),
              },
              {
                key: 'difference',
                ...
                render: (r: ReconciliationRow) => (
                  <strong
                    className={clsx(
                      'tabular-nums',
                      r.difference < 0 ? 'text-danger-strong' : 'text-amber-600'
                    )}
                  >
              ...
              },
              {
                ...
                render: (r: ReconciliationRow) => (
                  <span className="text-xs text-muted">
                    {r.capturedAt ? r.capturedAt.slice(0, 10) : '—'}
                  </span>
                ),
              },
            ]}
