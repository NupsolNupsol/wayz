...
export function AccountingDashboard() {
  ...
  const downloadActivity = (kind: EngineKind) =>
    run(kind, async () => {
      ...
      toast(
        'success',
        `${t(`common:engine.${kind}`)} workbook ready`,
        'Every transaction for that activity, with its own totals.'
      );
    });
  ...
  const downloadAll = () =>
    run('all', async () => {
      saveBlob(
        await accountingApi.downloadFullWorkbook({ from, to }),
        `wayz-all-activities-${from}_${to}.xlsx`
      );
  ...
  const presets: { label: string; range: { from: string; to: string }; current?: boolean }[] = [
    ...quartersOfYear(year).map((q, i) => ({
      label: q.label,
      range: q,
      current: i === currentQuarter,
    })),
  ...
  return (
    <div data-testid="accounting-dashboard">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-6 items-stretch">
            <Card className="p-4 flex flex-col" data-testid="accounting-vat-return">
              ...
              <div className="flex-1">
                {[
                  {
                    label: t('common:label.totalsalesexvat'),
                    base: vat.salesBase,
                    vat: vat.salesVat,
                    sign: '+',
                  },
                  {
                    label: t('common:label.totalreturnsexvat'),
                    base: vat.returnsBase,
                    vat: vat.returnsVat,
                    sign: '−',
                  },
                  {
                    label: t('common:label.totalpurchasesexpensesexvat'),
                    base: vat.purchasesBase,
                    vat: vat.purchasesVat,
                    sign: '−',
                  },
                ].map((row) => (
                  <div
                    key={row.label}
                    className="flex items-center justify-between gap-3 py-1.5 border-b border-line/70 dark:border-dk-border/70"
                  >
                ...
                <div className="flex items-center justify-between gap-3 pt-2.5">
                  <p className="font-semibold text-navy dark:text-dk-texthi text-sm">
                    {t('dashboard.netTaxableBase')}
                  </p>
                  <p
                    className="text-lg font-bold tabular-nums text-navy dark:text-dk-texthi"
                    data-testid="acct-net-base"
                  >
              ...
              <div className="flex items-center justify-between gap-3 pt-3 mt-2 border-t-2 border-line dark:border-dk-border">
                ...
                <p
                  className={clsx(
                    ...
                    vat.refundable ? 'text-brand' : 'text-danger-strong'
                  )}
            ...
            {zakat && (
              <Card className="p-4 flex flex-col" data-testid="accounting-zakat">
                ...
                <div className="flex-1">
                  {[
                    {
                      label: t('common:label.revenueafterreturnsexvat'),
                      value: zakat.revenue,
                      sign: '+',
                    },
                    {
                      label: t('common:label.costsrecordedbyhrexvat'),
                      value: zakat.costs,
                      sign: '−',
                    },
                    {
                      label: t('common:label.vatalreadypaidtozatca'),
                      value: zakat.vatPaid,
                      sign: '−',
                    },
                  ].map((row) => (
                ...
                <div className="flex items-center justify-between gap-3 pt-3 mt-2 border-t-2 border-line dark:border-dk-border">
                  <div>
                    <p className="font-semibold text-navy dark:text-dk-texthi">
                      {t('dashboard.zakatDue')}
                    </p>
                    <p className="text-xs text-muted">
                      {t('dashboard.ofNetProfit', { rate: (zakat.zakatRate * 100).toFixed(1) })}
                    </p>
                  </div>
                  <p
                    className={clsx(
                      'text-2xl font-bold tabular-nums',
                      zakat.profitable ? 'text-danger-strong' : 'text-muted'
                    )}
                ...
                {!zakat.profitable && (
                  <p className="text-xs text-muted mt-2" data-testid="acct-zakat-note">
                    {t('dashboard.zakatNotDue')}
                  </p>
                )}
              </Card>
            )}
      ...
      <div
        ...
        data-testid="accounting-activities"
      >
            ...
            <Card key={a.engineKind} className="p-4" data-testid={`acct-activity-${a.engineKind}`}>
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <span className="text-xs text-muted tabular-nums">
                  {summary.totals.salesBase > 0
                    ? ((a.salesBase / summary.totals.salesBase) * 100).toFixed(0)
                    : 0}
                  %
                </span>
              </div>
              ...
              <div className="h-1.5 rounded-full bg-line dark:bg-dk-border overflow-hidden mb-3">
                <div
                  ...
                  style={{ width: `${(a.salesBase / topSales) * 100}%` }}
                />
              </div>

              <p className="text-xl font-bold tabular-nums text-navy dark:text-dk-texthi">
                {money(a.netBase)}
              </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-4">
          ...
          <div className="lg:border-s lg:border-line dark:lg:border-dk-border lg:ps-6">
            ...
            <Button
              onClick={downloadAll}
              loading={busy === 'all'}
              data-testid="accounting-export-all"
            >
              <FileSpreadsheet size={16} />
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            filter: {
              kind: 'select',
              options: reportedActivities.map((k: EngineKind) => ({
                label: t(`common:engine.${k}`),
                value: k,
              })),
            ...
            render: (r: LedgerRow) =>
              r.engineKind ? (
                (t(`common:engine.${r.engineKind}`) ?? r.engineKind)
              ) : (
                <span className="text-muted">—</span>
              ),
          },
          {
            ...
            render: (r: LedgerRow) => (
              <span className="tabular-nums">{r.baseAmount.toFixed(2)}</span>
            ),
          },
          {
            key: 'vat',
          ...
          },
          {
            ...
            render: (r: LedgerRow) => (
              <strong className="tabular-nums">{r.totalAmount.toFixed(2)}</strong>
            ),
          },
        ]}
      />
