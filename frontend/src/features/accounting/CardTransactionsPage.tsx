      ...
      { transactions: parsed, source },
      {
        onSuccess: (res) => {
          ...
          if (!res.rejected.length) setImporting(false);
            ...
            setParseError(
              res.rejected
                .map((r) => `${r.externalRef || '(no reference)'}: ${r.reason}`)
                .join('\n')
            );
        },
        onError: (e) =>
          setParseError(
            e instanceof ApiError ? (e.errors?.join(' ') ?? e.message) : 'Import failed.'
          ),
      }
    );
  };
        ...
        columns={[
          {
            ...
            filter: {
              kind: 'text',
              value: (r: CardTransaction) => `${r.externalRef} ${r.authCode} ${r.maskedPan}`,
            },
            render: (r: CardTransaction) => (
              <div>
                <RefLink
                  to={`/accounting/settlement/transactions/${r._id}`}
                  testId={`txn-ref-${r._id}`}
                >
                  {r.externalRef}
                </RefLink>
          ...
          },
          {
            ...
            render: (r: CardTransaction) => (
              <span className="tabular-nums text-sm">{r.capturedAt.slice(0, 10)}</span>
            ),
          },
          {
            key: 'scheme',
          ...
          },
          {
            ...
            render: (r: CardTransaction) => (
              <strong className="tabular-nums">{r.netSettled.toFixed(2)}</strong>
            ),
          },
          {
            key: 'status',
            ...
            filter: {
              kind: 'select',
              options: ['CAPTURED', 'SETTLED', 'REFUNDED', 'REVERSED'].map((s) => ({
                label: s,
                value: s,
              })),
            ...
            render: (r: CardTransaction) => (
              <Badge
                tone={
                  r.status === 'SETTLED' ? 'success' : r.status === 'CAPTURED' ? 'info' : 'warning'
                }
              >
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={submitImport} loading={ingest.isPending} data-testid="import-submit">
