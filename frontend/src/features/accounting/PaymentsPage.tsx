    ...
    <div data-testid="payments-page">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-5">
        ...
        <StatCard
          ...
          value={money(sum(cash))}
          icon={<Banknote size={18} />}
          tone="info"
          sublabel={t('payments.paymentCount', { count: cash.length })}
          testId="pay-stat-cash"
        />
        <StatCard
          label={t('common:label.card')}
          value={money(sum(card))}
          icon={<CreditCard size={18} />}
          tone="success"
          sublabel={t('payments.paymentCount', { count: card.length })}
          testId="pay-stat-card"
        />
        <StatCard
        ...
        columns={[
          {
            ...
            filter: {
              kind: 'text',
              value: (r: LedgerPayment) => `${r._id} ${r.ref} ${r.customerName}`,
            },
            render: (r: LedgerPayment) => (
              <div>
                <RefLink
                  to={`/accounting/settlement/payments/${r._id}`}
                  testId={`payment-ref-${r._id}`}
                >
                  {r._id}
                </RefLink>
          ...
          },
          {
            ...
            render: (r: LedgerPayment) =>
              r.cardScheme ? (
                <span className="text-sm">{schemeLabel(r.cardScheme)}</span>
              ) : (
          ...
          },
          {
            ...
            filter: {
              ...
              value: (r: LedgerPayment) => r.kind,
            },
            render: (r: LedgerPayment) => (
              <Badge tone={r.kind === 'REFUND' ? 'warning' : 'success'}>
          ...
          },
          {
            ...
            render: (r: LedgerPayment) =>
              r.transactionId ? (
                <RefLink
                  to={`/accounting/settlement/transactions/${r.transactionId}`}
                  testId={`payment-txn-${r._id}`}
                >
                  {r.externalRef}
                </RefLink>
