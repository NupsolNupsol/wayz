...
export function CommissionRates() {
  ...
  const parsed = (scheme: CardScheme) => {
    ...
    return Number.isFinite(percentage) && (draft[scheme] ?? '') !== ''
      ? Number((percentage / 100).toFixed(6))
      : NaN;
  };
  const same = (a: number, b: number) => a.toFixed(6) === b.toFixed(6);
  const changed = rates.filter(
    (r) => Number.isFinite(parsed(r.scheme)) && !same(parsed(r.scheme), r.rate)
  );
  ...
  const submit = () => {
    const payload = Object.fromEntries(changed.map((r) => [r.scheme, parsed(r.scheme)])) as Partial<
      Record<CardScheme, number>
    >;
    save.mutate(
  ...
  const resetAll = () =>
    setDraft(Object.fromEntries(rates.map((r) => [r.scheme, (r.defaultRate * 100).toFixed(2)])));
    ...
    <div data-testid="commission-rates">
      ...
      <Card className="p-4 mb-6" data-testid="commission-rate-table">
        ...
        <DataTable
          ...
          columns={[
                ...
                return (
                  <div className="w-28">
                    <div className="relative">
                      ...
                      />
                      <span className="absolute end-2 top-1/2 -translate-y-1/2 text-muted text-xs">
                        %
                      </span>
                    </div>
            ...
            {
              key: 'contract',
              header: t('commissions.contract'),
              render: (r) => (
                <span className="text-muted tabular-nums text-xs">{percent(r.defaultRate)}</span>
              ),
            },
            {
              key: 'withheld',
              ...
              align: 'right',
              render: (r) => {
                const figures = summary?.byScheme.find(
                  (sc: SchemeFigures) => sc.scheme === r.scheme
                );
                return (
                  <span
                    className="tabular-nums font-semibold"
                    data-testid={`commission-withheld-${r.scheme}`}
                  >
        ...
        <label className="flex items-center gap-2 mt-3 text-xs cursor-pointer">
          <input
            ...
            data-testid="commission-reprice"
          />
      ...
      <Card className="p-3 mb-6 border-s-4 border-s-amber-400" data-testid="commission-treatment">
        <p className="font-semibold text-navy dark:text-dk-texthi text-sm">
          {t('commissions.howBooked')}
        </p>
        <p className="text-xs text-muted mt-0.5">{t('commissions.howBookedNote')}</p>
      </Card>
      ...
      <Card className="p-4" data-testid="commission-scheme-table">
        ...
        <DataTable
          ...
          pageSize={10}
          empty={{
            title: 'No card takings in this period',
            message: 'Pick another period, or import the terminal file.',
          }}
          columns={[
            {
              key: 'scheme',
              header: t('commissions.cardType'),
              sortValue: (r: SchemeFigures) => bilingual(r.label),
              render: (r: SchemeFigures) => (
                <span className="font-medium text-navy dark:text-dk-texthi">
                  {bilingual(r.label)}
                </span>
              ),
            },
            {
              key: 'rate',
              header: t('commissions.rate'),
              align: 'right',
              sortValue: (r: SchemeFigures) => r.rate,
              render: (r: SchemeFigures) => (
                <span className="tabular-nums text-muted">{percent(r.rate)}</span>
              ),
            },
            {
              key: 'count',
              header: t('common:column.transactions'),
              align: 'right',
              sortValue: (r: SchemeFigures) => r.count,
              render: (r: SchemeFigures) => <span className="tabular-nums">{r.count}</span>,
            },
            {
              key: 'gross',
              header: t('commissions.gross'),
              align: 'right',
              sortValue: (r: SchemeFigures) => r.grossAmount,
              render: (r: SchemeFigures) => (
                <span className="tabular-nums">{r.grossAmount.toFixed(2)}</span>
              ),
            },
            {
              key: 'commission',
              ...
              align: 'right',
              sortValue: (r: SchemeFigures) => r.commissionAmount,
              render: (r: SchemeFigures) => (
            ...
            },
            {
              ...
              align: 'right',
              sortValue: (r: SchemeFigures) => r.netSettled,
              render: (r: SchemeFigures) => (
                <span className="tabular-nums">{r.netSettled.toFixed(2)}</span>
              ),
            },
            {
              key: 'share',
              header: t('commissions.share'),
              align: 'right',
              sortValue: (r: SchemeFigures) => r.share,
              render: (r: SchemeFigures) => (
          ...
          footer={(shown: SchemeFigures[]) => {
            ...
            return [
              ...
              <span className="tabular-nums">
                {shown.reduce((t, r) => t + r.netSettled, 0).toFixed(2)}
              </span>,
              <span className="tabular-nums">
                {shown.reduce((t, r) => t + r.share, 0).toFixed(1)}%
              </span>,
            ];
          }}
        />
