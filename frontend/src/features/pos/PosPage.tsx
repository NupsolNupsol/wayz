  ...
  return (
    <div data-testid="pos-page">
      <PageHeader
        ...
        crumbs={[{ label: t('pos.home'), to: '/dashboard' }, { label: t('pos.title') }]}
      />
        ...
        {engines.map((kind) => {
          ...
          return (
            <button
              ...
              data-testid={`engine-${kind}`}
              className="text-start"
            >
              <Card className="lf-card-hover h-full">
                <div className="flex items-start gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-brand text-brand-fg flex items-center justify-center shrink-0">
                    <Icon name={m.icon} size={22} />
                  </div>
                  <div className="flex-1">
                    <h3 className="font-bold text-navy dark:text-dk-texthi">
                      {t(`common:engine.${kind}`)}
                    </h3>
