...
export function SignInPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t('login.failed', { defaultValue: 'Could not sign in.' }))
    } finally {
      setLoading(false)
    }
  }
            ...
            <form onSubmit={submit} className="w-full max-w-[340px]" data-testid="login-form">
              ...
              <p className="text-sm text-muted mb-8">
                {t('login.neutralSubtitle', {
                  defaultValue: 'Sign in with your work address. We will take you to your organisation.',
                })}
              </p>
              ...
              {error && (
                <p className="text-sm text-danger-strong mt-2" role="alert" data-testid="login-error">
                  {error}
                </p>
              )}
              ...
              >
                {loading ? t('login.signingIn', { defaultValue: 'Signing in…' }) : t('login.signIn')}
              </button>
            </form>
          ...
          <div className="mt-6" data-testid="demo-logins">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {organisations.map((org) => (
                <div key={org.id} className="bg-surface rounded-2xl p-4 shadow-sm" data-testid={`demo-org-${org.id}`}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="ms-auto text-[11px] text-muted">{org.activities.length} activities</span>
                  </div>
