      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
        ...
        <Card className="lg:col-span-2">
          ...
          <dl className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            <Meta
              label={t('common:column.identifier')}
              value={<RefText>{data.identifier}</RefText>}
            />
            <Meta
              label={t('common:column.kind')}
              value={t(`kind.${data.assetTypeKind}`, { defaultValue: data.assetTypeKind ?? '—' })}
            />
            <Meta
              label={t('common:column.activity')}
              value={data.engineKind ? engineLabel(data.engineKind) : '—'}
            />
            <Meta
              ...
              value={
                <span className="flex items-center gap-2">
                  ...
                  {data.priceOverride !== null && (
                    <Badge tone="warning">{t('table.ownPrice')}</Badge>
                  )}
                </span>
              }
            />
          ...
          {mayManage && (
            <div className="flex flex-wrap gap-2 mt-5">
              {data.status === 'AVAILABLE' && (
                <Button
                  variant="secondary"
                  onClick={() => quickStatus('OUT_OF_SERVICE')}
                  data-testid="asset-unit-suspend"
                >
                  {t('action.suspend')}
                </Button>
              )}
              {['OUT_OF_SERVICE', 'MAINTENANCE', 'BLOCKED'].includes(data.status) && (
                <Button
                  variant="secondary"
                  onClick={() => quickStatus('AVAILABLE')}
                  data-testid="asset-unit-restore"
                >
                  {t('action.restore')}
                </Button>
              )}
              {!SETTABLE.includes(data.status) && (
                <>
                  <Button
                    variant="danger"
                    onClick={() => quickStatus('AVAILABLE')}
                    data-testid="asset-unit-free"
                  >
                    {t('action.forceFree')}
                  </Button>
                  <p className="text-xs text-muted self-center">
                    {t('edit.statusLocked', { status: data.status.replaceAll('_', ' ') })}
                  </p>
                </>
              )}
            </div>
        ...
        footer={
          <>
            ...
            <Button
              ...
              data-testid="asset-unit-edit-submit"
            >
      ...
      >
        <Field label={t('common:column.identifier')} required>
          <input
            ...
            data-testid="asset-unit-identifier"
          />
        </Field>
        <Field
        ...
        <label className="flex items-start gap-2 text-sm cursor-pointer select-none mb-3">
          <input
            ...
            data-testid="asset-unit-own-price"
          />
          <span>
            {t('edit.ownPrice', { price: data.basePrice === null ? '—' : money(data.basePrice) })}
          </span>
        </label>
        {ownPrice && (
        ...
        <label className="flex items-start gap-2 text-sm cursor-pointer select-none mb-3">
          <input
            ...
            data-testid="asset-unit-own-penalty"
          />
          <span>
            {t('edit.ownPenalty', {
              price: data.effectivePenalty ? money(data.effectivePenalty) : '—',
            })}
          </span>
        </label>
        {ownPenalty && (
          <Field label={t('edit.unitPenalty')} required hint={t('edit.unitPenaltyHint')}>
            <NumberInput
              ...
              testId="asset-unit-penalty-input"
            />
          </Field>
        )}
      </Modal>
