...
export function AssetTypeDetailPage() {
  ...
  const openAdd = () => {
    setStationId(
      data.stations.find((s) => s.engineKinds.includes(type.engineKind))?._id ??
        data.stations[0]?._id ??
        ''
    );
    setKioskId('');
    ...
    addUnits.mutate(
      {
        ...
        body: {
          stationId,
          ...(atAGate ? { gateId: kioskId || null } : { kioskId: kioskId || null }),
          count,
        },
      },
      {
  ...
  const columns: Column<AssetUnitRow>[] = [
    ...
    },
    {
      ...
      filter: {
        kind: 'select',
        options: [...new Set(data.units.map((u) => u.status))].map((s) => ({
    ...
    },
    {
      ...
      filter: {
        kind: 'select',
        options: data.stations.map((s) => ({ label: s.name, value: s.name })),
        value: (r) => r.stationName,
      },
      render: (r) => (
      ...
      render: (r) => (
        <div className="text-end">
          <span className="tabular-nums">
            {r.effectivePrice === null ? '—' : money(r.effectivePrice)}
          </span>
      ...
      render: (r) =>
        r.currentBookingRef ? (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              navigate(`/bookings/${r.currentBookingId}`);
            }}
          >
      ...
      render: (r) => (
        <div className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}>
          {r.status === 'AVAILABLE' && (
            <Button
              variant="ghost"
              onClick={() => quickStatus(r, 'OUT_OF_SERVICE')}
              data-testid={`asset-suspend-${r._id}`}
            >
              {t('action.suspend')}
            </Button>
          )}
          {['OUT_OF_SERVICE', 'MAINTENANCE', 'BLOCKED'].includes(r.status) && (
            <Button
              variant="ghost"
              onClick={() => quickStatus(r, 'AVAILABLE')}
              data-testid={`asset-restore-${r._id}`}
            >
              {t('action.restore')}
            </Button>
          )}
          {!SETTABLE.includes(r.status) && (
            <Button
              variant="ghost"
              onClick={() => setFreeing(r)}
              data-testid={`asset-free-${r._id}`}
            >
              {t('action.forceFree')}
            </Button>
          )}
          ...
          <Button
            ...
            data-testid={`asset-remove-${r._id}`}
          >
            <Trash2 size={15} />
          </Button>
        </div>
    ...
    <div data-testid="asset-type-detail">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-5">
        <StatCard
          ...
          value={type.total}
          icon={<Boxes size={18} />}
          testId="asset-detail-total"
        />
        <StatCard
          label={t('table.inUse')}
          value={type.inUse}
          tone="info"
          testId="asset-detail-inuse"
        />
        <StatCard
          label={t('table.free')}
          value={type.available}
          tone="success"
          testId="asset-detail-free"
        />
        <StatCard
          label={t('table.down')}
          value={type.outOfService}
          tone={type.outOfService ? 'danger' : 'neutral'}
          testId="asset-detail-down"
        />
        <StatCard
          label={t('table.price')}
      ...
      */}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted" dir="auto">
                    {product.nameAr || t('price.noProductShort')} ·{' '}
                    {billingLabel(product.billingModel, product.engineKind)}
                  </p>
                </div>
        ...
        footer={
          <>
            ...
            <Button
              ...
              disabled={!stationId || (!kioskId && !heldByArea) || count < 1}
              data-testid="asset-detail-add-submit"
            >
              {t('add.submit', { count })}
            </Button>
          </>
      ...
      >
        ...
        >
          {placesHere.length > 0 ? (
            ...
            <p className="text-xs text-muted" data-testid="asset-detail-add-area">
      ...
      >
        <Field label={t('common:column.identifier')} required>
          <input
            ...
            data-testid="asset-edit-identifier"
          />
        </Field>
        <Field
          label={t('common:column.status')}
          hint={
            editing && !SETTABLE.includes(editing.status)
              ? t('edit.statusLocked', { status: editing.status.replaceAll('_', ' ') })
              : undefined
          }
        >
          <Select
            ...
            options={SETTABLE.map((s) => ({
              label: t(`status.${s}`, { defaultValue: s.replaceAll('_', ' ') }),
              value: s,
            }))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
          ...
          <Field label={t('edit.desk')} hint={t('edit.deskHint')}>
            <Select
              ...
              options={[
                { label: t('edit.noDesk'), value: '' },
                ...desksForMove.map((k) => ({ label: k.name, value: k._id })),
              ]}
              disabled={!!editing && !SETTABLE.includes(editing.status)}
        ...
        <label className="flex items-start gap-2 text-sm cursor-pointer select-none mb-3">
          <input
            ...
            data-testid="asset-edit-own-price"
          />
          <span>
            {t('edit.ownPrice', { price: type.basePrice === null ? '—' : money(type.basePrice) })}
          </span>
        </label>
        {ownPrice && (
        ...
        <label className="flex items-start gap-2 text-sm cursor-pointer select-none mb-3">
          <input
            ...
            className="mt-0.5"
            checked={ownPenalty}
            onChange={(e) => setOwnPenalty(e.target.checked)}
            data-testid="asset-edit-own-penalty"
          />
          <span>
            {t('edit.ownPenalty', { price: type.penaltyPrice ? money(type.penaltyPrice) : '—' })}
          </span>
        </label>
        {ownPenalty && (
          <Field label={t('edit.unitPenalty')} required hint={t('edit.unitPenaltyHint')}>
            <NumberInput
              ...
              step={0.5}
              value={unitPenalty}
              onChange={setUnitPenalty}
              testId="asset-edit-penalty"
            />
          </Field>
        )}
      </Modal>
        ...
        footer={
          <>
            ...
            <Button
              ...
              data-testid="asset-remove-submit"
            >
      ...
      <KindEditorModal
        ...
        kind={{
          ...
          maxRecommendedBagCount:
            (type.capacity?.maxRecommendedBagCount as number | undefined) ?? null,
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button
              onClick={submitPrice}
              loading={priceType.isPending}
              data-testid="asset-detail-price-submit"
            >
              {t('price.submit')}
            </Button>
          </>
          ...
          <>
            <Field label={t('price.base')} required hint={t('price.baseHint')}>
              <NumberInput
                ...
                testId="asset-detail-price-base"
              />
            </Field>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
            ...
            <Field label={t('price.deposit')}>
              <NumberInput
                ...
                value={deposit}
                onChange={setDeposit}
                testId="asset-detail-price-deposit"
              />
            </Field>
            <Field label={t('price.penalty')} hint={t('price.penaltyHint')}>
              <NumberInput
                ...
                value={penalty}
                onChange={setPenalty}
                testId="asset-detail-price-penalty"
              />
            </Field>
            {saleType === 'RENTAL' && (
              <Field label={t('price.overtime')} hint={t('price.overtimeHint')}>
                <NumberInput
                  ...
                  testId="asset-detail-price-overtime"
                />
              </Field>
            )}
            <label className="flex items-start gap-2 text-sm cursor-pointer select-none">
              <input
                ...
                data-testid="asset-detail-price-clear"
              />
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setFreeing(null)}>
              {t('common:action.cancel')}
            </Button>
            <Button
              variant="danger"
