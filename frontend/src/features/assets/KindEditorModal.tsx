    ...
    try {
      const capacity =
        kind.kind === 'COMPARTMENT'
          ? {
              maxRecommendedBagCount: Math.max(1, maxBags),
      ...
      if (clearOverrides) {
        const cleared = await priceType.mutateAsync({
          id: kind._id,
          body: { clearOverrides: true },
        });
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
        <Field
          ...
          hint={t('price.billingWillBe', { billing: billingLabel(billing, kind.engineKind) })}
        >
          <Select
            value={saleUnit}
            onChange={(v) => setSaleUnit(v as SaleUnit)}
            options={saleUnitsFor(kind.engineKind, SALE_UNITS).map((value) => ({
          ...
          <>
            <Field label={t('products.tourPrice')}>
              <NumberInput
                ...
                step={0.5}
                value={tourPrice}
                onChange={setTourPrice}
                testId="product-tour-price"
              />
            </Field>
            <Field label={t('products.tourMinutes')}>
              <NumberInput
                ...
                step={5}
                value={tourMinutes}
                onChange={setTourMinutes}
                testId="product-tour-minutes"
              />
            </Field>
          </>
        )}
