...
export const bookingApi = {
  list: (params?: { status?: string; engineKind?: EngineKind }) =>
    unwrap<Booking[]>(http.get('/bookings', { params })),
  get: (id: string) => unwrap<Booking>(http.get(`/bookings/${id}`)),
  order: (id: string) => unwrap<Order>(http.get(`/bookings/${id}/order`)),
  transitions: (id: string) =>
    unwrap<{ allowed: boolean; message: string; transitions: AvailableTransition[] }>(
      http.get(`/bookings/${id}/transitions`)
    ),
  create: (input: CreateBookingInput) =>
    unwrap<{ booking: Booking; order: Order }>(http.post('/bookings', input)),
  discount: (
    id: string,
    body: { reasonCode: string; percent?: number; amount?: number; note?: string }
  ...
  pay: (
    ...
    unwrap<{ booking: Booking; order: Order; receipt: Receipt }>(
      http.post(`/bookings/${id}/pay`, { splits })
    ),
  redeemVoucher: (id: string, code: string) =>
    unwrap<{ code: string; percent: number; name: string; order: Order }>(
      http.post(`/bookings/${id}/voucher`, { code })
    ),
  reserve: (id: string, unitId?: string) =>
    unwrap<Booking>(http.post(`/bookings/${id}/reserve`, unitId ? { unitId } : {})),
  reassign: (id: string, unitId: string, reason: string) =>
    unwrap<Booking>(http.post(`/bookings/${id}/reassign`, { unitId, reason })),
  scanOut: (id: string, barcode: string) =>
    unwrap<Booking>(http.post(`/bookings/${id}/scan-out`, { barcode })),
  settle: (
    id: string,
    splits: { method: PaymentMethod; cardScheme?: string | null; amount: number }[]
  ) =>
  ...
  returnHere: (id: string, code: string) =>
    unwrap<{ booking: Booking; wrongStation: boolean }>(
      http.post(`/bookings/${id}/return`, { code })
    ),
  ...
  transition: (id: string, code: string, payload?: TransitionPayload) =>
    unwrap<Booking>(http.post(`/bookings/${id}/transition`, { code, payload })),
  ...
  sendVerification: (
    ...
    unwrap<VerificationChallenge>(
      http.post(`/bookings/${id}/verification/send`, { purpose, channel })
    ),
  confirmVerification: (
    id: string,
