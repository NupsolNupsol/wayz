...
export const tripApi = {
  boats: (assetTypeId?: string) =>
    unwrap<BoatSpace[]>(
      http.get('/lagoon/trips/boats', { params: assetTypeId ? { assetTypeId } : undefined })
    ),
  release: (id: string) => unwrap<Trip>(http.post(`/lagoon/trips/${id}/release`, {})),
  board: (mine = false) =>
    unwrap<TripBoard>(http.get('/lagoon/trips', { params: mine ? { mine: 'true' } : undefined })),
  detail: (id: string) =>
    unwrap<{ trip: Trip; stations: MapPoint[] }>(http.get(`/lagoon/trips/${id}`)),
  setRoute: (id: string, stationIds: string[]) =>
    unwrap<Trip>(http.post(`/lagoon/trips/${id}/route`, { stationIds })),
  claim: (id: string) => unwrap<Trip>(http.post(`/lagoon/trips/${id}/claim`, {})),
  start: (id: string, unitId?: string) =>
    unwrap<Trip>(http.post(`/lagoon/trips/${id}/start`, unitId ? { unitId } : {})),
  clock: (id: string, stationId: string) =>
    unwrap<Trip>(http.post(`/lagoon/trips/${id}/stops`, { stationId })),
