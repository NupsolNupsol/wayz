...
export const assetApi = {
  estate: (engineKind?: EngineKind) =>
    unwrap<AssetEstate>(
      http.get('/assets/types', { params: engineKind ? { engineKind } : undefined })
    ),
  ...
  removeType: (id: string) =>
    unwrap<{ removed: string; name: string }>(http.delete(`/assets/types/${id}`)),
  typeDetail: (id: string) => unwrap<AssetTypeDetail>(http.get(`/assets/types/${id}`)),
  addUnits: (id: string, body: AddUnitsInput) =>
    unwrap<{ created: number; identifiers: string[] }>(
      http.post(`/assets/types/${id}/units`, body)
    ),
  ...
  returnPosition: (id: string) =>
    unwrap<UnitReturnPosition>(http.get(`/assets/units/${id}/return-position`)),
  updateUnit: (id: string, body: UnitPatch) =>
    unwrap<AssetUnitDetail>(http.patch(`/assets/units/${id}`, body)),
  removeUnit: (id: string) =>
    unwrap<{ removed: string; identifier: string }>(http.delete(`/assets/units/${id}`)),
};
