...
export interface PlatformReport {
  ...
  adoption: { kind: EngineKind; label: { en: string; ar: string }; organisations: number; bookings: number; revenue: number }[]
}
...
export const platformApi = {
  ...
  organisations: () => unwrap<{ organisations: PlatformOrganisation[] }>(http.get('/platform/organisations')),

  createOrganisation: (body: CreateOrganisationInput) =>
    unwrap<PlatformOrganisation>(http.post('/platform/organisations', body)),

  updateOrganisation: (id: string, body: { name?: string; activities?: EngineKind[]; branding?: Record<string, string> }) =>
  ...
  removeKnowledge: (id: string) => unwrap<{ removed: string }>(http.delete(`/platform/knowledge/${id}`)),
}
