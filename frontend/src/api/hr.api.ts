...
export const categoryLabel = (category: ExpenseCategory | string): string =>
  i18n.t(`status:expenseCategory.${category}`, {
    defaultValue:
      ENGLISH_CATEGORIES[category as ExpenseCategory] ?? String(category).replaceAll('_', ' '),
  });

export const SYSTEM_CATEGORIES: ExpenseCategory[] = ['BANK_COMMISSION'];

export const ENTERABLE_CATEGORIES = EXPENSE_CATEGORIES.filter(
  (c) => !SYSTEM_CATEGORIES.includes(c)
);
...
export const hrApi = {
  ...
  hours: (params: { from?: string; to?: string } = {}) =>
    unwrap<HoursWorked>(http.get('/hr/hours', { params })),
  ...
  voidExpense: (id: string, reason: string) =>
    unwrap<Expense>(http.post(`/hr/expenses/${id}/void`, { reason })),
