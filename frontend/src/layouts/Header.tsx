...
export function Header({
   ...
   */
  const where =
    [me.station?.name, me.kiosk?.name].filter(Boolean).join(' · ') || t('header.noDesk');
  ...
  return (
    ...
    >
      ...
      <div className="hidden lg:block shrink-0 min-w-0">
        ...
        <div
          className="text-xs text-muted mt-1 flex items-center gap-1 truncate"
          data-testid="header-where"
        >
        ...
        {searchable && (
          <div className="relative w-full max-w-[440px]" ref={searchRef}>
            ...
            {searchOpen && query.trim().length >= 2 && (
              <div
                className="absolute top-[calc(100%+6px)] left-0 right-0 lf-card p-1 shadow-pop z-[9999] max-h-[70vh] overflow-y-auto scroll-thin"
                ...
                {results.length === 0 ? (
                  ...
                  results.map((hit) => (
                    ...
                    >
                      ...
                      <span className="text-muted text-xs ms-auto truncate max-w-[45%]">
                        {hit.sublabel}
                      </span>
                    </button>
                  ))
      ...
      <div className="flex items-center gap-1 shrink-0" ref={ref}>
        <span
          ...
          className={clsx(
            'p-2 rounded-lg',
            online ? 'text-success' : 'text-danger-strong bg-red-50'
          )}
        >
        ...
        <button
          ...
          className="p-2 rounded-lg text-muted hover:bg-black/5"
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2.5 px-2 py-1.5 rounded-[10px] hover:bg-black/5"
            data-testid="user-menu-btn"
          >
            <div className="w-[34px] h-[34px] rounded-full bg-brand text-brand-fg text-[13px] font-bold flex items-center justify-center">
              {initials(me.fullName)}
            </div>
            <div className="hidden md:block text-start leading-tight max-w-[120px]">
              <div className="text-[13px] font-semibold text-navy dark:text-dk-texthi truncate">
                {me.fullName}
              </div>
          ...
          {userOpen && (
            <div
              className="absolute top-[calc(100%+8px)] end-0 w-44 lf-card p-1 shadow-pop z-[9999]"
            ...
            >
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm hover:bg-red-50 text-danger-strong text-start"
